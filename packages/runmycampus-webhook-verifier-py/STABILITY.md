...
`runmycampus-webhook-verifier` PyPI package, the versioning policy
...
`runmycampus_webhook_verifier` package and are considered **stable**
...
These names form the source of truth that the package's
`runmycampus_webhook_verifier.__all__` enumerates. Anything not listed
above is **internal** — its name, signature, and existence may change
in any release.
...
| Change shape                                    | Bump   |
|-------------------------------------------------|--------|
| Removing an exported name; renaming a parameter; changing a parameter type; flipping a default that alters behavior; removing or renaming a class | **major** |
| Adding a new exported name; adding a new keyword-only parameter with a default that preserves prior behavior; adding a new class | **minor** |
...
3. Calling the deprecated name emits a
   `warnings.DeprecationWarning` whose message points at the
   replacement.
...
|-------------|---------|-----------|
| **Stable**  | `verify`, `verify_signature_strict`, `compute_signature`, `canonicalize`, `canonical_sha256_hex`, `VerifyResult`, all `*Error` classes, all `*_HEADER` and `*_PREFIX` and `*_SECONDS` constants, `__version__` | Frozen per the versioning policy above. |
| **Deprecated alias** | `verify_signature` | Continues to work; emits `DeprecationWarning`. New code should use `verify(...)` which returns a `VerifyResult` carrying the legacy-header-family flag. Removal scheduled for 2.0 (no earlier than 90 days after 1.0.0 ships). |
| **Unstable** | Any name prefixed with `_` (e.g. `_coerce_bytes`, `_coerce_header`, `_get_header_case_insensitive`); any module path under `runmycampus_webhook_verifier._*`. | May change in any release, including patches. Do not import. |

## Legacy-header dual-emit window

The `verify(...)` function accepts a keyword-only argument
`accept_legacy` whose default is **`True`** in the 1.x line. This
preserves compatibility for subscribers still reading the original
...
Subscribers who have migrated should pass `accept_legacy=False`
