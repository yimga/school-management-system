...
no exported name has been removed, no parameter type has been
tightened, and no default has been flipped between 0.2.0 and 1.0.0.

If you were already using `from runmycampus_webhook_verifier import
verify, VerificationError, ...` in 0.2.0, your code continues to work
unchanged in 1.0.0.
...
- `verify_signature()` is now formally a deprecated alias for
  `verify()`. It still works; it now emits a
  `warnings.DeprecationWarning` to nudge you toward the new API.
...
- `verify(..., accept_legacy=True)` — the default flips from `True` to
  `False`. After the 2026-05-18 → 2026-08-18 dual-emit window closes
  and the dispatcher stops emitting the legacy `X-Migration-Cloud-*`
  family entirely, the SDK default-on for legacy acceptance becomes a
  silent failure-attractor. The flip is gated behind at least 90 days
  of field test after 1.0.0 ships and at least 90 days after the
  dispatcher cutover, whichever is later.
  ...
  **Forward-compatible idiom (works in 0.2.0+ AND in 2.0+):**
  ...
  result = verify(request.headers, body_bytes, secret, accept_legacy=False)
  ```
...
- `verify_signature()` (the deprecated alias) is removed. Replace with
  `verify(...)` returning a `VerifyResult`, or with
  `verify_signature_strict(...)` if you want exception-based control
...
1. Replace `verify_signature(body, sig_header, secret, ...)` with
   `verify(request.headers, body, secret).valid` — note that `verify`
   takes the entire header map, not just the signature value, so it
   can transparently fall back across header families.
2. Pass `accept_legacy=False` explicitly once your stack has migrated
   to reading the canonical `X-RunMyCampus-*` family.
3. Inspect `result.used_legacy_header_family` and emit a one-line
