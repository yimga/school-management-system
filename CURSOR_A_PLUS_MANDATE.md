...
**W24 — Immunization & health records (NEW).** Medical-record tracking + automated missing-vaccine / health-filter alerts for rising grades (year-end + intake). **STATUS 2026-08-13 — increment 1 SHIPPED `ea368593c`** (10/10 must-fire tests green): structured `VaccineRequirement` + `ImmunizationRecord` models (schoolops; FK mirrors HealthRecord so cross-tenancy-fk holds); pure `compute_missing_immunizations`; per-school `check_missing_immunizations_task` sweep reusing the badge-expiry rail → PII-safe guardian `finance.Notification`; runnable `manage.py check_missing_immunizations`. Behaviour-preserving (no requirement rows → no alerts). Residual (follow-ups): nurse compliance UI, intake gate, beat schedule + rollover-loop wiring (the year-end trigger), age-hint enforcement.
