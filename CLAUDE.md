...
| `audit_role_permission_matrix.py` | n/a (flag-pinned at **36** via `--max-candidate-anonymous 36`, not a JSON baseline). Was 56 at v2.80 introduction; v2.82 recognized 12 more project-specific auth decorators (`require_super_access_with_host`, `require_super_access`, `require_control_plane_access`, `require_school`, `require_school_permission`, `require_feature`, `require_parent_child_access`, `finance_access_required`, `evaluation_access_required`, `mfa_required`, `observability_auth_required`, `webhook_security_required`) and refactored `student_portal_grades` + `regulatory_export` from inline auth checks to `@login_required` so the AST can see them. | `architectural-boundaries.yml::rbac-matrix` | AST-walks `apps/*/views*.py` for auth decorators (`@login_required`, `@role_required`, `@permission_required`, DRF `permission_classes`) and joins to `apps/*/urls.py` patterns. Fails CI when a NEW route lands without recognized auth (the `candidate_anonymous` count exceeds the gate). Outputs JSON + CSV at `docs/generated/role_permission_matrix.{json,csv}`. SOT for triage at `docs/ROLE_PERMISSION_MATRIX_2026_05_16.md`. **Honest caveat**: the scanner is blind to inline `if not request.user.is_authenticated` patterns and middleware-based protection — when a route is auth-checked via something the AST can't see, refactor to a decorator OR add `# rbac-allow: <reason>` marker. |
