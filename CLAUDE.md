...
| `scan_magic_numbers.py` | **485** (v2.53+N1; was 482 at v2.47, +3 drift caught by `check_documented_baselines.py` — natural growth from new constants in code added since v2.47) | `architectural-boundaries.yml` | Integer literals `abs(v) >= 100` outside `config/` / `*/constants.py` should be named constants. Drift-detection only. Mark allowed sites with `# magic-number-allow: <reason>`. |
