...
| `scan_magic_numbers.py` | ~1104 (v2.36; was ~2776 at v2.22 — RFC HTTP status codes 2xx/3xx/4xx/5xx added to `ALLOWED_LITERALS` since they're universally understood, not business-rule magic) | `architectural-boundaries.yml` | Integer literals `abs(v) >= 100` outside `config/` / `*/constants.py` should be named constants. Drift-detection only. Mark allowed sites with `# magic-number-allow: <reason>`. |
