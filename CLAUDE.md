...
|---|---|---|---|
| `scan_tenant_queryset_safety.py` | 741 | `tenant-isolation-scan.yml` | Tenant-scoped models need `school=` / `school_id=` / `school__isnull=` kwargs on `.filter/.get/.all/.update/.delete`. Mark safe cross-tenant queries with `# tenant-isolation-allow: <reason>`. |
...
| `scan_migration_model_imports.py` | 33 | `architectural-boundaries.yml` | Migrations use `apps.get_model("X", "Y")` for historical-state safety; do not import live models. |
...
| `scan_role_strings.py` | 367 | `architectural-boundaries.yml` | Hardcoded role names (`ADMIN`/`TEACHER`/`PARENT`/`STUDENT`/`PROPRIETOR`) must reference `apps.platform_runtime.role_registry` or `User.Role` TextChoices. Both are SOT (registered in scanner `SOT_MODULES`). Mark other allowed sites with `# role-string-allow: <reason>`. |
...
| `scan_magic_numbers.py` | ~2776 | `architectural-boundaries.yml` | Integer literals `abs(v) >= 100` outside `config/` / `*/constants.py` should be named constants. Drift-detection only (large baseline). Mark allowed sites with `# magic-number-allow: <reason>`. |
