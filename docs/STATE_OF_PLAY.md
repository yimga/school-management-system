...
**Last updated:** 2026-05-11 (passes 7 → 14 — both A-waves and B-waves closed; C-waves remain only for infra-credentialed work)
...
|---|---|---|---|
| **8.C** | Importer rebuild — wave 3: wizard-side JS poll loop driving a progress bar against `/api/v1/migration-jobs/<id>/`, downloadable error CSV from snapshots, full Fees/Payments persistence to Invoice + Payment honoring Part F 25.1 invoice immutability, SubjectAssignment auto-create in the roster importer (needs Specialty resolution helper), vendor schema_hints for FACTS / Skyward / Alma | 1-2 wk | Full PowerSchool migration in <1 hour for 5k-student schools |
| **9.C** | Audit-log UI wave 3 — operator-facing queue page for ExportJob / EraseRequest with approve / reject / complete workflow + SLA tracking columns (extend the existing models with `due_at` + `sla_breach_at`), evaluation drill-down once the canonical detail view lands, sample `@audit_pii_view` applications on the top 5 PII detail views | 3-5 days | SOC 2 audit window readiness |
| **10.C** | Accessibility wave 3 — operator-driven `inject_table_captions --write` runs against each template subdirectory (template-author review per batch), authenticated-template axe coverage once a Selenium session fixture lands, wire CI to fail when `RUN_A11Y_TESTS=1` finds severe violations | 2-3 wk | US public-district sales (Section 508) hard requirement |
| **11.C** | Observability wave 3 — full RUM transaction sampling (needs a CDN-allowlisted `@sentry/browser` bundle or a self-hosted vendored copy), `sentry_sdk.start_transaction` decorators around attendance.submit / grade.publish / parent dashboard render, promote `sentry/alerts.yml` rules to the live Sentry project via the Sentry API | 3-5 days | Production incident MTTR; SLO enforcement |
| **12.C** | API maturity wave 3 (infra-credentialed) — publish `runmycampus` to PyPI + `@runmycampus/sdk` to npm from `sdk/`, host the public reference at `docs.runmycampus.com`, dynamic per-tenant CORS allowlist sourced from SiteConfig | 1-2 wk | First external integrators; marketplace foundation |
| **13.C** | AI differentiation wave 3 (multi-quarter) — replace the grade-only at-risk baseline with a real ML model in `apps/analytics/ml_inference.py`, policy + handbook RAG via existing `AIEmbeddingStore` + a policy-PDF ingestion task, Anthropic budget controls + per-tenant spend caps (extend the existing `_check_and_consume_budget` path in `services/ai_gateway.py`), in-product UI hooks for `draft_parent_message` + `draft_report_card_comment` | Multi-quarter | 2026 ed-tech AI parity (Schoology, MagicSchool) |
| **14.C** | Marketplace + partner program wave 3 (infra-credentialed, multi-quarter) — app submission UI + automated security/compliance checks, sandbox tenant infrastructure for developer testing, tenant-facing `/admin/apps/` install/uninstall UI on top of the existing AppInstallation model, Stripe Connect onboarding through PlatformMarketplaceEarning, partners.runmycampus.com + certification + portal | Multi-quarter | "AWS-of-education" positioning |
