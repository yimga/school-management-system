...
**Completion audit:** §2e row 6 (broad except) and §2e row 7 (structured logging) are **complete** for application code; migrations out of scope. Verification: `manage.py check`, `lint_broad_except --strict`, `verify_operating_discipline_docs`, Phase H audit, smoke + targeted tests passed. **Done and dusted:** See [BACKLOG_AND_DEFERRED_CLOSURE.md](BACKLOG_AND_DEFERRED_CLOSURE.md) §2e "done and dusted" table — nothing left for rows 6 or 7.
...
| services.py | DONE | payment.received/invoice.created emit_event: (ImportError, AttributeError, TypeError, ValueError, KeyError) + debug; parent_finance_summary: (NoReverseMatch, ImproperlyConfigured, ObjectDoesNotExist, DatabaseError, AttributeError, TypeError, ValueError) + debug. Allowlist 0. |
...
| **management/commands/generate_models_diagram.py** | — | **DONE** | **DONE** | handle() subprocess graph_models: _GENERATE_MODELS_DIAGRAM_ERRORS (OSError, ValueError, TypeError, subprocess.CalledProcessError, RuntimeError) + log_exception_with_context; FileNotFoundError and TimeoutExpired kept as separate handlers. Allowlist 0. §2e row 6. |
...
| **management/commands/index_ai_knowledge.py** | — | **DONE** | **DONE** | _index_blueprint_packs: _INDEX_AI_KNOWLEDGE_ERRORS (ImportError, AttributeError, TypeError, ValueError, KeyError, DatabaseError, OperationalError, OSError) + log_exception_with_context; _index_policy_bundles, _index_workflow_packs, _index_report_templates already typed. Allowlist 0. §2e row 6. |
| **management/commands/recover_database.py** | — | **DONE** | **DONE** | backup: _RECOVER_DATABASE_BACKUP_ERRORS (OSError, PermissionError, shutil.Error) + log_exception_with_context; unlink: (OSError, PermissionError) + logger.warning. Allowlist 0. §2e row 6. |
| **management/commands/i18n_commands.py** | — | **DONE** | **DONE** | GenerateRegionalReportsCommand handle(): _I18N_GENERATE_REPORT_ERRORS (ValueError, TypeError, KeyError, AttributeError, DatabaseError, OperationalError, ImportError, OSError, FileNotFoundError) + log_exception_with_context then CommandError. Allowlist 0. §2e row 6. |
...
| **management/commands/bootstrap_platform_catalog.py** | — | **DONE** | **DONE** | _run_all_bootstrap step loop: _BOOTSTRAP_PLATFORM_CATALOG_ERRORS (CommandError, DatabaseError, IntegrityError, OperationalError, ProgrammingError, OSError, ImportError, AttributeError, TypeError, ValueError, KeyError) + log_exception_with_context(school_id=None, extra: command, extra_args, error). Allowlist 0. §2e row 6. |
...
| **test_i18n.py** | — | **DONE** | **DONE** | TranslationCommandsTestCase: compile_translations + validate_translations → _TEST_I18N_COMMAND_ERRORS (CommandError, ImproperlyConfigured, ImportError, AttributeError, OSError, ValueError, TypeError, KeyError); allowlist 0. §2e row 6. |
...
- [x] **Tests (lint skips):** tenancy/tests/test_control_plane_boundary.py _file_contains_forbidden_imports → OSError; schools/tests/test_control_plane_boundary.py resolver traversal → (AttributeError, TypeError, IndexError, KeyError). **schools/tests/test_tenant_isolation_and_provisioning.py** response.json() → (ValueError, TypeError, KeyError, json.JSONDecodeError). **schools/tests/test_tenant_isolation_schema_mode.py** tearDown c.delete() → _teardown_delete_errors (DatabaseError, IntegrityError, OperationalError, ProgrammingError). **api/tests/test_offline_sync.py** reverse() → NoReverseMatch. §2e row 6 incremental.
