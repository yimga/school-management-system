...
| **management/commands/generate_models_diagram.py** | — | **DONE** | **DONE** | handle(): graph_models subprocess → _GENERATE_MODELS_DIAGRAM_ERRORS (OSError, ValueError, TypeError, subprocess.CalledProcessError, RuntimeError) + log_exception_with_context(school_id=None, extra: command, output, error); FileNotFoundError and TimeoutExpired kept as separate handlers. Allowlist 0. §2e row 6. |
