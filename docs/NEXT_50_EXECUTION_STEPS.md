...
6. **PARTIAL (BLOCKED)** — Delete migrated legacy paths after each replacement. ensure_gilead_admin removed; customizer→studio_os redirect in place. **Further legacy-path removal BLOCKED on product confirmation** (BACKLOG §2d). Clear note: no additional legacy URL deletion until product confirms.
