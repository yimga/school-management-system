# Shell surface inventory (generated)

**UTC** `2026-05-04T15:41:26.413614+00:00`  
**Status** `PASS`  

| Template | Substrings (required) |
| --- | --- |
| `templates/accounts/backend_dashboard.html` | ok |
| `templates/accounts/school_onboarding_card.html` | ok |
| `templates/admin/base_site.html` | ok |
| `templates/backend_base.html` | ok |
| `templates/base.html` | ok |
| `templates/components/admin_nav_bridge.html` | ok |
| `templates/control_plane_base.html` | ok |
| `templates/control_plane_skeleton.html` | ok |
| `templates/marketplace/tenant_app_catalog.html` | ok |
| `templates/partials/backend_portal_breadcrumb_actions.html` | ok |
| `templates/partials/shell_chrome_backend_finance_pulse_strip.html` | ok |
| `templates/partials/shell_chrome_backend_operational_status_load.html` | ok |
| `templates/partials/shell_chrome_backend_ops_audit_snapshot.html` | ok |
| `templates/partials/shell_chrome_backend_ops_depth_summary.html` | ok |
| `templates/partials/shell_chrome_backend_ops_strip.html` | ok |
| `templates/partials/shell_chrome_backend_planner_recommended_next_strip.html` | ok |
| `templates/partials/shell_chrome_backend_stats_core_strip.html` | ok |
| `templates/partials/shell_chrome_backend_system_indicators.html` | ok |
| `templates/partials/shell_chrome_breadcrumb_row_between_primary_and_actions.html` | ok |
| `templates/partials/shell_chrome_breadcrumb_row_close.html` | ok |
| `templates/partials/shell_chrome_breadcrumb_row_open.html` | ok |
| `templates/partials/shell_chrome_contextual_info_banner.html` | ok |
| `templates/partials/shell_chrome_django_messages.html` | ok |
| `templates/partials/shell_chrome_django_messages_base_bootstrap.html` | ok |
| `templates/partials/shell_chrome_django_messages_control_plane.html` | ok |
| `templates/partials/shell_chrome_django_messages_tenant_portal.html` | ok |
| `templates/partials/shell_chrome_impersonation_session_strip.html` | ok |
| `templates/partials/shell_chrome_marketplace_tenant_ops_strip.html` | ok |
| `templates/partials/shell_chrome_page_heading_actions_strip.html` | ok |
| `templates/partials/shell_chrome_site_preview_banner_top.html` | ok |
| `templates/partials/shell_portal_layout_wrap_open.html` | ok |
| `templates/partials/shell_rmc_registry_html_attrs.html` | ok |
| `templates/partials/shell_skip_link.html` | ok |
| `templates/portal/student_attendance_export.html` | ok |
| `templates/portal_base.html` | ok |
| `templates/schools/super_blueprints_catalog.html` | ok |
| `templates/schools/super_workflow_packs.html` | ok |
| `templates/siteconfig/academic_years_setup_evidence.html` | ok |
| `templates/siteconfig/bulk_letters.html` | ok |
| `templates/siteconfig/config_mutation_audit_evidence.html` | ok |
| `templates/siteconfig/curriculum_templates.html` | ok |
| `templates/siteconfig/departments_setup_evidence.html` | ok |
| `templates/siteconfig/entity_catalog_overview.html` | ok |
| `templates/siteconfig/grading_scale_bands.html` | ok |
| `templates/siteconfig/metadata_dynamic_fields_operator.html` | ok |
| `templates/siteconfig/metadata_operator_hub.html` | ok |
| `templates/siteconfig/partials/configuration_control_center_staging_publish.html` | ok |
| `templates/siteconfig/region_comparison.html` | ok |
| `templates/siteconfig/region_grading_scales_matrix.html` | ok |
| `templates/siteconfig/region_validation_dashboard.html` | ok |
| `templates/siteconfig/scheduled_reports_delivery_hub.html` | ok |
| `templates/siteconfig/school_theme_settings.html` | ok |
| `templates/siteconfig/tenant_runtime_configuration_hub.html` | ok |
| `templates/siteconfig/term_publish_status_evidence.html` | ok |
| `templates/siteconfig/user_preferences.html` | ok |
| `templates/studio_os/partials/shell_main_content.html` | ok |
| `templates/studio_os/shell.html` | ok |
| `templates/studio_os/shell_control_plane.html` | ok |

