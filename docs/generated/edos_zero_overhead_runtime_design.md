...
**Verdict:** `EDOS_RUNTIME_ABSTRACTION_READY`

## Scope

Defines the 15 runtime context primitives that propagate through every kernel operation. Each context is tenant-safe, request-scoped or explicitly passed, testable, serializable where edge/offline needs it, secret-redacted, and propagates safely into async jobs, AI calls, and PWA offline sync queues.
...
### Propagation rules (non-negotiable)
...
- services/ai_helpers.py — AIContext gateway (boundary baseline 0)
