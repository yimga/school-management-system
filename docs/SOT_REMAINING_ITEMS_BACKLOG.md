...
| **KB/FAQ + LibreOffice T4 (WOPI / Collabora)** | Blocked | Owner: Platform Ops. **Not** a closed-source SaaS requirement: default path is **self-hosted OSS** Collabora on your Render/k8s/VM. **Concrete blocker observed:** `collabora.<domain>` DNS routes to the **Django** service (discovery `302` → `school-not-found`); must route that host to a Collabora container/service. After routing fix: `curl -I …/hosting/discovery` → `200`, then smoke + operator/tenant browser sign-off per `docs/execution/COLLABORA_PRODUCTION_ROLLOUT_CHECKLIST.md`. **Product escape hatch:** org policy “no second service” → keep T0–T3 DONE, leave T4 blocked with explicit policy owner until allowed. Tenant data: `tenant_command seed_office_documents --schema=…`. |
