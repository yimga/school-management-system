...
| `/siteconfig/customizer/clear-preview/` | URL | **REDIRECT** | `siteconfig:clear_preview` (`/siteconfig/preview/clear/`) | Legacy customizer preview-clear route; redirects preserving query string; canonical path is `preview/clear/`. |
...
| `/siteconfig/theme-experience/` | URL | **REDIRECT** | `studio_os:experience` (`/studio/experience/`) | Legacy theme editor entry-point; requires `settings.manage` and preserves `next=` when provided. |
