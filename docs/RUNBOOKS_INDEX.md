...
| Topic | Doc | Notes |
|-------|-----|--------|
| **TLS certificate — onboarding decision** | docs/EDGE_TLS_RUNBOOK.md | The school chooses `off` / `selfsigned` / `provided` (any CA) / `acme`, and can change later in either direction. Do this WITH the school before go-live. Without a certificate the origin is not a secure context, so WebCrypto is withheld and offline PIN / local mode cannot be enabled on any browser — "Local access could not be enabled on this browser" is the URL, not the browser. Gilead runs `selfsigned`. Devices install the box CA by browsing to `http://<box>:10000/edge/trust/` (fingerprint + QR + the certificate) rather than being handed a file -- plain HTTP on purpose, since a device reaches it BEFORE it trusts the box. On Windows, macOS and Linux that page hands over ONE command per platform which fetches the certificate, checks its fingerprint against the box's own and installs only on a match (5b-i) -- nothing fetched is ever executed, deliberately, because the page is plain HTTP on a school LAN. **If the school MANAGES its devices, none of that per-device work applies** -- and nobody has to run anything either: `edge-bootstrap.sh` writes the Apple .mobileconfig, the Android Enterprise `caCerts` snippet and the .crt each console wants into `mdm/` beside the repo on every run (runbook 5c; `manage.py edge_tls --export-mdm <dir>` is the by-hand equivalent). On managed Chromebooks and supervised iPads a per-device install does not stick at all, and on Android 11+ a hand-installed authority is ignored by apps. The page ends with a check that runs on load and tells the device whether the install actually took (5d). The bootstrap needs no passphrase set up front any more -- it generates and stores one, then reuses it (runbook 4). |
| Edge onboarding wizard: the TLS + trust step | `manage.py edge_onboarding_verify --slug <slug>` | Step `edge_tls_trust` validates the mode, the certificate and the trust anchor from FILES ONLY, so it answers on a box whose database is still migrating. `off` PASSES -- it is a real choice -- but says out loud that offline PIN cannot be enabled at a plain-http origin. Its self-heal reissues the leaf for the addresses the box now holds and NEVER mints a certificate authority; it also cannot restart the terminator from inside a container, and says so. |
