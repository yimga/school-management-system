...
| B4 | Single currency per tenant | ~~PARTIAL~~ **CLOSED 2026-06-24** | `HoldingCurrencyRollup` + `holding_rollup.materialize_*`; tenant parent dashboard + operator `/super/billing/holding-currency-rollups/`; daily Celery beat `holding-currency-rollup-daily`. |
...
tested, no rebuilds. **§2 competitive backlog is fully CLOSED** (O1–O4, B1–B4, R1–R3, T1–T3 as of 2026-06-24). **Playwright CI proof shipped** (batches **1711**–**1713** role-home + **1712** globe layout). **50-app matrix CI shipped** (batch **1714**). **Marketing gate alignment** (batch **1715**) + **CI harness bundle** (batch **1716**). **Footprint preview + Prometheus stack drift gates** (batches **1717**–**1718**). Next product loops: operator deploy of prometheus-client + compose stack, Zero-Friction phase 6 template burndown, counsel/RLS external gates.
