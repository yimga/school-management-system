...
- **Custom admin site:** `BaseRunMyCampusAdminSite` / `TenantAdminSite` / `PlatformAdminSite` (`config/admin.py`) customize index (dashboard), app list order, and add URLs (dashboard, activity-logs, system-health). App grouping (Accounts, People, Academics, Evals, etc.) and **Configuration Control Center** (**Config center** in admin IA) last is clear.
