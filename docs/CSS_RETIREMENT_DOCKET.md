...
- **Belt-and-suspenders** — `rmc-tenant-shell-metrics.js` `apply()` idempotently re-asserts `portal-body-with-layout` on the tenant body each tick (self-no-ops on the operator control plane, which has no `.tp-header`), restoring the rest of the `body.portal-body-with-layout` layer that wasn't individually re-anchored. Verified: post-load body class = `portal-body-with-layout portal-backend-dark`.
