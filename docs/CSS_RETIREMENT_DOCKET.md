...
**1. Deletions did not propagate — in either direction, ever.** The delta is built by scanning `filter(updated_at__gt=since)`, and a deleted row leaves nothing to scan. So a withdrawn student stayed enrolled on the appliance, a revoked invoice stayed payable offline, and a classroom deleted on the box came back on the next pull because the cloud simply re-offered it. Every other field converged; the absence of a row did not, and nothing reported a problem. New `apps/sync_engine/tombstones.py` + `SyncTombstone` + `sync_services.apply_deletes`: a `post_delete` receiver records the burial, tombstones ride the EXISTING rail as `op="delete"` rows using `deleted_at` as their `updated_at` (so paging, signing, cursors, directives and replay all work unchanged), and delete-dominance is resolved BY TIMESTAMP so both sides reach the same answer regardless of which is asked first. A strictly newer write resurrects the row and drops the tombstone, because a delete must not be a permanent ban on a pk.

**Chosen against the brief's own proposal, deliberately.** The brief said `is_deleted` columns. Building it showed that to be worse here: it needs a migration on fifteen live TENANT tables, every existing `.delete()` call site would have to be rewritten or the column would never be set, and a database CASCADE — the most common way rows actually disappear — fires `post_delete` for every child but would set nobody's `is_deleted`. One receiver captures all of it, cascades included, with no change to a single call site.

**Deletion is the only sync operation that is not self-healing**, so it is the only one with three guards: policy (a money/grade delete travelling UPWARD is refused, and the cloud then RE-ASSERTS its row so the appliance gets it back instead of the two diverging in silence), a per-bundle flood cap (`RMC_SYNC_MAX_DELETES_PER_BUNDLE`, refused WHOLE — half a wipe plus an error is the worst outcome), and a kill switch. Nothing is retroactive: the table starts empty.

**2. A wall-clock cursor loses writes, and nothing notices.** `updated_at__gt` has two holes a real outbox does not: a transaction that COMMITS after a cycle read the high-water stamps a timestamp already behind it and is never re-offered (lost, not delayed), and two rows sharing one clock tick can straddle a page boundary. `models.get_sync_cursor_for_request` now re-asks from slightly behind the cursor. **The bound is stated and asserted by a test** rather than glossed: a transaction open longer than the window still slips through, and only a monotonic sequence column would close it completely — which costs the same fifteen-table migration.

**3. Files never crossed the boundary at all.** `_derive_sync_fields` drops every `FileField` — correctly, since a bundle carries column values and a synced path would dangle — so student photos, scanned report cards and payment proofs simply did not exist across the boundary while every status read green. New `file_manifest.py` / `file_sync.py` / `api/sync_files_api.py` / `SyncFileTransfer` / `manage.py edge_sync_files`: chunked, resumable from a DURABLE offset (a transfer that restarts from zero on every drop never finishes on an intermittent link), sha256-verified BEFORE anything is committed to storage, and budget-bounded. Kept off the row rail entirely — its own command, so a 50 MB scan can never delay or fail a data cycle. The chunk endpoints take a path, which is a traversal hole unless something constrains it; the constraint is membership of the school's OWN file set, so it cannot be argued into `../../etc/passwd` and cannot reach another tenant's media.

**4. No schema handshake between the two deployments.** The local half existed ("this box is N migrations behind"); the cross-deployment half did not, so a stale box received rows for columns it lacks and the operator saw "12 NOT applied" with no cause. Migration heads are now exchanged per APP in `X-RMC-Sync-Schema-Head`, and the cloud **withholds only the entities owned by an app the box is behind on** — a box stale only on `finance` keeps receiving its attendance. Refusing the whole cycle would take a school offline for a migration it may not need.

**5. The conflict screen was a BYPASS of the authority model it exists to serve.** Money and grades are cloud-authoritative and a set of per-field columns (salary, payroll/leave authorization, offboarding, the grading coefficient) may never travel upward — and the Sync Center offered "Keep client" to anyone who could reach the page, which wrote the value the rail had just refused. Two smaller holes underneath: the apply ignored `_DOWN_ONLY_FIELDS_PER_ENTITY` entirely (so resolving a benign teacher conflict in the client's favour wrote the box's SALARY), and it caught only `DoesNotExist`, so a payload pointing at a since-deleted parent raised out of the view — on PostgreSQL not even there, but at COMMIT. Now `may_resolve` requires the same authority a direct write would (superuser or the model's own `change_` permission), `_client_updates_for` strips cloud-governed columns, an ONLINE_REQUIRED domain can never be settled from the offline copy at all, and the review screen shows an aligned field-by-field comparison with a recorded reason instead of two raw dict dumps. A button that would be refused is no longer offered — the same lesson as the admin index.

**6. Cloud→box latency was poll-bound.** The appliance is behind NAT so the cloud cannot push, and the adaptive cadence deliberately backs off on a quiet school. New long-poll changes feed (`api/sync_changes_api.py` + `change_beacon` + `manage.py edge_sync_watch`) collapses it to about a second over plain HTTP. It carries NO row data — it only says "there is something for you" — so every cursor, policy and replay guarantee is untouched, and the cadence remains a complete fallback.

**7. A signature proves who BUILT a bundle, never that it is new.** A captured bundle replayed later verifies perfectly, and now that deletions propagate that is not harmless: a bundle captured before a row was deleted RESURRECTS it. New `replay_guard.py` + `SyncBundleReceipt`: a random nonce inside the SIGNED header, refused on a second sighting, with the retention window as the honest limit of the guarantee.

**8. Convergence is now measured, not asserted.** `convergence_harness.py` + `manage.py verify_edge_sync_convergence` (always rolled back — safe against a live box) run the sequences: clean sync, a 14-day outage with writes on both sides, a mid-bundle drop, a power cut between apply and cursor advance, a ten-minute clock skew, a duplicate bundle, delete propagation, and the authority invariants across the whole registry. **Its limit is stated rather than papered over:** it proves the PROTOCOL converges against one database and a modelled peer; it cannot prove two independent Postgres databases agreeing, and on the one property where they differ (deferred FKs) SQLite is the WEAKER environment — which is exactly how the 2026-08-19 wedge stayed invisible.

### Four defects found while building, none of them on the list

**9. A classroom could not be CREATED across the rail in either direction.** `classroom` is one of the three original entities and keeps a curated field set of `{name, academic_year_id}` — but `Classroom.department` is NOT NULL and `code` is a required UNIQUE column. A class created on the cloud in September hit the box's create path and died on `NOT NULL constraint failed: academics_classroom.department_id`, reported as a per-row 422 and otherwise invisible; a class created offline could never be pushed up for the same reason. Adding `department_id` alone would have moved the failure rather than fixed it — the second create would then collide on `code=""`. Only UPDATES ever worked, which is exactly why it went unnoticed: the pk-preserving clone already had every classroom that existed at clone time, and edits to those converged perfectly. The exam-governance booleans (`gce_eligible`, `allows_third_term`) deliberately stay OFF the rail.

**10. A retried pull manufactured conflicts out of the engine's own writes.** After applying a pulled row, this side's `updated_at` is newer than the cloud's. Re-offering that row — after any failed cycle, and routinely once the overlap existed — graded the apply as a local edit and raised a `SyncConflict`, asking an operator to adjudicate between a value and itself. The apply ledger already knew the row had not been touched locally; it is now consulted. A REAL local edit after a pull is still a conflict, and there is a test for exactly that so the fix cannot swallow genuine divergence.

**11. An identical re-apply still bumped `updated_at`,** re-entering the row into the delta in the other direction — churn the engine manufactured itself. An apply whose every value already matches is now a no-op with an `unchanged` result.

**12. `Invoice.delete()` is a SOFT delete** for legal traceability. `apply_deletes` calls the INSTANCE's `delete()`, never a queryset delete, so the sync rail cannot overrule a model's own deletion semantics — and it then CLEARS the tombstone, because a row that still exists must not be treated as buried (leaving it would refuse every later update to that invoice).

### And the one that has nothing to do with sync

**13. Six live API list endpoints return HTTP 500 on every GET, and the test written to prevent it has never run.** Found because two modules failed to COLLECT during the regression sweep. `REST_FRAMEWORK["DEFAULT_PAGINATION_CLASS"]` is `CursorPagination`, whose own default `ordering` is `"-created"`. `TeacherProfileViewSet`, `StudentGuardianViewSet`, `ClassroomViewSet`, `MobileDeviceViewSet`, `PushNotificationViewSet` and `OfflineSyncViewSet` declared no `pagination_class`, and **none of those six models has a `created` field** — so every list call raises `FieldError: Cannot resolve keyword 'created' into field`. Proven by running `order_by("-created")` against all six models, not by reading.

The part that matters as much as the bug: `test_entity_api_pagination_loop.py` and `test_mobile_api_pagination_loop.py` were written on **2026-06-27** for precisely this, and both reference pagination classes that were never created. So each module raised `AttributeError` at IMPORT, pytest reported a *collection error* rather than a failure, and the guard was written, never ran, and the defect it guards against shipped anyway. Both modules now collect and pass; the six pagination classes exist with the orderings those tests assert.

**14. Nothing checked that JavaScript parses.** The 2026-08-19 Python parse gate's own write-up named this hole and stopped. The two languages fail differently, and the JavaScript one is worse: a Python module that does not compile fails loudly at import, while a `.js` file that does not parse fails **silently in the user's browser** — the tag 200s, the engine throws into a console nobody is watching, and the page renders looking normal with one feature dead. No server log, no Sentry event, no test. `static/js/service-worker.js` is the worst case: a SyntaxError there kills registration, so the offline shell a sovereign appliance depends on stops updating and nothing says so. New `scripts/verify_javascript_files_parse.py` — 430 files in ~0.7s using **node's own parser** (so it cannot disagree with the engine that will run the file), both module systems tried, minified/vendor drops excluded, no baseline and no allow-marker. Registered in `REQUIRED_GATES`, its own CI job, and in `pre_push_boundary_check.py` — which gained a real **SKIP** state for it, because a gate whose toolchain is absent has not passed and must not be reported as though it had.
...
**8. The go-live button could 500, and that is unacceptable for the one control an activating school cannot route around.** `execute_launch_view` guarded only `(AttributeError, DatabaseError, TypeError, ValueError)` — while `GUIDED_ONBOARDING_SOFT_FAILURES`, defined ten lines below it in the same module, adds `KeyError`. So the codebase already knew a `KeyError` reaches these paths; the launch view simply never got it. Anything outside that tuple (a `KeyError`, a `ValidationError`, an `ImportError` from an optional integration) became a bare 500 with no message, no log reference and nowhere to go. Now: the named soft failures keep their "please try again" message, and everything else is logged at ERROR with the full traceback and surfaced with an 8-character reference the operator can quote, always returning to a page they can act on. **`AssertionError` is re-raised ahead of the catch-all** — `DatabaseOperationForbidden` subclasses it precisely so fail-soft guards cannot swallow a forbidden write, and a catch-all written without that guard would have silently re-opened the hole. There is a test asserting exactly that, because it is the kind of regression nothing else would catch.

**9. The admin landing page invited people to be refused.** `templates/admin/index.html` rendered nine hardcoded links — a "Users" button to `admin:accounts_user_changelist` and eight "Open" buttons to `admin:app_list` — with **zero** permission checks anywhere in the file, so each was a guaranteed 403 for a viewer lacking the matching permission. All nine now gate on Django's own answer: `perms.<app>` is true exactly when `admin:app_list` will admit the viewer, and `perms.accounts.view_user` is exactly what the user changelist enforces, so the page cannot drift from what the admin actually allows. **What the audit found along the way, and which changes the claim:** on a TENANT host the Django admin is operator-only — a school-role staff user is refused at `/admin/` itself even holding real model permissions, which is the deliberate isolation posture (`scan_staff_gate_on_tenant_surface`, `TenantSuperAdminRequiredMiddleware`). So the reported 403 is the system working; the ungated buttons matter on the OPERATOR host, where non-superuser operators with partial permissions are exactly who the page serves. Sealed with a contract test over the template source (no `{% url 'admin:… %}` outside a `perms.` guard, plus a check that the guards name each link's own app so a blanket `{% if perms %}` cannot satisfy it) rather than an end-to-end tenant render that cannot honestly be produced.

**10. `apps/accounts/tasks.py` did not compile on `main`, and nothing in CI noticed.** Found while running the accounts suite for regressions: five unrelated test modules failed to *collect*. The cause was a `SyntaxError` — the file was **truncated mid-statement** at line 705, `return {` left unclosed. Commit `0711ac109` made a legitimate change earlier in the file (routing year-lock through `lock_source_year`) and, in the same diff, deleted the last three lines. A module that does not compile cannot be imported at all: Celery autodiscovers `<app>.tasks` for every installed app, so this breaks worker startup, and any `from apps.accounts.tasks import …` is an ImportError 500. Restored the three deleted lines verbatim from `0711ac109~1`; the module imports and exposes its 12 tasks again.

**Why no gate caught it, and the new one that will.** `scan_import_reference_integrity` AST-parses a target module to resolve symbols but treats a parse failure as *opaque* and skips it — it is deliberately biased toward false negatives, so a broken file is silently excused rather than reported. The runtime verifiers (`verify_get_model_integrity`, `verify_url_name_integrity`, …) only ever reach modules Django already imported successfully. And the suite only imports what a selected test happens to touch. Each of those is reasonable alone; together they left the most basic invariant — *the source compiles* — unenforced. New `scripts/verify_python_files_parse.py` walks `apps/ services/ config/ scripts/` (8,411 files, ~1s, stdlib only, no baseline and no allow-marker, because a file that does not parse is never intentional), wired into `pre_push_boundary_check.py` **first** (every gate after it is otherwise answering about a tree that does not run), into `architectural-boundaries.yml` as its own job, and into `verify_ci_gate_wiring.py::REQUIRED_GATES` so it cannot be silently un-wired — 47 required gates, 0 un-wired. Proven by mutation: re-deleting the exact three lines makes the gate report `apps/accounts/tasks.py:705: '{' was never closed` and exit 1. 8 stdlib tests.

**Reported but NOT reproduced, stated plainly:** the 500 on `gilead-tech.runmycampus.com/siteconfig/guided-onboarding/execute-launch/`. That tenant's data is not in this dev database and the route answers 302 here; item 8 removes the *class* of failure (the button can no longer 500 whatever `execute_launch` raises) without having reproduced that specific instance. SW `sms-v4.06.68-sync-schema-drift-guard-2026-08-19`.
...
**1. THE ROOT CAUSE — cloud→box had no create path at all.** `client_offline_id` marks a row created OFFLINE ON A BOX, so a row authored on the CLOUD carries an empty one and `apply_pulled_bundle` routes it to `apply_changes` (update-by-pk). That path answered `404 Not found` for a pk the box had never seen and moved on. **Every record created on the cloud after a box was cloned was therefore permanently invisible to that box** — departments, subjects, specialties, terms, classrooms — with no error anywhere. The box did not report a problem; it quietly diverged, and then failed outright on the first child row referencing a parent it had never been given. Proven against the dev DB before fixing: a `department` pk the box lacks → `404`, row not created. Fixed by `_create_from_cloud_pull` — creating BY PK is correct in this direction and only this direction (the clone is pk-preserving, the cloud is authoritative on a pull); a box push keeps the 404, because a box minting pks on the operator is a pk collision and an authority inversion. `_INSERT_HELD_ENTITIES` (`teacher`) is refused with its reason in both directions.

**2. THE AMPLIFIER — the per-row savepoint could not do its job on PostgreSQL.** `_apply_changes_inner` wraps each row in a savepoint and catches `IntegrityError` *specifically* so that "a FK to a deleted parent" degrades that ONE row; its comment even notes "SQLite doesn't enforce these, only prod Postgres does". Half right. Django emits every foreign key on PostgreSQL as **`DEFERRABLE INITIALLY DEFERRED`** (`DatabaseOperations.deferrable_sql()`), so the violation is not raised by `save()` — it is raised by the **COMMIT of the outermost transaction**, long after the savepoint has been released. The error escaped `apply_changes` entirely, rolled back every good row alongside the bad one, and surfaced as a cycle-level `pull failed`. SQLite checks immediately, **which is exactly why the whole suite was green while production was wedged.** Proven side-by-side in one file (both constraint modes, same savepoint code): IMMEDIATE degrades the bad row and the good row lands; DEFERRED releases cleanly and then loses the entire batch at COMMIT. Fixed two ways — `_unresolvable_fk` preflights every FK **before** the write (backend-independent, reports a precise `409 missing_reference` naming the field, the value and the target table, memoised per bundle so hundreds of children cost one query per distinct parent), and `_force_immediate_constraints()` issues `SET CONSTRAINTS ALL IMMEDIATE` on Postgres so the existing savepoints finally behave as documented.

**3. THE RATCHET — and why refusing the row cleanly is still not convergence.** A failed apply leaves the pull cursor where it was, so the next cycle re-downloaded the identical bundle with the identical poison row, forever. Preflighting alone stops the wedge but not the divergence: a `missing_reference` means the parent's own `updated_at` is older than the cursor, so the incremental delta will never offer it again and the child would be refused on every future cycle. `_request_replay_for_missing_parents` rewinds the PULL cursor so the next cycle replays the corpus — which DOES contain the parent, and the new create path lands it. **Bounded** (one replay per 6h per school, via an atomic `cache.add` cooldown) because a replay is expensive and a reference a replay cannot satisfy would otherwise rewind on every cycle.

**Bundle order made it likelier still.** Rows ship sorted by `updated_at` because that is what makes a page boundary a safe cursor; `updated_at` order says nothing about dependency, so a specialty could be applied ahead of the department sitting later in the same bundle. The apply now runs in `_insert_dependency_order` (referents before dependents, stable within a rank) — free, because the pull cursor comes from the bundle's high-water header, never from apply order — and results are still emitted in the caller's original order.

**Visibility: a partial pull must not render as a clean one.** `pulled` counts rows RECEIVED, so a bundle in which every row was refused still reported "pulled 12" and a green cycle. `apply_pulled_bundle` now returns `skipped` + `skipped_reasons`, the runner names them in the run message (`3 NOT applied (missing_reference x3)`), `EdgeSyncRun.skipped` persists the count (migration `sync_engine.0005`, additive + defaulted), and the live panel gains a **Not applied (24h)** tile, a per-cycle column, and an explanation that fires *before* the healthy states — a cycle can be green, connected and idle while still having refused rows. A conflict is deliberately NOT counted as skipped: it already has its own surface and its own operator workflow.

**Isolation preserved.** `sync_origin=None` (the ordinary online DeltaSyncAPI) still 404s, so no non-edge tenant on a shared deployment sees new behaviour. SW `sms-v4.06.67-sync-referential-integrity-2026-08-19`.

## 2026-08-17 — Edge sync: the backlog deadlock, the cloud's impossible button, and Wave 5 (TeacherProfile)

**Reported symptom:** "from the cloud I have tried to sync with the box and it fails consistently", plus a Sync Center screenshot showing *Last sync failed* / *Sync is not enabled on this deployment* / pushed 0, pulled 0. Both causes were found by RUNNING the cycle, not by reading it.

**1. The push deadlock (the reason a stale box stayed stale).** `sync_runner.run_sync_cycle` — the function the "Sync now" button AND every automatic trigger call — built ONE bundle with `since=None` (every row the school has) and POSTed it whole. `SyncBundleUploadView` caps a bundle at `RMC_SYNC_BUNDLE_MAX_ROWS` (default 500) and rejects an oversized one with `400 bundle_too_large`, applying **nothing**. So the further behind a box was, the more certain every attempt was to fail — identically, forever. Proven by test before fixing: 7 rows against a cap of 3 produced `rejected == [7]`, zero rows applied. The push now **pages** at the receiver's cap, and on a `400` believes the operator's reported `max_rows` over its own setting.

**2. No cursor.** `since=None` in both directions and the `X-RMC-Sync-High-Water` header the download endpoint already stamps was discarded into `_high_water`, so every 180s tick re-shipped the whole corpus. (The `post_edge_outbox`/`pull_edge_inbox` *commands* did keep file cursors — only the runner did not, while `edge_scheduler`'s docstring claimed otherwise.) New `EdgeSyncCursor` model (migration `sync_engine.0003`) holds a durable per-direction high-water that advances **only over confirmed work**, so a rejected page or the per-cycle page ceiling (`RMC_EDGE_SYNC_MAX_PAGES_PER_CYCLE`, default 20) leaves the remainder for the next cycle — incremental, durable convergence.

**Paging had to respect insert integrity.** `apply_edge_inserts` remaps a new-references-new FK from an IN-BUNDLE map and **drops** the FK when the referent is not in the same bundle, so naive chunking would silently unlink offline-created children from their parents. Offline-created rows therefore travel **first and, wherever they fit, whole**; sending them first is also what makes an update page's high-water a valid global cursor. An unavoidable insert split is reported as a `WARNING` in the run message rather than being silent.

**3. The cloud's impossible button.** The Sync Center rendered "Sync now" / "Dry-run sync" on **every** deployment, but a box behind NAT cannot be reached from the cloud — so on the cloud every click failed and wrote a red `EdgeSyncRun` row. That was the screenshot. An existing test asserted this behaviour and thereby locked the defect in; it has been inverted. The cloud now gets the control it can honour: **Queue full resync** (`EdgeSyncDirective`, migration `sync_engine.0004`), a one-shot directive that rides back to the box on `X-RMC-Sync-Directive` during the box's **own** next download, whereupon the box rewinds both cursors and replays the corpus. Idempotent while pending; observable as *queued / collected at &lt;time&gt;* so "the box is offline" is distinguishable from "the box has it". Box-side twin: `python manage.py edge_sync_resync [--run]`.

**4. Per-field direction policy leaked on the INSERT path (security-relevant).** `_DOWN_ONLY_FIELDS_PER_ENTITY` was enforced in `_apply_changes_inner` and **nowhere else** — `apply_edge_inserts` filtered by the allowed set and settable names only, so the whole policy was bypassable by presenting an edit as a new row. Closed on both paths with one reported contract (`rejected_down_only_fields`), proven against the pre-existing `subject_assignment.coefficient` seed so the seal does not depend on the new entity.

**5. Wave 5 — `people.TeacherProfile` rides, with mixed direction.** The last deferred entity. Roster fields (`staff_id`/`phone`/`position_title`/`department`/`reports_to`/preferences) converge two-way; **12 fields ride down-only** — compensation, plus the `allow_finance_panel`/`allow_paystub_access`/`allow_leave_approvals` flags (NOT preferences: `allow_finance_panel` gates the teacher payroll block — `PayrollEmployee`, payslips, net pay — so an upward push could grant payroll visibility on the cloud), plus `is_active`/`merged_into_id` (offboarding and duplicate-merge are governance). **Identity:** `user` is a non-nullable OneToOne to the SHARED `accounts.User`, so `user_id` is already dropped as non-portable (verified by running `_derive_sync_fields`); UPDATE is safe, and an offline-CREATED teacher is **refused with its reason** (`_INSERT_HELD_ENTITIES`, 409) because landing one would require the rail to mint a login — an authentication decision, not a data merge. Doc: `docs/EDGE_SYNC_IDENTITY_HOLD.md`.

**6. The production-migration caution, answered.** New read-only `python manage.py check_edge_sync_deploy_readiness` (safe on production) verifies the two things a green suite cannot: that the four `client_offline_id` columns **and their partial-unique constraints** exist in **every tenant schema** (`evals`/`finance` are TENANT_APPS, so `migrate_schemas` applies per schema and can succeed for some tenants and not others; a column without its constraint silently accepts duplicate offline ids and splits one record into several), and that `accounts_accessrole` is populated. The AccessRole scare was **diagnosed, not guessed**: migration `accounts.0029` is unconditional and idempotent, and the ordinary dev DB holds all 25 rows — the empty table is a *persisted test database* artifact (0029 recorded applied 2026-08-17 15:21, table at 0 rows), exactly the "applied but empty" state 0029's own docstring warns about and, because it is recorded, one a plain `migrate` will not repair. CI seal: `test_edge_sync_deploy_readiness_2026_08_17.py` asserts the DDL against the migrated DB.

**Registry after this wave:** 15 edge entities, all with an anchor + `updated_at`; `evaluation`/`invoice` protected (down-only), `teacher` LWW-with-12-down-only-fields + insert held. Money and marks policy unchanged. Ops guide: `docs/EDGE_SYNC_OPERATIONS.md`. No CSS/JS shipped, so the service worker is not bumped.
...
**Strategic trace (PROMPT S §S7):** lifts the config-SOT floor (~6.5 — “resolver has 1 caller vs ~200 ad-hoc reads”). Every raw config read in `apps/` + `services/` is now ADJUDICATED: key-shaped reads go through the canonical keyed facade (`apps.platform_runtime.config_resolver.get_effective_config` — same merged resolver, same request/TTL cache, so N keyed reads cost N getattrs on one cached object), and namespace-shaped reads carry a reviewed, specific `# config-resolver-allow:` marker. The one-way ratchet floor drops **182 → 0**: any NEW raw read fails CI.

**What landed (119 paths, no migration, no SW bump):** ~75 call-sites converted to keyed reads with exact default preservation (`getattr(x, k, d)` → `default=d`; None-guards folded only where behavior-identical — each conversion parity-checked against the facade contract: `default` returned only on missing key or None resolver), 3 dead reads deleted (`_site =` assigned-never-read), and ~105 marker sites in five honest classes: (1) write-path singletons (`create=True` provisioning / rows mutated+saved — admin forms, theme apply, seeds), (2) namespace-as-form-instance surfaces (ThemeColorsForm/CockpitPayloadForm edit the settings object itself), (3) namespace-as-template-context (the `SITE` context processor and per-view `site` context objects), (4) method-bearing domain reads (`get_finance_runtime_config()`, `get_feature_control_settings()`, `get_offline_runtime_settings()` etc. — methods, not keys), (5) namespace passed whole to helpers (`_bool_default_on`, `resolve_brand_profile`, `build_portal_sidebar_items`, visibility/policy helpers). Baseline JSON regenerated to `finding_count: 0`; CLAUDE.md gate row updated. Two files carried unrelated peer WIP and were converted on ORIGIN copies (`apps/finance/views_invoicing.py`, `apps/siteconfig/context_processors.py` — pushed blobs contain origin + only these conversions). Origin-red repair forward: 2 unmarked username-cap literals in `apps/migration_cloud/landers/structure_lander.py` (peer push skipped the magic-number baseline) now carry markers.

**Evidence:** scanner = 0 unmarked on the pushed state (worktree scan 4 = the two quarantined files' raw sites, whose pushed blobs scan 0 — verified with the scanner's own AST logic); 147-file `py_compile` sweep clean; `manage.py check` clean; 128-test battery covering every test that patches converted symbols (portal gates, finance receipt upload, MFA enforcement modes/redirect safety, context-processor helpers, portal-roles helpers, user-preferences API, report-card preview, template gallery, tenant identity, policies package, scanner unittests) — 126/128 with the 2 failures PRISTINE-PROVEN pre-existing (identical battery on the pre-wave tree fails the same 2): `test_sidebar_hides_workflow_when_portal_disabled` (fails standalone) and `test_no_profile_returns_empty_enrichment_with_unread_metric` (cache-order-dependent SimpleTestCase). Test patch-target fix included: `test_student_portal_gate` now patches `get_effective_config` (the old raw-symbol patch target was removed by the conversion). All other ratchet gates green (tenant queryset/marker-quality, access-resolver 0, granular-rbac 0, role-strings, magic-numbers after the forward repair, print/bare-except, staff-gate).

**Deploy:** no migration, no behavior change by construction (same resolver, same precedence, same caching; defaults preserved site-by-site). Rollback: revert the commit (no state). Registered follow-ups: migrate the `_bool_default_on(site, key)` helper family to request-based keyed reads to retire ~10 engine-view markers; `student_transfer_export`-style namespace pass-through helpers could take keyed params similarly; the 2 pre-existing portal test reds (sidebar gate, cache-order SimpleTestCase) are origin reds needing their own fix.

## 2026-07-09 — Data-sovereignty routing: unresolvable-region early-returns closed, fail-open → fail-closed (9.8-regime A-wave)

**Strategic trace (PROMPT S §S7):** closes the sovereign cluster's registered open design item (~7.5 row: "router/middleware unresolvable-region early-returns"). The border-lock HARD GATE (`enforce_region_match`) was already fail-closed (2026-07-03 closeout), but the three routing layers that CALL it still early-returned when no region could be resolved — so under `DATA_RESIDENCY_ENFORCE` a tenant with a foreign residency promise was silently served from the default store, contradicting the layer's own "fails the op closed" docstrings (≤9.7 contradiction + enforce-mode fail-open).

**The three closed fail-opens:** (1) `TenantDatabaseRouter._enforce_residency_for_alias` returned WITHOUT any check when no alias resolved, and swallowed plumbing failures (`OPTIONAL_DB_ROUTER_ERRORS`) even under enforcement; (2) `RegionalDatabaseMiddleware` silently default-routed a school whose region store was unprovisioned; (3) `DataResidencyMiddleware` swallowed a crashed alignment check in strict mode, and its `assert_aligned_or_log` treated a blank operational binding as aligned AND read only the raw Django setting (an ops env flip enforced the border lock but left it soft-logging).

**Design — declare the default store's region, then every path is comparable:** new `DATA_RESIDENCY_DEFAULT_STORE_REGION` (env-or-setting, default `"global"` — the readiness preflight already codifies "global is served by the default DB by definition", so single-region deployments do NOT brick) + `region_for_alias()` mapping the two alias-naming conventions (`replica_<region>` Wave-K4 env registration → its region — previously a tenant's OWN replica would have spuriously blocked; region-named `regional_cluster` aliases pass through; blank/`default` → the declared default-store region). The unresolved-alias landing is now adjudicated as a KNOWN comparison (e.g. `eu_central` vs `global`) — in-region/global tenants keep working with zero replicas, foreign-promise tenants fail closed, and the `DATA_RESIDENCY_STRICT_UNKNOWN=0` opt-out keeps its documented meaning (unknown regions) without reopening this hole. Plumbing failures under enforcement now DENY (`PermissionDenied("Data residency decision unavailable")`, mirroring `pdp_enforce`), via an import-free flag read so the fail-closed arm cannot depend on the module whose failure it handles. A thread-local reentrancy guard stops the audit-write recursion (the check's own region-resolution reads + violation AuditLog write re-enter `db_for_read`/`db_for_write`; without the guard that recursion was unbounded — latent stack-exhaustion bug, found by design review this wave).

**What landed (8 files, no migration, no SW bump):**

| Change | Files |
|---|---|
| `default_store_region()` + `region_for_alias()`; `assert_aligned_or_log` blank-binding arm delegates to the hard gate (typed, audited `ResidencyViolation`) + mismatch arm env-aware via `residency_enforced` | `apps/schools/data_residency.py` |
| Router: enforcement-flag fast path, unresolved alias adjudicated vs declared default-store region, plumbing failure fails CLOSED under enforcement, thread-local reentrancy guard | `apps/siteconfig/db_router.py` |
| Request-time twin: unresolved region store refused up front (`_enforce_unresolved_region`); inbound pre-pinned alias check now region-mapped (`region_for_alias`) | `apps/platform_runtime/middleware_regional_db.py` |
| Alignment middleware: crashed check fails CLOSED under enforcement; `ResidencyViolation` re-raised (403) alongside `CrossRegionWriteError` | `apps/schools/middleware_residency.py` |
| `DATA_RESIDENCY_DEFAULT_STORE_REGION` setting (env passthrough, default `"global"`) + registry row | `config/settings.py`, `config/settings_registry.py` |
| Failure-injection battery: foreign-promise-on-default-store blocked (router + both middlewares), global/declared-region tenants pass (no-brick proof), own-replica no spurious block, plumbing-crash denies vs flag-off silent, reentrancy guard adjudicates exactly once, env-flag-alone binds, strict-unknown opt-out does NOT reopen the known-comparison block, flag-off unchanged everywhere | `apps/compliance/tests/test_residency_unresolvable_region.py` (21 tests) |
| Border-lock module docstring: registered residual → CLOSED with pointer | `apps/compliance/tests/test_data_residency_border_lock.py` |

**Evidence:** 85/85 residency-adjacent battery (21 new + 24 border-lock + cross-border + data-residency + alignment-middleware + readiness + dynamic-routing + reports export gate) — first run — re-proven on a FRESH keepdb alongside the policies package and the tenant-identity Client suite (full middleware stack, so the changed middlewares run on every request); `manage.py check` + `makemigrations --check` clean; ratchet gates green on the pushed state. HONEST NOTE: a whole-package `apps.schools`/`compliance`/`platform_runtime` sweep was attempted and KILLED after hanging (network/lane-inappropriate tests; its failures were never enumerable and are NOT adjudicated here — not claimed pre-existing, not claimed mine). The wave's flag-off delta is zero by construction (enforcement-off short-circuits precede every new arm), which the targeted battery confirms. Flag-off (deployed default) behaviour is unchanged — this wave hardens the ENFORCE contract, it does not flip it.

**Deploy:** no migration. `DATA_RESIDENCY_ENFORCE` stays default-off; when ops flips it, the preflight (`verify_residency_readiness`) already refuses exactly the states this wave now blocks at runtime. Rollback of the strictness (not the wave): `DATA_RESIDENCY_STRICT_UNKNOWN=0` restores unknown-region passes; `DATA_RESIDENCY_DEFAULT_STORE_REGION=<region>` declares the primary's true region so its in-region tenants pass. Remaining honest scope (register): physical per-region replicas remain an ops/deploy item (application-layer control fails closed until they exist); the 7-day offline endurance CLIENT legs remain EXTERNAL_PROOF_REQUIRED (real-device/multi-browser); `DATABASE_READ_REPLICA_ALIAS` read-split stays residency-unchecked by documented design (operational split, not a regional binding).
...
**B-measure:** 24-gate battery (22 green; 2 compare-reds adjudicated REAL on origin) + independent validators over the previously-lowest domains. **A-wave 1 closed every CONFIRMED red:**

- **Residency border-lock fail-closed by default** - under `DATA_RESIDENCY_ENFORCE=1` an unknown/blank source or TARGET region now BLOCKS: unset `DATA_RESIDENCY_STRICT_UNKNOWN` FOLLOWS enforcement (tri-state: `0` explicit opt-out, `1` forces strict in audit-only posture). The UI predicate `cross_border_export_blocked` now mirrors the enforcer (`effective_region` + strict-unknown) so the export button and the fail-closed gate can never disagree. `.env.example` + `settings_registry` document the contract; border-lock tests updated + extended (default-block, explicit opt-out, predicate parity - 24/24).
- **Access-resolver ratchet meta-protected** - `scan_access_resolver_fragmentation` ran in CI (architectural-boundaries.yml:372) but was absent from `verify_ci_gate_wiring.REQUIRED_GATES`, so nothing would notice it being silently dropped. Added (20 required gates).
- **Owner console Overview owners tile** now filters `suspended_at__isnull=True` - the tile matches the `is_active_owner` gate instead of flattering a suspended owner.
- **Two CI compare-gates were red on main** from earlier waves, closed with honest markers: the role-keyed display-label map in `owner_first_login.py` (`role-string-allow`), incident-note/stack caps `4000` and RFC-5321 email `254` (`magic-number-allow`).

**Validator honesty note:** the "ratchet wired in ZERO workflows" finding was the off-tree-push stale LOCAL tree - origin has the job; the REAL finding was the missing meta-protection. Adjudicate workflow claims against origin, always.

**Register after this wave (open, honest):** router/middleware residency early-returns on unresolvable region (multi-region design work, not a quick fix); config-resolver adoption (1 caller vs ~200 ad-hoc `get_effective_site_settings` reads - ratchet wave queued); PDP enforcement promotion (advisory on 3 surfaces, enforcing on 0); 7-day offline endurance client legs (real browser = EXTERNAL_PROOF_REQUIRED); merge/split completeness lift (grades term/subject FK resolution + guardian re-link provisioning) = next program wave.
...
**Context (design D6; Waves A/B shipped the transfer half):** duplicate person rows now have a first-class, quarantine-honest consolidation path. `RecordManagementService.merge_records` — a comment-only no-op since its creation ("Consolidate data… Delete secondary" over a `return`) — is real, and the ai_dedup candidates that migration runs have been writing into bundle `mapping_summary["dedup_candidates"]` (visible nowhere until today) are now an operator inbox with an action path.

**What landed**

| Piece | Files | What it does |
|---|---|---|
| Operation of record | `apps/people/models_merge.py`, migration `people.0064` | `RecordMergeOperation`: UUID pk, school FK, kind (student/teacher/guardian), guarded FSM (draft→previewed→approved→applying→applied/failed, journaled `advance()` like TransferCase), `preview` JSON (per-relation re-point counts), `collisions` JSON (the quarantine ledger), CheckConstraint primary≠secondary. `StudentProfile.merged_into` self-FK = the D6 tombstone. |
| Merge engine | `apps/people/merge_service.py` | **Self-maintaining inbound-FK walker** (D6's explicit rejection of a curated FK list): walks `apps.get_models(include_auto_created=True)` for every concrete FK/OneToOne targeting the person model — auto-created m2m through tables included — so a new FK is covered the day it lands. `preview_merge` counts the footprint per relation; `apply_merge` re-points under a `cache.add` single-flight lock with a bulk-UPDATE fast path per relation and a per-row savepoint fallback: ANY per-row failure (unique-constraint IntegrityError, `full_clean` ValidationError from models that validate on save) **quarantines** — the row stays attached to the retired secondary, listed on the operation with model/pk/error — never deleted, never silently skipped. Secondary soft-retires (`is_active=False` + `merged_into`); lineage stamped on the survivor (`computation="people.merge"`, wave-c) + best-effort compliance audit. Guards: approved-only, same-school-only (cross-school moves are transfers), already-merged secondary refused, primary≠secondary. |
| `merge_records` made real | `apps/people/people_management.py` | The stub now delegates: create op → preview → approve → apply; returns `{merged_into, retired, operation_id, repointed, quarantined}`. Nothing is deleted (the old stub's comment promised deletion it never did — now the behavior is real AND non-destructive by design). |
| Operator console + dedup inbox | `apps/portal/views_merge.py`, `apps/portal/urls.py`, `templates/super/merges/index.html` | `/portal/super/merges/` (transfers-console pattern: staff-only, `?format=json`, csrf mutations, 500-row cap, best-effort AuditLog): create-and-preview → approve → apply, with per-op status/rows/quarantine/last-note columns; the **dedup inbox** lists ai_dedup candidates from recent bundles (deterministic + AI scores) so an operator can open a merge from a surfaced pair — closing the "candidates can be seen, never resolved" gap from the design's B-measure. |

**Explicitly out of scope, stated:** GenericForeignKey references (audit/history rows naming the secondary by string id remain truthful history — not re-pointed); scalar string references (`TransferCase.source_profile_pk` is a Char field by design — case history preserved); teacher/guardian kinds are wired through the same walker but Wave C's e2e proof covers the student kind (the ~64-FK blast-radius model); collision RESOLUTION (operator re-keys the colliding row, then re-merges) stays manual via the quarantine ledger.

**Proof:** 13/13 wave suite green on the FIRST run; 43/43 adjacent (transfer consent/console/real-apply/wave-A + passport vault) green on top of `people.0064`; render-safety / tenant-scan / marker-quality gates 0; URL-name integrity 3,853 refs / 0 unresolved; `makemigrations --check` + `manage.py check` clean. The e2e seeds two duplicates where BOTH hold attendance for the same (classroom, date) unique boundary: the clean guardian + attendance rows re-point to the primary, the colliding row quarantines on the operation and stays queryable on the retired secondary (asserted: both rows still exist — nothing deleted), the secondary lands `is_active=False` + `merged_into=primary`, lineage row present; approval-gating, cross-school refusal, already-merged refusal, illegal FSM transitions, and the full console flow (create→approve→apply, 409 without approval, non-staff 302) all asserted.

## 2026-07-03 — 9.8-regime merge/split Wave B: transfer go-live (migrations `people.0062` + `people.0063` + `migration_cloud.0034`, no SW bump)

**Context (design `docs/TRANSFER_MERGE_SPLIT_ORCHESTRATOR_DESIGN.md` §4–§5; Wave A shipped the dry-run foundations):** Wave B is the go-live slice — a consented, approved `TransferCase` now moves a REAL student end-to-end: guardian consent artifact → sealed envelope export → real migration-cloud apply at the target school → passport GUID linking both profiles → source retirement as `TRANSFERRED` → reconcile. The exit criterion was a real-apply e2e, and exactly as in Wave A, running the real path (not the dry-run) exposed production bugs that every prior wave's dry-run scoping had masked — including one data-model flaw that made ANY inter-school transfer structurally impossible.

**What landed**

| Piece | Files | What it does |
|---|---|---|
| Consent artifact | `apps/people/models_transfer_consent.py`, `apps/people/migrations/0062_*` | `TransferConsent` clones the GuardianConsentToken discipline verbatim: raw token returned exactly once from `mint()` (sha256-only persisted, `hmac.compare_digest`), immutable consent-text version+sha256, server-captured IP/UA, decision FSM (pending→consented/declined/expired). `consent()` advances the case CONSENT_PENDING→APPROVED and pins `consent_reference`; `decline()` cancels the case. Expiry guard runs BEFORE `transaction.atomic()` so the EXPIRED marking survives the raise (found by test: an in-block raise rolled the save back). |
| Anonymous consent pages | `apps/people/views_transfer_consent.py`, `templates/people/transfer_consent/{landing,decided,_consent_text_v1}.html`, `config/urls.py` | Uniform-200 anti-enumeration landing + decide pages (token in query, sha256 lookup, `mark_first_seen` stamp). Routes carry `# rbac-allow: anonymous-by-design-consent-token-in-url-sha256-lookup`. |
| Transfer engine | `apps/people/transfer_service.py` | `run_transfer_case`: APPROVED guard → offline-pending guard (QUEUED/SYNCING/FAILED/CONFLICT `OfflineAction`s at the source block the export — replay after a school change would strand frozen-`school_id` writes) → EXPORTING → envelope sealed (checksum journaled onto the case) → APPLYING → `ingest_transfer_envelope(dry_run=False)` idempotency-keyed on the case → passport link (id-mapping first, external-ref fallback) + source retirement → APPLIED → reconcile (case reaches RECONCILED only if the bundle does) → lineage stamp (`student.transfer`, wave-b) + compliance audit. Apply failures compensate: COMPENSATING → best-effort `trigger_rollback` on the bundle's runs → FAILED. |
| Orchestration runner | `apps/orchestration/runners.py` | `StudentTransferRunner` (`code="student_transfer"`) binds a case to the orchestration rails (append-only step events, retry, SLA); malformed/unknown `case_id` returns a `case_not_found` outcome instead of a 500 (UUID `ValidationError` caught — found by test). |
| Operator console | `apps/portal/views_transfers.py`, `apps/portal/urls.py`, `templates/super/transfers/cases_index.html` | `/portal/super/transfers/` staff-only console (devices-console pattern: `?format=json` branches, csrf-protected mutations, 500-row cap, best-effort AuditLog): case list, create (same-school 400), request-consent (mints token, returns the consent URL path exactly once), run (409 on guard refusals). |
| School-scoped landers | `apps/migration_cloud/landers/{_helpers,student_lander,guardian_lander,enrollment_lander,attendance_lander,grades_lander}.py` | `resolve_student` + upsert keys are school-scoped on single-schema deployments — an unscoped external-id lookup could resolve a same-id student from ANOTHER school, exactly the hazard a transfer creates by design (source and target share the ref). Created rows bind to the bundle's school. |
| Status honesty | `apps/people/models.py` | `StudentProfile.Status.TRANSFERRED`; the source profile retires as TRANSFERRED + `is_active=False` — never deleted, never silently ALUMNI. |

**Production bugs found by the real-apply e2e (the wave's honest headline — none were visible to dry-run):**

1. **Global-unique student identifiers made inter-school transfer structurally impossible** (`apps/people/models.py`, migration `people.0063`). `student_code` and `admission_number` were `unique=True` GLOBALLY — two different schools could not both hold the same number, so the target-school upsert hit IntegrityError and the student row quarantined on every transfer. Schools issue their own identifiers; uniqueness is now per school (`UniqueConstraint(school, student_code)` / `(school, admission_number)`, blank-excluded, fields keep `db_index`). The 5 school-unscoped `.get(student_code=)` sites this exposed (4 in `apps/evals/importers.py`, 1 in `apps/evals/management/commands/import_grades.py`) now pin to the import's school via its subject-assignment/academic-year — previously a same-code student at another school could be matched by a grade import. `apps/customersuccess/bulk_csv_student_import.py` already prefixes `S{school_id}-` and stays correct.
2. **Enrollment lander never implemented its own contract** (`apps/migration_cloud/landers/enrollment_lander.py`). Its docstring promised "section … linked to a Section if present" but nothing was linked: a canonical `academic_year` string was assigned onto the FK column (ValueError → every enrollment row carrying that column quarantined), and the extractor's emitted keys (`section`, `enrollment_date`) didn't match the lander's expected keys (`section_code`, `enrolled_at`) — so at real apply, enrollment was a silent no-op and attendance (classroom NOT NULL) could never bind. The lander now resolves placement school-scoped — classroom by code-then-name, academic year by name (or from the resolved classroom) — writes `section`/`joined_date` scalars, and quarantines VISIBLY when a named structure doesn't exist at the target ("create the structure and re-drive"), never landing a half-placed student silently.
3. **`MigrationIdMapping` audit rows were silently clobbered across domains** (`apps/migration_cloud/models.py` + `landers/_helpers.py`, migration `migration_cloud.0034`). The unique constraint `(legacy_namespace, legacy_id, canonical_model, school)` omitted `domain`, and `record_id_mapping` passed `domain` in `defaults` — so when the enrollment lander touched the same student the students-domain row's `domain` was rewritten to `enrollment`, erasing the students audit entry on EVERY multi-domain bundle (transfer or ordinary migration). `domain` is now part of the constraint and the lookup identity; each domain keeps its own row.
4. **Consent expiry marking was rolled back by its own transaction** (`models_transfer_consent.py`): `_guard_undecided` saved EXPIRED then raised inside `transaction.atomic()` — the raise rolled the save back, so expired tokens stayed `pending` forever. Guard moved before the atomic block.
5. **Runner 500 on malformed case id** (`apps/orchestration/runners.py`): `filter(pk="not-a-uuid")` raises `ValidationError` on a UUID pk; now a `case_not_found` outcome.

**Proof (runs 1→4, keepdb-free lane, orchestrator worker pinned to 1):** run 1 exposed bugs 1/4/5 (40/45); run 3 after those fixes exposed bug 3 via the sole remaining failure (44/45); a worker-pinned diagnostic caught the domain-clobber red-handed (mapping row present with `domain=enrollment`); run 4 (all fixes + `migration_cloud.0034`) = Wave B + Wave A suites AND the 12-module adjacent set (passport vault, envelope depth/runtime, mc intake/hardening/connector-secret/tier3, orchestration runners/workflow-engine, evals import hardening/route-health): **173 tests, 172 green**; the single error was a peer mid-save race in an adjacent render test (`text_on_brand` filter used by a peer-modified `portal_base.html` before its templatetag registration landed — neither file is in this wave), green on re-run against the consistent tree. e2e asserts: NEW target profile (different pk, same admission number), enrollment placement resolved to the TARGET's classroom, attendance bound to it, guardians quarantined visibly (target has no linkable guardian user yet — Wave C re-link), passport GUID equal on both profiles, source TRANSFERRED + inactive, per-domain id-mapping rows present, unapproved/rerun/offline-blocked all refused, drained queue unblocks the same case.

**Wave B.1 — same-day adversarial audit closeout (4-track audit of the shipped slice; every confirmed finding closed, none deferred):**

1. **Consent-surface hardening restored to the GuardianConsentToken bar** (`views_transfer_consent.py`, `models_transfer_consent.py`): the landing page holds the LIVE raw token (hidden field + query string) but shipped with no anti-cache discipline — now `@never_cache` + `Cache-Control: no-store` + `Pragma`/`Expires` + `Referrer-Policy: no-referrer` on both anonymous views (the global `strict-origin-when-cross-origin` policy was re-sending the token-bearing URL as same-origin Referer). Decision IP is now XFF-first — behind the Render proxy `REMOTE_ADDR` is the load balancer, so every consent's "written evidence" IP was a constant infra address. The decide view also catches `TransferStateError` so a concurrent double-decision renders the uniform page instead of a 500 on the anonymous surface. Audit-verified clean: CSRF (stricter than the clone — no `csrf_exempt`), token high-entropy + constant-time sha lookup, zero raw-token logging, uniform-200 anti-enumeration, autoescape intact.
2. **Double-run race closed** (`transfer_service.py`): `advance()` trusts the in-memory instance, so two operators clicking Run concurrently would both pass the APPROVED guard and drive the SAME bundle from two pipelines — `run_transfer_case` now single-flights on `cache.add` (the platform's billing-lock pattern) + re-reads status from the DB under the lock.
3. **Stuck-case recovery path** (`views_transfers.py::transfer_case_abort` + console button): a process crash mid-APPLYING stranded a case forever (the transition table allows APPLYING→FAILED but nothing could drive it). Abort cancels pre-run cases, marks a crashed EXPORTING/APPLYING run FAILED, refuses success states (409).
4. **Grades excluded from console-created cases** until the grades lander resolves term/subject FKs at the target (Wave C) — a default-domains run would have quarantined every grade row and never reconciled clean. Console rows now surface the journal's `last_note` (failure/abort/reconcile-below-threshold reasons) in JSON and HTML.
5. **Consent-link email delivery wired best-effort** (`schoolops.email_compat`, `fail_silently`) — the console still returns the URL exactly once, so a mail outage never blocks or re-mints.
6. **Five more school-unscoped admission-number lookups found and fixed** (constraint blast-radius sweep beyond the five evals sites): `portal/forms.py` LinkChildForm resolved a guardian's "my child" by admission number GLOBALLY (pre-existing cross-tenant link hole; now scoped + ambiguity-rejecting), StudentOnboardingForm's duplicate check would now wrongly reject a number free at THIS school (and leaked cross-tenant usage; also its `school` kwarg was popped but never stored), and `views_parent.py`'s link-child wizard had two more (step-1 `.get` → scoped + `MultipleObjectsReturned` handled; step-3 card → scoped `.first()`); all three `LinkChildForm(...)` callers now pass the tenant school. Verified safe: `generate_admission_number` sequencing is per-year/per-school (the change FIXES a pre-existing cross-tenant IntegrityError coupling), `pos_checkout` and both bulk importers already scoped, remaining `get_object_or_404` sites are pk-based.

Proof: 39/39 green on the keepdb lane (transfer consent/console/real-apply/wave-A + portal link-child + onboarding-verification — the last two exercise the form/wizard scoping); tenant-scan/marker-quality/render-safety/magic-numbers/url-name gates all clean vs baseline; `manage.py check` clean. Residual accepted with reasons: no throttle on the anonymous consent endpoint (parity with the guardian-consent clone; 256-bit token, no timing oracle) and `ProcessDefinition("student_transfer")` not seeded (the runner is rails for orchestration-driven ops; the console is the go-live driver — seeding lands with Wave C's periodic advancement work).

**Honest scope:** grades stay out of the e2e domain set (the grades lander passes a term STRING to Evaluation's term FK — it would quarantine; Wave C item). Guardian re-linking at the target needs user-provisioning (Wave C). Consent email delivery is not wired (the console returns the consent URL for out-of-band delivery). Reconcile below threshold leaves the case APPLIED with a journaled note for operator review — it does not fake RECONCILED. Wave C = record merge (FK-walker re-parent, collision quarantine, tombstone); Wave D = school merge/split composition.

## 2026-07-03 — 9.8-regime merge/split Wave A: transfer foundations (migration `people.0061`, no SW bump)

**Context (B-measure 5.8, design `docs/TRANSFER_MERGE_SPLIT_ORCHESTRATOR_DESIGN.md`):** Wave A of the transfer orchestrator — the foundations that make a student transfer an artifact instead of an aspiration. Everything here is DRY-RUN scoped by design: the real apply (consent artifact, orchestration runner, operator console, offline-pending guard, link/reconcile) is Wave B, so the merge/split domain score does not move yet.

**What landed:** (1) `TransferEnvelope` gains an optional checksummed `domain_rows` payload (`{canonical_domain: [rows]}`, validated against `DOMAIN_CANONICAL_HEADERS`, omitted-when-empty so pre-existing single-record envelopes keep verifying byte-for-byte) + `envelope_from_dict()` wire round-trip. (2) NEW `apps/interop/student_transfer_export.py` — the first code in-tree that actually READS a student's rows into an envelope: per-student canonical extraction across students/guardians/enrollment/attendance/grades (finance deliberately excluded by default — design §9), `student_external_ref()` mirroring the lander's lookup-candidate order, sealed via `build_student_transfer_envelope()`. (3) NEW `TransferCase` (`apps/people/models_transfer.py`, migration `people.0061` — in `people` not `interop` because interop is not an installed app): guarded FSM (draft→consent_pending→approved→exporting→envelope_sealed→applying→applied→reconciled | failed/compensating/cancelled) where illegal transitions raise and legal ones journal into `history`; `source ≠ target` DB constraint; loose `target_bundle_id`/`consent_reference` refs so `people` stays uncoupled from migration_cloud. (4) NEW `apps/migration_cloud/transfer_intake.py` — the missing internal-tenant source: verifies envelope checksum + target-school hash, stages per-domain canonical CSVs through `connector_bundle_bridge` into ONE idempotent bundle (key = envelope checksum; re-ingest reuses the bundle, duplicate artifacts skip), and routes via `source_hint="runmycampus_canonical"` so the deterministic accelerator supplies identity mappings (zero AI calls). (5) Design §8 defects fixed: passport dual-rail (`passport_services` now also sets `StudentProfile.passport` — plus NEW `backfill_passport_links` command reconciling both directions, dry-run default); tier3 export's declared-but-unimplemented `guardians` domain now emits real rows; `apply_teacher_transfer_envelope` made idempotent (existing-Employment reuse + assignment `get_or_create` — its mocked tests updated to the new contract + a re-apply test).

**THE BIG FIND — 3 real production bugs in migration_cloud's advance→apply chain, exposed by the first dry-run e2e ever run through it:** (a) U4 assigned `bundle.mapping_summary` then called `mark_status` (fixed `update_fields` WITHOUT mapping_summary) followed by `refresh_from_db()` — the per-artifact column mappings were NEVER PERSISTED; (b) U3's `per_artifact_domain` write was lost the same way; net effect of (a)+(b): `_build_jobs` saw no mappings/domains, so **every apply driven by `advance_bundle` silently fell back to the `custom_fields` lander** — domain misroute + zero real-domain rows; (c) `_run_lander_under_schema` raised `AttributeError: 'DatabaseWrapper' has no 'tenant'` on non-tenant DB backends (single-schema sqlite lane), failing every artifact. All three fixed minimally (persist before mark_status ×2; `hasattr(connection, "set_schema")` capability probe). Evidence trail: diagnostic dumps show mapping_summary/per_artifact null and `MigrationRun` rows `custom_fields:students.csv FAILED` pre-fix; identity mappings persisted + students-domain dry-run counts post-fix.

**Evidence:** 26/26 wave tests green (8 passport dual-rail/backfill + TransferCase FSM; 9 envelope domain_rows/tamper/v1-compat/from_dict + real extraction; 5 teacher-apply incl. new idempotency; 4 intake e2e — dry-run counts ≥1 created, bundle back to MAPPED with `last_dry_run`, NO rows written at target, re-ingest reuses bundle, wrong-target refused, empty-envelope refused). Adjacent regression 114/114 (passport vault, both envelope suites, connector-secret ingest, hardening wave, API alpha, tier1-3, offboarding auto-export). `manage.py check` clean, `makemigrations --check` clean, tenant-scan clean for this wave (sole finding remains the peer T13 orchestration site), marker-quality clean, print/bare-except/migration-imports 0, role-strings/magic-numbers no new. Honest iteration trail: the e2e failed four distinct ways before green (thread-pool connections can't see TestCase transactions → worker-count env pin in test; source-hint didn't route to the accelerator; then bugs a-c above).

**Honest residual:** dry-run only — no data moves yet; `TransferCase` is not yet wired to the intake (the Wave B runner binds case→bundle and advances the FSM); no consent artifact, console, or offline-pending guard yet (Wave B); grades rows fall back to subject NAME when the subject has no code; real apply will need target-side student resolution order proven for history domains; extractor has no row caps yet.

**Deploy:** migration `people.0061_transfercase` (pure CreateModel). No SW bump (no static assets).


## 2026-07-02 — 9.8-regime wave 5: derived-value lineage — record-level provenance (migration `metadata.0015`, no SW bump)

**Context (B-measure 6.5):** the metadata app answers the *design-time* lineage question ("what uses this entity/field" — `MetadataDependency` + `lineage_api`), but the platform could not answer the *record-level* one: which Evaluation rows produced this report-card average, which windows fed this at-risk score? Derived values were computed and persisted with ZERO record of their inputs — the most consequential numbers on the platform (report cards, risk scores) were the least traceable, and a disputed grade or score had no evidence trail.

**What landed:** (1) new `apps/metadata/models_derived_lineage.py` — `DerivedValueLineage` (UUID PK, nullable school FK, output_model/output_pk, indexed computation name + version, `granularity` ∈ row/scope, `inputs` JSONField capped at 500 entries with an explicit `inputs_truncated` flag, indexes on (school, output_model, output_pk) + (computation, created_at)); `record_derived_lineage()` is best-effort — it never blocks the compute path that calls it, and refuses outputs without a PK (a lineage row that can't identify its output is noise, not provenance); `lineage_for(output)` query helper; migration `metadata.0015` pure CreateModel. **The granularity contract is the honesty mechanism**: `row` = explicit input PKs, `scope` = a queryset descriptor (model + filter scope + window) where the computation aggregates without materializing PKs — each caller records what it actually supports instead of pretending. (2) **Term report cards get row lineage**: `term_report_context` now exposes `lineage_evaluation_ids` (the exact Evaluation rows it read), and new `_record_report_card_lineage()` in `apps/reports/views.py` is wired after `_record_report_hash` at BOTH generation sites — term stamps `report_card.term` with those Evaluation PKs; annual stamps `report_card.annual` at honest scope granularity (annual aggregates the year without materializing rows). (3) **Nightly risk gets scope lineage**: `compute_nightly_risk` records `analytics.nightly_risk` per persisted `RiskFactor` with `risk_scope_inputs()` (attendance/evals windows + invoice scope + model version incl. heuristic fallback) — wrapped so lineage can never fail the batch. Evidence: 9/9 green (7 new `apps/metadata/tests/test_derived_lineage.py` — roundtrip, 500-cap + flag, best-effort refusals, risk-scope shape, term-context ids, term stamp row lineage, annual stamp scope — plus the 2-test cameroon report-context adjacency); the FIRST run failed 4/9 and both causes were real and fixed, not masked (the helper happily created a garbage row for an unsaved output → pk-guard added; test fixtures violated `Evaluation.full_clean()` year-match → corrected to the cameroon fixture pattern); `makemigrations --check` clean, migration-imports compare clean, tenant-scan clean for this wave's files (sole finding remains the peer T13 orchestration site), `manage.py check` clean. **Honest residual:** no lineage UI yet (query-layer only — an operator/auditor console is the natural next slice); attendance percentages, financial statement lines, and transcript archives still compute without provenance; scope-granularity records describe *which* querysets were read, not a value snapshot — it is not replay; no retention/purge policy on lineage rows yet.

**Deploy:** migration `metadata.0015_derived_value_lineage` (pure CreateModel). No SW bump (no static assets).
...
**Context (B-measure 6.0):** the platform ran two forked SSO stacks. Control-plane OIDC/SAML (`apps/api/oidc_rp.py` + `saml.py`) wrote a 1:1 `UserTenantBinding` and NO `SchoolMembership` — a fresh control-plane SSO user could authenticate with zero tenant access, because access is governed by membership, not the binding (its reverse accessors are never read for routing). Tenant-scoped SSO (`accounts/views_oidc.py`/`views_saml.py`) wrote membership and no binding. The OneToOneField artificially forbade the multi-school reality `SchoolMembership` already supports; the operator reassign view was keyed on `user_id` alone; and the binding model had ZERO direct tests. Pre-flight: the previously recorded `accounts.0040` migration drift was verified STALE — `makemigrations --check` clean at leaf 0045.

**What landed:** (1) `UserTenantBinding.user` OneToOneField → ForeignKey (`related_name="tenant_bindings"` — the old singular accessor had zero readers, verified), new `is_primary` (default True = exact backfill for existing one-per-user rows), `UniqueConstraint (user, school)` + partial `UniqueConstraint (user) WHERE is_primary` so tenant selection stays deterministic; migration `0046_multi_tenant_binding` (AddField + AlterField + 2 AddConstraints, no RunPython needed). (2) `_bind_tenant_for_user` re-keyed to `(user, school)` get_or_create (primary-first school resolution preserved; re-login refreshes provider/subject/issuer, never duplicates) and now calls new `_ensure_membership_for_binding` — **JIT `SchoolMembership`** mirroring `user.role` (operator claim-map value or least-privileged User default — never the membership model's ADMIN default), get_or_create-only so existing memberships are never modified; SAML `acs` inherits both via the shared `_bind`. (3) Operator reassign re-keyed: upserts the `(user, school)` binding, promotes it to primary demoting the old inside one transaction (demote-first — the partial constraint rejects two primaries), flips source=manual; other-school bindings are never deleted (audit trail intact). Console index + JSON now show `is_primary`. (4) First-ever direct tests: `apps/accounts/tests/test_tenant_binding_multi.py` (9: two-school binding, duplicate (user,school) rejected, two-primaries rejected, bind creates binding+JIT membership with role mirror, re-login no-duplicate + claim refresh, JIT never downgrades an existing membership, no-school writes nothing, reassign adds second binding + moves primary + preserves old subject, same-school reassign idempotent). Evidence: 9/9 new; SSO regression suites 15/16 — the 1 failure (`test_saml_acs_provisions_user_and_membership`, 400!=302) was PROVEN PRE-EXISTING by re-running it on pristine origin/main with this wave fully removed (identical failure; introduced by the earlier SSO-hardening seal `c02d23270` whose DSig validation rejects that test's unsigned mock — flagged for the peer/next wave, NOT masked), `makemigrations --check` clean, migration-imports 0, render-safety 0, marker-quality clean, role-strings compare clean, tenant-scan clean for this wave's files (the one new finding is a peer T13 orchestration site, noted for follow-up). **Honest residual:** flows C/D (tenant-scoped SSO) still don't write a binding row (operator console blind to them) — reconciliation direction decided (binding = audit sidecar alongside membership) but the C/D write is a follow-up; no tenant-chooser UI yet for multi-bound users (primary wins deterministically); Postgres CI remains ground truth for the migration.

**Deploy:** migration `accounts.0046_multi_tenant_binding` (additive + field alter; existing rows backfill valid under both constraints). No SW bump (no static assets).


## 2026-07-02 — 9.8-regime wave 2: cross-rail sync observability (no SW bump)

**Context (B-measure 6.3):** "sync" is three rails with three conflict stores and wildly unequal visibility — delta `SyncConflict` (well-instrumented: admin + Sync Center + SLO dashboard), SODP `OfflineAction` conflicts (tenant-portal ONLY — a MANUAL_REVIEW grade conflict could sit forever with zero operator signal), and the WAL review streams `rmc.wal.deadletter.<hash>`/`rmc.wal.conflict.<hash>` (WRITE-ONLY: no reader, UI, metric, or alert anywhere; permanently-failing offline writes vanished silently after 5 attempts). Zero `emit_counter`/`emit_gauge` calls existed in any sync path; `/healthz` never probed sync; no periodic job watched any backlog.

**What landed:** (1) `apps/observability/sync_health.py` — the single cross-rail collector: `collect_sync_health()` returns SODP (queued/failed/conflict + oldest-age lag), delta (pending + lag), WAL (backlog/dead-letter/conflict stream counts + depths, degrades to `available: False` without Redis) and emits 7 gauges through the observability metrics bridge (first sync metrics ever: `rmc.sync.sodp.*`, `rmc.sync.delta.pending`, `rmc.sync.wal.*`); `peek_wal_review_streams()` is the FIRST READER of the dead-letter/conflict streams (per-stream depth + newest samples, fields reduced to `error`/`domain` and truncated — triage surface, never full envelopes); `evaluate_backlog_incidents()` checks per-rail thresholds and auto-opens/auto-resolves a `PlatformIncident` via the idempotent `incident_services` (key `sync_backlog_<rail>`, type DATA, severity HIGH) — so breaches surface on the existing operator incident console/banner rails. (2) Operator console `apps/portal/views_sync_health.py` + `templates/super/sync/health_index.html` at `/portal/super/sync-health/` (staff-only, JSON+HTML): all three rails on one page incl. the WAL review-stream tables. (3) Hourly LIGHT job `observability.sync_backlog_monitor` registered in the self-healing `periodic.py` registry (auto_eligible — runs off the /health/ tick, watched by the dead-man's-switch). (4) Thresholds settings-driven: new `RMC_SYNC_SODP_CONFLICT_MAX` (25) + `RMC_SYNC_WAL_DEADLETTER_MAX` (10) in settings + registry rows; delta rail reuses the existing `SYNC_CONFLICT_PENDING_SLO_MAX`. Evidence: 9 new tests (`apps/observability/tests/test_sync_health.py` — collector counts all rails, Redis-absent degrade, reader truncation + field-scoping + unknown-kind, breach→incident→idempotent-update→recovery-resolve, below-threshold no-op, console JSON shape + non-staff 302, job registration), render-safety 0, tenant-isolation 0 (3 new markers, quality gate clean), inline-style 0, undefined-css 0, magic-numbers compare clean, settings-registry coverage OK, documented-baselines no drift. **Honest residual:** the monitor is platform-rollup (not per-tenant incidents); WAL dead-letter REPLAY (not just triage) and SODP conflict rows on the SLO dashboard aggregation remain for the next sync wave; `/healthz` still does not probe sync (deliberate — hot-path budget).

**Deploy:** no SW bump (Python + server-rendered template only). No migrations.

## 2026-07-02 — 9.8-regime wave 1: device governance — revocation is a capability (no SW bump)

**Context (B-measure under the 9.8/lowest-dimension regime):** six-domain evidence sweep scored device governance the true floor (4.9 — below merge/split's 5.8) on a confirmed security enforcement hole: `DeviceRegistration.revoked_at` was a field with no capability behind it. No endpoint or UI could revoke a device; the token's own `revoked_at`/`is_valid` was never consulted at any enforcement point; and — worst — BOTH registration paths (`apps/api/offline_device_api.py` mint + `apps/platform_runtime/offline_queue.py::_apply_provision_signup` queued-signup replay) hard-set `"revoked_at": None` in their `update_or_create` defaults, so a revoked (stolen) device silently un-revoked itself by re-minting. The only revocation triggers were whole-user offboarding and tenant purge.

**What landed:** (1) both registration paths now REFUSE a revoked device (`device_revoked`: mint → 403, queue applier → `{"ok": False}`) and no longer reset `revoked_at`; (2) new operator console `apps/portal/views_device_governance.py` + `templates/super/devices/registrations_index.html` at `/portal/super/devices/` (staff-only, filterable, JSON+HTML like the SSO-bindings console): **revoke** cascades to all outstanding `OfflineCapabilityToken` rows (school-scoped update) and **reinstate** is the single explicit, audited path back — both write best-effort `compliance.AuditLog` entries (PERM_REVOKE/PERM_GRANT); (3) `scripts/verify_offline_auth_contract.py` extended — fails if either path ever regains a `revoked_at: None` reset or loses the `device_revoked` refusal, or if the revoke surface stops cascading to tokens; (4) new `apps/api/tests/test_offline_device_governance.py` (8 tests: mint-refuses-revoked incl. no-resurrect + no-token-minted, queue-replay-refuses-revoked, active-device mint unaffected, revoke-cascades-to-tokens, reinstate, non-staff 302, index JSON, unknown-device 404). Evidence: 14/14 (8 new + 6 existing mint-contract), provision-signup suite green, `manage.py check` clean, tenant-isolation 0 (cascade update genuinely school-scoped, no new markers), render-safety 0, inline-style 0, undefined-css 0, contract verifier PASS. **Honest residual (documented in the revoke docstring):** an already-minted signed IAM snapshot keeps self-verifying offline until its TTL expires — revocation bounds the future (no re-mint, all server-checkable tokens dead now); it cannot reach into an offline device. Device list/limits per tenant-admin (vs operator) + stale-device expiry job remain for the next device wave.

**Deploy:** no SW bump (Python + server-rendered template only, no static assets changed). No migrations (fields already existed — the defect was enforcement).

## 2026-07-02 — Sovereign-audit A-wave: offline mint contract + strict residency + ONLINE_REQUIRED (`v4.05.94`)

**Offline capability mint contract (SW `sms-v4.05.94`):** the SODP write-capability loop had no functioning client driver — `rmc-offline-auth-bootstrap.js`'s refresh (a) was gated on a sealed vault that nothing ever seals (`sealCapability` has zero callers), (b) POSTed without the REQUIRED `device_id` → 400, (c) sent no `X-CSRFToken` → 403 under SessionAuthentication, (d) read `capability_blob_b64` while the server returns `capability_blob`, and (e) wrote a non-sealed shape into the PIN-wrap vault that `openCapability` can never open. Fixed: mint now fires when the tenant offline flag is on, throttled to the token's half-life via `rmc_offline_capability_mint_state`, sends `device_id` + CSRF (DOM-first pattern from `offline-queue-client.js`), applies the returned `iam_snapshot` via `RMCIamSnapshot.applyMintResponse`, and hands the raw blob to a PIN-holding surface via the `rmc-offline-capability-minted` event instead of storing it unsealed. Server TTL is now settings-driven (`RMC_OFFLINE_CAPABILITY_TOKEN_TTL_HOURS`, default 12h unchanged; registered in `settings_registry`) so long-offline campuses can extend the write window without a code change. `verify_offline_auth_contract.py` extended to lock both sides of the wire contract (device_id, CSRF header, `capability_blob` field names, no hardcoded TTL literal). New `apps/api/tests/test_offline_capability_mint.py` (6 tests: required device_id, school_required, wire-contract keys, fingerprint-only storage, default + overridden TTL, re-mint dedupe).

**Data-residency strict-unknown mode:** the border-lock gate's unknown-region fail-open is now policy-controlled — `DATA_RESIDENCY_STRICT_UNKNOWN` (env or setting, default off/backward-compatible; registered) makes an unknown TARGET region or an unresolvable SOURCE region BLOCK (`ResidencyViolation`, audited) instead of silently passing when enforcement is on. 3 new tests in `test_data_residency_border_lock.py` (blank-target block, resolver-exception block, matching-regions still pass). Residual documented in the module docstring: db-router/middleware call sites still early-return when no region concept applies (single-region deployments must not brick).

**CRDT policy registry v2 — `ONLINE_REQUIRED` tier:** closes the sovereign audit's one real gap in `policy_registry.py` — a tier for domains that may not even be QUEUED offline (vs SERVER_AUTHORITATIVE where the attempt is accepted and the server wins): `tenant_lifecycle_action` (suspend/offboard/purge), `security_credential` (password/MFA), `payment_settlement` (live gateway charges; offline evidence stays on `fee_payment`/`payment_proof`). Aliases (`offboarding`, `mfa`, `password`, `settlement`, `tenant_lifecycle`), `is_online_required()` helper, explicit `reject_offline` arm in `conflict_resolver.resolve_one` (override-blocked like all protected entities), `POLICY_VERSION` 1→2. 2 new tests in `test_sync_policy_registry.py`.

**Seven-day offline endurance harness v1 (sovereign Wave 4):** new deterministic `apps/platform_runtime/tests/test_seven_day_offline_endurance.py` — a mocked-clock 7-day disconnect against the REAL server rails: 12 actions across 2 users/devices with a per-day duplicate-resend storm (deduped at enqueue), day-7 double-replay (zero loss, zero double-application, 12 unique tickets), tenant-isolation probe (foreign tenant processes nothing), token posture (default 12h token dead by day 1; settings-driven 168h token covers the week and hard-expires past its anniversary), and CRDT order-independence + replay-idempotence under a week of skew. HONEST CLAIM LEVEL (updated same day): the client legs now ran in a REAL browser — `scripts/verify_client_offline_endurance.py` (selenium/Chrome, persistent profile) proved 8/8: sealed PIN-vault + mint-throttle state + outbox row survive a full browser kill+relaunch, capability decrypts with the correct PIN and refuses a wrong one, localStorage quota genuinely reached, `saveSealed` never throws under pressure, capability still opens after pressure. Server rails + client legs together = **SEVEN_DAY_OFFLINE_PROVEN — REPO SCOPE** (caveats: Chrome-only, synthetic page origin, single device profile; multi-browser + real-PWA-shell runs remain for a device-lab pass).

**Deploy:** SW `CACHE_VERSION` → `sms-v4.05.94-offline-capability-mint-contract-2026-07-02` (bootstrap JS changed; v92-93 were claimed by a concurrent operator-dropdown wave). No migrations.
...
**Axe (local real-browser run, 5 suites):** 3 REAL findings documented for review, NOT auto-fixed (shell-palette + brand-color territory): (a) **MARKETING SHELL FIXED 2026-07-02** (`theme-preference-bootstrap.js` now excludes `html[data-surface="marketing"]` from the backend body palette; real-Chrome proof `scripts/verify_marketing_palette_scope.py` — marketing palette-free AND portal control still themes dark; base-shell pair forgot-password/notifications REMAINS PARKED for the dual-plane reviewed wave). Original finding: public/marketing pages serve `<body class="portal-backend-dark">` → dark-theme `#dee2e6` text measured at **1.21:1** on the cream `#faf7f2` marketing surface (marketing home, pricing, release-notes, forgot-password, notifications) — the documented backend-palette class-wipe leaking cross-surface; (b) `.btn.btn-primary` white-on-`#0ea5e9` = **2.77:1** (help-center/retry buttons); (c) ~~`/healthz/` missing `html lang` + `title`~~ **CLOSED 2026-07-02 as harness artifact:** the view returns `JsonResponse` (`apps/observability/views.py::healthz`) — pure `application/json`; the flagged markup was Chrome's JSON-viewer wrapper. Machine endpoints must be excluded from the axe URL set. Harness note: `axe-selenium-python` bundles axe-core 3.1 (2018) which emits `#0NaN…`/null-ratio false positives on `color-mix()` CSS — recommend migrating the axe harness before treating its full output as a gate.
...
**Audit + validation (2026-06-29) — "platform-wide + tenant-wide, past/present/future tenants":** verified the whole tenant-shell fix family (scroll/rail/More/full-bleed) is UNIVERSAL by construction, not per-tenant: (1) **Loading** — `portal_base.html` links every tenant shell sheet inside `{% if request.public_host_kind != 'manager' %}` (HOST-based = every tenant subdomain, NOT a per-tenant flag); `rmc-nav-sidebar.css` + `rmc-platform-vertical-compact.css` load unconditionally; the platform shell `control_plane_skeleton.html` loads the operator-header + rail + compact sheets and is full-bleed via its 3-col grid. (2) **Gate** — the rules key off `data-rmc-tp-v3-shell="1"` + `data-rmc-cp-scroll="canvas"`, emitted whenever `tp_v3_tenant_shell` is true; that resolves through `use_v3_shell_for_request` whose policy key `use_v3_shell` **defaults to True** (`tenant_experience_policy_defaults`), so every past/present/future tenant is on the v3 shell + gets the fixes BY DEFAULT. The ONLY exception is a deliberate per-tenant opt-out via the cockpit field `txp_use_v3_shell` (default checked; help text "When off, the school uses the legacy portal chrome") — an intentional escape hatch, left intact. (3) **Wipe-survival** — every fix selector is scoped to the attribute/id hooks (proven `classWipeActive:true` while `tpv3Attr:"1"` + header still full-bleed); the only live `.portal-body-with-layout` selectors left in the edited sheets are the INTENTIONALLY-dead `container-type` rule (nav-sidebar.css — re-scoping it reactivates the 2px collapse) and a tangential role-home ticker-dedup rule. **In-browser validation @1920px:** header full-bleed (`headerInner_rightGap:0`, `padding-inline-end:0`) and footer full-width in BOTH copilot collapsed AND expanded states (the old gutter would have inset the header 360px when expanded). **Test reconciliation:** the shipped CSS changes (v4.05.88 measured main-cap + v4.05.89 gutter removal) had left 2 stale string-assertions red — `test_mission_strip_lives_in_dashboard_surface_not_header` (asserted the retired `max-height: 100% !important` → now asserts the `--rmc-tenant-main-h` measured cap + `height:100%` + `overflow-y:auto`) and `test_tenant_copilot_horizontal_collapse_contract` (asserted the retired header gutter → now asserts the retained MAIN-canvas `padding-right` gutter in the compact sheet). Also fixed a pre-existing Django-API test bug in `test_use_v3_shell_false_disables_shell` (`get_user_model()(is_authenticated=True)` → `get_user_model()()`; `is_authenticated` is read-only). Tenant/shell-context suites green (42 passed across the 3 modules); the 7 control-plane errors are sandbox sqlite test-DB locks (`WinError 32`), not assertion failures. CSS/JS unchanged this pass → no SW bump.
...
**ROOT CAUSE (pivotal, probe-proven):** the backend theme bootstrap leaves `<body>` carrying only the palette token (`portal-backend-dark`) — the server-rendered `portal-body-with-layout … tp-v3-inner` classes are gone post-load (all `data-*` attributes survive; only the class is reset, via an HTML-parse path that bypasses every DOM setter — `className=`/`setAttribute`/`classList.*` instrumentation caught nothing). That class is the scope for the **entire** `body.portal-body-with-layout` tenant CSS layer (app-shell sticky chrome, viewport-lock canvas scroll, `rmc-tenant-cp-parity` sidebar item layout). Once gone, those rules silently de-scope → items render bare inline `<a>`, the canvas lock / sidebar scroll break. Re-adding the class in-browser flipped `.cp-sidebar__item` `display:inline`→`flex` and `padding-left` `0`→`26px`, confirming the mechanism.

**Shipped:**
- **#3 sidebar + main scroll** — `rmc-tenant-shell-metrics.js` now publishes `--rmc-tenant-footer-h` + `--rmc-tenant-sidebar-inner-h` (exact = canvas workspace height − collapse toolbar, measured; ResizeObserver-tracked). `rmc-tenant-workspace-canvas.css`: body-lock rules re-anchored from `body.portal-body-with-layout[data-rmc-cp-scroll="canvas"]` to attribute-only `body[data-rmc-cp-scroll="canvas"]` (survives the rewrite); `.tp-sidebar-inner` capped to the published var (calc fallback). **Why a cap, not `height:100%`:** in-browser, `.portal-sidebar-col` is a flex item that is ITSELF a column flex container — it grows to content and ignores `height/max-height:100%` (both proven not to constrain it), overflowing below the locked body unreachably. Only capping the scroll TRACK works (main col already scrolls via the existing `height:100%!important` block). Verified: inner `clientH 563 < scrollH 923` (scrolls); col 614 ≈ the 616 viewport-minus-chrome.
- **#2 collapsed-rail leak** — `rmc-nav-sidebar.css`: the rail label-hiding only ever covered the legacy portal grammar + the operator `#cp-sidebar-col`; added the tenant v8 `#portal-sidebar-col.rmc-nav-sidebar--rail` grammar (`.cp-sidebar__section`/`.cp-nav-label`/`.cp-sidebar__pin`/`summary`/badges/toggle-label/filter inputs → `display:none`), center the icons, and force `<details>` group items visible as icons in the rail. Verified: rail label `display:none`, `labelLeak:false`.
- **#4 submenu items one-per-line** — `rmc-tenant-cp-parity.css`: re-anchored all 62 selectors from `body.portal-body-with-layout:not(.control-plane-shell)` to `body:not(.control-plane-shell)` (the `html[data-rmc-tp-v3-shell="1"]` prefix already scopes to the tenant shell, so the class was redundant — dropping it restores the whole layer under the rewrite). Verified: `.cp-sidebar__item` `display:flex`, pad `26px`.
- **#1 operator "More ▾"** — `rmc-cp-header-200x.css`: `.cp-primary-nav` `overflow-x:auto`→`overflow:visible` (the scroll + the direct clip of the dropdown); expanded the `:has(.dropdown-menu.show)` escape-hatch to lift overflow on EVERY container in the chain (consolidated-inner / chrome / row / nav-slot / cp-nav-row / platform-header inner+actions / cp-primary-nav itself), + closed-state `overflow:visible` on any nav placement, + `z-index:1060` on the menu. Verified on a real-CSS harness: nav `overflowX:visible`, `hScroll:0`, menu `display:block` `z:1060`, drops below the band, `elementFromPoint` = the menu `<li>` (on top + hittable).
- **Do NOT re-assert `portal-body-with-layout` (reverted, hotfix `d41d42181`→next):** the first cut of this wave had `metrics.js apply()` re-add the class as belt-and-suspenders to restore the rest of the de-scoped layer. That REGRESSED the main canvas: reactivating `body.portal-body-with-layout .portal-main-col > .page-wrap.rmc-shell-content-grid { container-type: inline-size }` collapsed `.page-wrap` to a **2px sliver** inside the flex-column main col (every page's content crushed into a vertical strip — owner-reported). That layer has been INACTIVE in production for the tenant's whole lifetime (the class is always stripped), so reactivating it is unsafe — it surfaces latent bugs like this. The re-add was removed; the shell fixes stand alone on their stable anchors. Verified in-browser after removal: page-wrap width 1054 (≈ main col 1118, was 2), and #2/#3/#4 all still pass with the class absent (`bodyClass = portal-backend-dark`). LESSON: re-anchor individual rules off the wiped class; never blanket-reactivate the dead layer.

**Files:** `static/js/rmc-tenant-shell-metrics.js`, `static/js/service-worker.js`, `static/css/rmc-tenant-workspace-canvas.css`, `static/css/rmc-nav-sidebar.css`, `static/css/rmc-cp-header-200x.css`, `static/css/rmc-tenant-cp-parity.css`.

**LESSON (durable):** any tenant CSS scoped to `body.portal-body-with-layout` is fragile — the backend palette rewrite drops that class. Prefer the stable hooks: `html[data-rmc-tp-v3-shell="1"]` + `data-rmc-cp-scroll` attribute, or the element id. The `#portalHeader` re-anchor (prior wave) and these are the same fix family.
