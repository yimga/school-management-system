...
| super_dashboard, super_command_center, governance_console, app_catalog | cp-hero / proof-hero (role-home or catalog) | Keep hero; ensure one primary CTA. **governance_console:** Back to Dashboard first in cp-hero-actions; dashboard_url in context. **super_command_center:** All 7 queue empty blocks use studio_os/components/loading_empty_states (Pending approvals, Trial watchlist, Provisioning breaches, Stale support, Platform incidents, Billing exceptions, Churn risk). |
