...
- **Tenant example links:** set `TENANT_EXAMPLE_SLUG` (e.g. `gilead-school`) in env so marketing copy can link to a real subdomain. If `MARKETING_DEMO_TENANT_URL` is empty, settings derive `https://{TENANT_EXAMPLE_SLUG}.{MULTI_TENANT_BASE_DOMAIN}/` for the “Try demo” CTA. See `seed_gilead_demo_users`.
