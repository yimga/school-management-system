...
### Epic P2 — repository vs per-database verification (2026-03-28)

- **Repository / merge bar:** `scripts/pre_deploy_gate.sh`, `scripts/verify_phase_5_siteconfig.py`, and `scripts/verify_siteconfig_decomposition_depth.py` prove the **committed** migration chain, **domain_ownership**, and **runtime-first** wiring are consistent. This satisfies **“Batch 14+ verified”** for **the codebase and CI gate DB** in the SOT epic sense.
- **Per environment (staging / each production database / tenant schema):** Running `migrate` and `scripts/verify_phase_b_execution.py` on **live** databases remains **operator-owned**. Record hostname, date, and migrate output in your **release runbook**; this file does not certify every remote DB has applied every migration.

### Scoped gravity — `SiteSettings` reference trend (inventory)

Decomposition slices should **pair** domain alignment with an honest count of remaining `SiteSettings` mentions in product Python:

- **Metric:** `docs/generated/platform_inventory.json` → `scoped_gravity_counts` → **`site_settings_refs_apps_py_excl_migrations_tests`** (and sibling `site_settings_refs_apps_py_excl_migrations` for migrations-included comparison).
- **Regenerate / verify:** `python scripts/generate_platform_inventory.py --write` then `python scripts/generate_platform_inventory.py --check` (also ordered in `scripts/pre_deploy_gate.sh`).
- **Gate pairing:** Run **`scripts/verify_siteconfig_decomposition_depth.py`** on every train that touches `domain_ownership` / Phase B snapshots; refresh inventory when §11.4 slices change app code so the trend line stays current in git.
- **P2 gross-scan product floor (§11.4 batch 136, 2026-03-30):** both **`site_settings_refs_apps_py_excl_migrations`** and **`_excl_migrations_tests`** can read **1** — only the structural **`SiteSettings`** class in **`apps/siteconfig/models.py`** remains in non-migration **`apps/**/*.py`** for the `\bSiteSettings\b` counter. Further Phase **14+** depth is **typed tables / field moves and parity scripts**, not chasing that string count.
- **P3 product-Python surface (§11.4 batch 1033+1036, 2026-04-26):** **`python scripts/audit_sitesettings_python_surface.py`** writes **`docs/generated/sitesettings_python_surface_audit.json`** (**`schema_version` 2** + **`per_file`**, unified **`violations`**, **`by_kind`**), aggregates **`get_effective_site_settings`**, etc., and **exits 1** on disallowed class-level **`SiteSettings`** access (including **`import SiteSettings`**, **`get_model('siteconfig','SiteSettings')`**, not only **`SiteSettings.objects`**). Bundled in **`verify_cursor_phase6_siteconfig_sitesettings.py`**. **1035** adds **`site_settings_read_access`** as the import surface for approved **`get_effective_***`** entry points.
...
- **Marketplace / integration credential slice (2026-03-30):** Tenant and Celery paths use `get_effective_marketplace_integration_settings(request=, school=)` in `platform_runtime.helpers` (same dict as `SiteSettings.get_marketplace_integration_settings()` on the merged facade). Contract tests: `test_marketplace_integration_helper_contract`, `test_get_effective_marketplace_integration_settings_matches_facade_method`, `verify_marketplace_integration_first_class_parity.py` + first-class `RuntimeDefaults` columns (**0029**–**0034**, **0037** partner secret).
- **Marketplace_integrations read path (Batch 14+ carry):** `get_effective_marketplace_integration_settings(request=, school=)` in `platform_runtime.helpers` is the canonical entry for tenant apps and Celery tasks; `SiteSettings.get_marketplace_integration_settings()` remains the facade implementation behind resolver merge. High-churn secrets are **RuntimeDefaults** first-class columns (`0029`–`0034`, `0037`); parity is enforced by `scripts/verify_marketplace_integration_first_class_parity.py`.
...
| **7** | **marketplace_integrations** — snapshot row (excludes typed integration secrets from JSON payload) | **COMPLETE** | Secrets stay off snapshot; **`sms_api_key`** (**0029**), **`ai_provider_api_key`** (**0030**), **`whatsapp_api_token`** (**0031**), **`marksheet_ocr_api_key`** (**0032**), **`smtp_password`** (**0033**), **`webhook_signing_secret`** (**0034**) are **`RuntimeDefaults`** first-class columns—not duplicated in **`RuntimeDefaults.payload`**; **`phase_b_domain_snapshots._MARKETPLACE_SECRET_KEYS`** strips them from materialized marketplace rows. Resolver strips first-class keys from JSON before shallow merge. Write path remains **`SiteSettings`**. |
...
| **14+** | **First-class tables / bounded-context moves** (e.g. **metadata_governance** `DynamicField*` consolidation, high-churn **marketplace_integrations** secrets split, **reports** SKUs) | **`DynamicField*` 0–5b; report SKUs + gates + **`TenantReportSchedule`** (**`reports.0018`**) + PostgreSQL RLS (**`reports.0019`**); ministry/EMIS **v1 API**; scheduled-report **API CRUD** + list **?is_active**/**?delivery_ready** + **POST/PATCH** echoes + **DELETE** **schema** + **tenant admin** (**last_run**, bulk deactivate empty recipients) + hub **staff** link + **api_v1** list **href**; **`send_scheduled_reports`** **--strict-no-skip**/**--json-summary**/**failed** **`CommandError`** + **`run_due_reports`** kwargs; **`/siteconfig/reports/download/`** + **`bulk-letters/`** gates; staff publish/promotion; **`pre_deploy_gate.sh`** → **`verify_gilead_full_tree_classification.py`**; marketplace RT **`0029`–`0034`** (**`webhook_signing_secret`** **`0034`**); **`seed_cursor_twelve_phases`** phase **10**; **`0036`** **`PlatformReportPlatformSkuDefault`** (manifest operator default bundle) | **Reports:** **`test_scheduled_reports_hub`** (+ non-staff + **zero-recipient** warning row), **`TenantReportScheduleModelValidationTests`**, **`test_scheduled_report_patch_activate_without_recipients_returns_400`**, **`test_scheduled_reports_list_delivery_ready_false_when_inactive_empty`**, **`test_scheduled_reports_post_inactive_allows_empty_recipients`**, **`test_scheduled_reports_post_active_rejects_empty_recipients`**, **`TenantReportScheduleAdminRecipientDisplayTests`** (+ **`test_active_empty_recipients_filter_queryset`**), **`SendScheduledReportsCommandTests`** (**`--school-id`**/**`--limit`**/stdout/**stderr**/dry-run/summary/**`assertLogs`**/**json-summary**/**strict-no-skip**/**SMTP** failure), **`ScheduledReportRunnerDelegationTests`** (+ **strict/json** forward), **`TenantReportScheduleAdminOperationsTests`**, API list **query-filter** cases, hub **`href`** assertion, **`ReportCustomBuilderFeatureGateAnyOfTests`** (download/bulk), **`TenantReportScheduleAdminRegistrationTests`**. **Workflow sim (§11.4 batches 140–141 + **145** + **150** + **156** + **161** + **163** + **165** + **169**):** **`test_two_step_playbook_failed_second_step_stops_audit_records_failed`**; **`test_two_step_playbook_failed_first_step_does_not_invoke_second`**; **`test_playbook_empty_profile_slugs_fails_without_steps_and_records_log`**; **`test_playbook_unknown_profile_slugs_fails_without_steps_and_records_log`**; **`test_playbook_inactive_profile_slug_skipped_active_remaining_runs_one_step`**; **`test_playbook_all_listed_profiles_inactive_fails_without_steps`**; **`test_single_step_playbook_dry_run_user_none_records_success_log_without_triggered_by`**; **`test_playbook_no_valid_profiles_user_none_records_failed_log_without_triggered_by`**; **`test_single_step_playbook_dry_run_school_none_records_success_and_null_school_on_log`**; **`test_playbook_no_valid_profiles_school_none_records_failed_log_with_null_school`**; **`test_single_step_playbook_dry_run_school_and_user_none_records_success`**; **`test_playbook_no_valid_profiles_school_and_user_none_records_failed`**; **`test_two_step_playbook_partial_then_success_yields_partial_audit`**; **`test_two_step_playbook_success_then_partial_yields_partial_audit`**; **`test_two_step_playbook_both_steps_partial_yields_partial_two_partial_steps`**; **`test_two_step_playbook_partial_then_failed_yields_failed_mixed_steps`** (`apps/automation/tests/test_workflow_playbook_simulation.py`). **CI:** **smoke-light** → **`verify_gilead_full_tree_classification.py`**. **Next:** **`0037+`** for the **next** marketplace credential column (parity train); **`0035`** = webhook audit; **`0036`** = **`PlatformReportPlatformSkuDefault`** (manifest **`operator_default_report_platform_bundle`**). Further Phase B typed tables per ownership. **Per DB:** **0168**; **`reports.0018`**–**`0020`**. |
| **14+ registry (2026-03-30)** | **EXACT_FIELD_OWNERS ↔ RuntimeDefaults columns** — no orphan keys | **DONE (repo):** **`apps/siteconfig/domain_ownership_storage.py`** (`VIRTUAL_ONLY_EXACT_FIELDS`); **`scripts/verify_domain_ownership_exact_storage.py`**; **`verify_phase_5_siteconfig.py`** invokes it; **`EXACT_FIELD_OWNERS`** extended for every **`RUNTIME_DEFAULTS_FIRST_CLASS_FIELD_NAMES`** entry (**`header_bg_color`**, **`grade_approval_auto_validate`**, **`public_brand_*`**, …); **`RuntimeDefaultsAdmin`** fieldsets grouped by **`domain_ownership`** domain labels; tests **`test_domain_ownership_storage`**, **`TenantSettingsLintTests.test_verify_domain_ownership_exact_storage_passes`**. |
...
**Between JSON snapshots and one-row-per-field:** `PlatformPhaseBDomainSnapshot` now carries **per–top-level-key** fingerprints (`payload_key_checksums`) plus the operator **Phase B snapshot diff** page (key-level drift + re-materialize from live SiteSettings). That narrows incident response **without** replacing the long-term plan to land **first-class tables** per high-churn ownership domain where `domain_ownership` calls for it.

**Verification:** `python scripts/verify_phase_5_siteconfig.py` (artifacts 0162 + 0163 + 0002 + `0007_platform_phase_b_domain_snapshots`); `python scripts/lint_phase_b_batch3_sitesettings_fk_writes.py` (no `site.save(update_fields=[...])` / `SiteSettings.objects.create(..., theme_pack=...)` for removed FKs); after DB migrate, `python scripts/verify_phase_b_execution.py`; `pre_deploy_gate.sh` runs these. **Mechanical pytest lane (§11.4 batch 138):** when touching app code or epic docs that feed **`generate_platform_inventory.py`**, run **`python scripts/generate_platform_inventory.py --write`** (then **`--check`**) **before** **`python -m pytest apps/platform_runtime/tests/test_tenant_settings_lint.py`** so **`test_generate_platform_inventory_check_passes`** stays green on committed JSON. Targeted tests: `apps.brand_experience.tests.test_platform_global_branding`, `apps.platform_runtime.tests.test_phase_b_domain_snapshots`.
