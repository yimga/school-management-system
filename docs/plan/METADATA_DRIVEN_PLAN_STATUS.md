...
| **2. Decompose siteconfig** | **Done** | Bounded-context app shells: `brand_experience`, `runtime_blueprints`, `policies_rules`, `plans_entitlements`, **`global_registries`**, **`integrations_marketplace`**. `docs/architecture/PLATFORM_DEFAULTS.md` documents platform defaults. |
...
| **4. Metadata catalog MVP** | **Done** | EntityCatalogEntry, FieldCatalogEntry, MetadataDependency, BusinessGlossaryEntry; `seed_entity_catalog`; `/super/metadata-catalog/` and impact view; **usage hooks**: `metadata.usage_registry.register_usage()` called from dashboard_registry, policies.resolver, runtime_resolver (workflow/dashboard steps). |
...
| **6. Governance, audit, isolation** | **Done** | MetadataChangeLog model; **post_save signals** record MetadataChangeLog for EntityCatalogEntry, FieldCatalogEntry, LayoutDefinition (metadata.signals), PolicyBundle, BlueprintPack (policies.signals); scope on LayoutDefinition; metadata roles in METADATA_GOVERNANCE_ROLES.md. |
...
| **8. Marketplace alignment** | **Done** | Blueprint, **theme** (template_gallery + brand_import), **workflow pack** (post_save WorkflowPackAssignment), **dashboard pack** (post_save DashboardPackAssignment) all route through PackageEngine; MARKETPLACE_PACKAGE_ALIGNMENT.md checklist all checked. |
