...
| Structural tech debt (mega-files) | **PARTIAL** | Accounts/schools splits done; `lint_mega_files` advisory; further splits per LEGACY_PATH_INVENTORY + backlog |
| Premium UX / sidebar / touring | **PARTIAL** | Phase H automation; **full manual “ultra high-end” pass** = release activity, not a single PR |
...
1. **§0.3 Pillar 1 [ ] structural tech debt** — Ongoing file splits and orchestration clarity; track via mega-file lint + LEGACY_PATH_INVENTORY.
2. **§0.3 Pillar 6 [ ] premium bar + global sidebar** — Requires design QA, nav audit, and optional productized touring; not closed by backend-only work.
