...
| Non-repudiation logger (WebAuthn-signed, write-once vault) | `apps/observability`, `apps/compliance`, `apps/migration_cloud` (audit-chain) | PARTIAL (audit chain shipped; WebAuthn-per-action + S3 Object Lock GAP) | `non-repudiation-logger` |
...
| State-reporting bridges (CALPADS / EdFacts / etc.) | `apps/platform_runtime/statutory_tenant_extract.py` + `emis/` | PARTIAL (generic aggregate; jurisdiction format GAP) | `state-reporting-bridges` |
...
**Wave D — Logistics (Phase 5). ◑ PARTIAL 2026-06-07 (boarding shipped).** Built the **non-phone RFID/NFC/QR boarding monitor** — `POST /api/transport/boarding/ingest/` (`BusBoardingIngestAPI`) → `schoolops/boarding_monitor.record_boarding` writes an append-only, idempotent `BusBoardingEvent` (migration `0025`), resolves the route from the student's `TransportAssignment`, fires a best-effort guardian notify (`parent_notified`). Append-only + idempotent = offline-safe by construction. Tests `test_boarding_monitor` 5/5; gates register 7/7, migration drift clean, check/DRF/print/bare-except 0. **Honestly NOT closed:** (a) route optimizer is **blocked on a prerequisite** — `schoolops.Stop` has no lat/lng, nothing to optimise until geo fields exist; (b) field-trip compliance stays PARTIAL — the consent e-sign pipeline supports field-trips but a dedicated surface + offline medical checklist is a further build.

**Wave E — Year-end governance (Phase 6). ◑ PARTIAL 2026-06-07 (auditor magic-link shipped).** Built the **auditor magic-link**: operator mints a revocable, time-bounded `compliance.AuditorAccessGrant` (migration `0016`) → `TimestampSigner` magic link; the **public** `compliance:auditor_inspect` view needs no login (the signed, unexpired, unrevoked grant IS the auth), returns **only** PII-masked projections (`pii_masking.mask_student_for_auditor` → initials, birth-year, guardian withheld), and logs every view to `AuditorAccessLog`; staff console creates/lists/revokes. Tests `test_auditor_access` 6/6; gates register 7/7, migration drift clean, `check`/print/bare-except/**pii-logging** 0. **Honestly NOT closed:** (a) **platform-wide per-action WebAuthn non-repudiation** — heavy crypto/infra; the tamper-evident hash-chain exists only in `migration_cloud`, and a per-action WebAuthn-signed, write-once (S3 Object Lock) platform log is its own focused build; (b) **state-reporting bridges** — foundation exists (`statutory_tenant_extract.py` + `emis/`) but per-jurisdiction format exporters (CALPADS/EdFacts) remain; (c) geo-fencing is captured (IP logged) not yet enforced.

**Wave F — Mesh sync deepening (cross-cutting).** Extend `rmc-lan-mule-sync.js` (LAN hub) toward device-to-device Bluetooth/Wi-Fi-Direct mesh, with a **real** convergence strategy (version vectors / proper CRDT in `sync_engine`), not the pasted last-write-wins merger.
