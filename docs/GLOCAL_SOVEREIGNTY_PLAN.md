...
**Status:** DONE at repository scope (reconciled 2026-06-09). Converts the owner's "RunMyCampus-as-dominant-infrastructure / Cloud-Dependency-Collapse" manifesto into a grounded, phased plan mapped onto the **real** codebase. Every register-tracked lifecycle capability now has a resolving proof; deployment credentials, authority stamping, production load, and physical-device pilots remain external evidence.
...
| Bucket | What | Action |
|---|---|---|
| **Adopt (strategy)** | Local-first sovereignty, the 6-phase lifecycle framing, the regional competitor audit (PART 1), region-aware payments/messaging, ambient capture, vocabulary injection | Use as roadmap + messaging. Repository gaps are closed in §5; deployment evidence remains explicit. |
...
Status legend: **SHIPPED** = a repository mechanism and resolving proof are present · **EXTERNAL** = provider, authority, production, or physical-device evidence is still required. **All statuses below were reconciled on 2026-06-09** against the live code, focused tests, and `feature_gap_register.py`.
...
| Manifesto tool | Real home | Status | Register slug |
|---|---|---|---|
| Dual-identity profile matrix (school + evening coaching) | `apps/student360/dual_identity.py`, `academics.StudentDegreeEnrollment` | SHIPPED — primary school + active program contexts | `dual-identity-profile` |
| Labor-contract / payroll blueprint (offline tax/pension) | `apps/payroll` | SHIPPED — contracts, Decimal tax/contributions, offline capture, disbursement export | (existing payroll) |
| 1-click substitute broker (absence → WhatsApp/SMS broadcast) | `apps/schoolops/substitute_handover.py` + tenant ops surface | SHIPPED — tenant discovery, deterministic ranking, consent-aware broadcast | `substitute-broker` |

### Phase 3 — Financial ledger & marketplace (daily)
| Manifesto tool | Real home | Status | Register slug |
|---|---|---|---|
| Split-wallet checkout (80/10/10 multi-vendor split) | `apps/finance`, `apps/billing` | SHIPPED — Decimal-safe allocation + ledger posting | `split-wallet-checkout` |
...
| Fiscal e-invoice adapters (SAT/SEFAZ stamped invoices) | `apps/finance/fiscal_einvoice.py` | SHIPPED (repo) / EXTERNAL (certificates, signing, authority/provider transport) | `fiscal-einvoice-adapters` |
...
| Plickers-style card-sweep (no-device camera shape-parse) | `static/js/rmc-plickers-card-sweep.js`, marks entry | SHIPPED (repo) / EXTERNAL (camera-browser field pilot) | `plickers-card-sweep` |
| Zero-data homework queue buffer (<500KB cache, gate sync) | `apps/sync_engine`, `offline_mode_bundle.py`, offline queue | SHIPPED — homework action validates and applies through the canonical queue | `zero-data-homework-buffer` |

### Phase 5 — Transit ops & event logistics
| Manifesto tool | Real home | Status | Register slug |
|---|---|---|---|
| Smart fleet route optimizer (offline map files) | `apps/schoolops/route_optimizer.py` | SHIPPED — lat/lng + offline haversine nearest-neighbor kernel | `smart-fleet-route-optimizer` |
...
| Field-trip compliance factory (e-sign + offline medical checklist) | `schoolops/field_trip.py`, `compliance.ConsentRequest`/`ConsentRecord` | SHIPPED — e-sign consent + supervising-teacher medical checklist | `field-trip-compliance` |
...
| Predictive retention / cash-flow analytics | `apps/analytics`, `apps/reports` | SHIPPED — governed at-risk inference, forecasting, BI, and scheduled persistence | (existing analytics) |
...
| GDPR Right-to-be-Forgotten anonymization (preserve aggregates) | `apps/compliance` (`EraseRequest` + `gdpr_services`) | SHIPPED — verified anonymization; literal crypto key shredding is not claimed | `gdpr-key-shredding` |
...
### 4.1 Historical discovery baseline (2026-06-07)

The initial discovery found **6 of 10 candidate gaps with partial primitives** and **4 absent**. This historical baseline explains the wave sequence; it is not the current status table.
...
**Current reconciliation (2026-06-09):** every register row is `shipped` with a route, model, command, or CI proof. The final closure added encrypted offline intake drafts, repaired CSP scheduling, dual-identity projection, substitute discovery/broadcast, fail-closed fiscal adapters, and browser card-sweep proposals.
...
**Wave G — Aggressive final gap closure. ✅ DONE 2026-06-09.** Re-opened every remaining `planned`/`in_progress` register row and stale roadmap claim. Shipped encrypted fail-closed Setup Studio drafts, repaired bounded CSP timetabling, Student 360 dual-identity projection, ranked substitute discovery plus consent-aware WhatsApp/SMS broadcast, deterministic fail-closed CFDI/NF-e adapters, browser QR card-sweep proposals, and persisted "Was this helpful?" telemetry. Reconciled stale roadmap endpoints and lifecycle documentation. Proof: PLATFORM_AUDIT_BUNDLE_PASS (21/21), 51/51 focused Django tests, Django system check, no migration drift, fiscal adapter command, JavaScript syntax/behavior probes, targeted Ruff/compile, and zero open register rows.

Sequence rationale: A is near-free trust (claim what's built), B/C are highest owner-visible value, D/E close the lifecycle bookends, F is the deepest engineering, and G is the final evidence-driven reconciliation.

---

## 6. SOT updates (2026-06-07 through 2026-06-09)
...
4. **2026-06-09 reconciliation** — all lifecycle rows are now `shipped` with resolving proofs; this plan, the timetable guide, the single execution SOT, and the autonomous execution log record the final repository closure and external-evidence boundary.
