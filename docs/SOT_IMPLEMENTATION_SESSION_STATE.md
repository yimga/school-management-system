...
| **Last completed** | **`verify_section10_5_layers.py` PASS** + **`verify_section7_gate.py` PASS** + **`generate_platform_inventory.py --write`** (2026-03-22). **Code:** `GetActiveYearAndTermTests` uses empty school; SQLite **`timeout`** in settings. **Full `pre_deploy_gate`:** re-run after closing locks on `.django_test_dbs/*.sqlite3` (see [TEST_DATABASE.md](TEST_DATABASE.md)); local DBs may need `clean_django_test_dbs.py --all` if migrations error. |
