...
| **Last completed** | **Backlog execution batch (2026-03-22):** Wave 4 **`ops_pos ?export=csv`** (5000 lines) + **`test_pos_export_csv`**; N28 **parent** **`parent_upcoming_events`** on **`parent_dashboard`** (merged deadlines + calendar, link to unified calendar) + tenant **`get_active_year_and_term(school=…)`**; SOT + backlog updated. **Prior:** `pre_deploy_gate` green; `verify_sot_pillar_evidence` OK (98 paths). **Open SOT `[ ]` rows** remain multi-sprint — see [SOT_REMAINING_ITEMS_BACKLOG.md](SOT_REMAINING_ITEMS_BACKLOG.md). |
