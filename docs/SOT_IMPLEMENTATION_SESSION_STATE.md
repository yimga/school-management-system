...
| **Last completed** | **Runbook §6:** `PRE_GATE_FRESH_TEST_DB=1 SKIP_VISUAL_QA=1 bash scripts/pre_deploy_gate.sh` **PASS**; log: `docs/generated/pre_deploy_gate_run.txt`. **`verify_section7_gate.py` PASS** with `PRE_GATE_FRESH_TEST_DB=1 DJANGO_TEST_DB_FILE=.django_test_dbs/section7_verify.sqlite3` (isolated DB avoids Win32 lock). **`verify_sot_pillar_evidence.py` / `verify_section10_5_layers.py`** green. **`pre_deploy_gate.sh`:** if `PRE_GATE_FRESH_TEST_DB=1` cannot remove `pre_deploy_gate.sqlite3`, auto-fallback to `pre_deploy_gate_run.sqlite3`; [TEST_DATABASE.md](TEST_DATABASE.md) updated (migrate duration, timeouts). **`record_pre_deploy_gate_output.sh`:** optional `RECORD_PRE_DEPLOY_GATE_FROM` to copy an existing log without re-running the gate. |
...
| **Date (UTC)** | 2026-03-22 (§7 + gate green) |
