...
That script runs Gunicorn with `config/gunicorn.conf.py`, which sets `bind = "0.0.0.0:{PORT}"` and **`timeout = 120`** (override with env `GUNICORN_TIMEOUT`). If you override the start command in the Render dashboard, use either this script or run Gunicorn with `-c config/gunicorn.conf.py` (or pass `--bind 0.0.0.0:$PORT` explicitly). A bare `.venv/bin/gunicorn config.wsgi:application --bind 0.0.0.0:$PORT` omits that config and falls back to Gunicorn’s **30s** worker timeout and a **single sync worker**, which blocks Render’s **5s** health probe when `/super/` is building a large HTML page (`POST /super/` ~10s in access logs). **`build.sh` installs a wrapper** at `.venv/bin/gunicorn` that always adds `-c config/gunicorn.conf.py` (gthread + 120s timeout) even when the dashboard start command is bare. After deploy, logs should show Gunicorn loading `config/gunicorn.conf.py`; `/health/` may show `"web_start":"gunicorn_conf_wrapper"` when only the wrapper runs, or `"render_start_web"` when `render_start_web.sh` runs. Binding only to `127.0.0.1` or omitting `--bind` will cause “No open HTTP ports detected” and deploy failure.

**Verify in Render logs after deploy:** you must see `[web-start] starting config.wsgi:application via config/gunicorn.conf.py`. If you see only `Running '.venv/bin/gunicorn config.wsgi:application --bind 0.0.0.0:$PORT'`, the dashboard override is still active — fix **Settings → Start Command** to match `render.yaml`.
