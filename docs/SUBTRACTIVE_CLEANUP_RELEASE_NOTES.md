...
| (this release) | `ensure_superadmin` management command | **2026-03-12:** Removed as standalone implementation; **current:** thin alias in `apps/accounts/management/commands/ensure_superadmin.py` that calls `ensure_superuser` with fixed `admin`/`admin` for deploy scripts. Prefer `ensure_superuser` for custom credentials. LEGACY_PATH_INVENTORY §2 **KEEP (thin alias)**; `test_ensure_superadmin_command.py`. |
...
| 2026-03-25 | `super:admin_bridge_*` URL names (8) | **Removed** from `super_urls`; use `super:admin_bridge` + `kwargs`. Legacy **paths** (`…/integrations-marketplace/` etc.) **301** to canonical slug (`super_admin_bridge_legacy_path_redirect`). LEGACY_PATH_INVENTORY §2 row; `test_super_config_migration_urls`. |
