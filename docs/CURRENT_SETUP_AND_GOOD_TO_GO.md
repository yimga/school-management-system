...
Short summary of how multi-tenant admin and the current tenant (Gilead) are set up, and what to verify before go-live.
...
- **Migration 0013** (`apps/schools/migrations/0013_link_default_admin_to_gilead.py`): Links the default platform admin user to the Gilead school via **SchoolMembership** so that user can use the tenant Backend and school picker shows Gilead. No password or tenant data change.

### 2. Main (public) admin

- **URL:** `/admin/` on the **primary domain** (e.g. `https://school-management-system-2kzk.onrender.com/admin/` or `http://localhost:8000/admin/`).
- **Access:** Superuser only (`config/admin.py` → `GileadAdminSite.has_permission`).
...
Use this to confirm the current tenant and access are ready.

- [ ] **Migrations applied**  
  `python manage.py migrate` — includes 0012 (Gilead school), 0021 (admin user), 0013 (admin → Gilead membership).
...
- [ ] **Tenant Backend**  
  Open `/` or `/authentication/login/` on the same host → log in with `admin` (or a user with membership to Gilead). After login you should land on the Backend dashboard for Gilead (single-tenant on main URL).

- [ ] **Gilead unchanged**  
  In main admin, open **Schools**. The Gilead school should have `slug=gilead-school`, `subdomain=gilead-school`. Do not change name, slug, subdomain, or user credentials unless you intend to.
...
| Default admin linked to Gilead | Done: migration 0013. |
