...
**Phase 6 / SiteSettings product-Python surface:** `python scripts/verify_cursor_phase6_siteconfig_sitesettings.py` runs **`audit_sitesettings_python_surface.py`**, which writes **`docs/generated/sitesettings_python_surface_audit.json`** and fails on non-allowlist **`SiteSettings` class access** (direct import, **`SiteSettings.objects`**, **`get_model('siteconfig','SiteSettings')`**, **`SiteSettings.get_solo`**, etc.) outside **`apps/siteconfig/models.py`** and **`apps/platform_runtime/helpers.py`**. Runtime reads: **`apps/platform_runtime.site_settings_read_access`** (re-exports approved **`get_effective_site_settings`**, etc.).
...
| 2026-04-26 | **SOT §11.4 1069–1075 (admin gravity E2E):** **`config_mutation_audit_evidence`** (read-only **`ConfigMutationAuditLog`**) + **`scheduled_reports_delivery_hub`** **operator** **strip**; **`shipped_category_regression_checks`** + **entity** **catalog** **template** **order** under **`audit_admin_gravity` `--strict`**; **roadmap** **ships** for **reports**/**audit** **waves**; **1072/1073/1074** **PARTIAL** per **honest** **audit** (marketplace **template** **hits** **empty**; **workflow** **hits** **ledger**-**only**; **sweep** **not** **exhaustive**). |
| 2026-04-26 | **SOT §11.4 1087 (UI — backend dashboard density):** **`backend-dashboard-tokens.css`** **tightens** **spacing** **tokens**; **no** **layout** **data** **change**; **partial** **until** **CI** **re**-**stamps** **`test_backend_dashboard_shell_render`**. |
| 2026-04-26 | **SOT §11.4 1066 + 1062 evidence (admin gravity depth):** **`siteconfig:metadata_dynamic_fields_operator`** (EAV read-only operator) + **verify** / **shell** / **metadata** **tests**; **`scheduled_reports_delivery_hub`**: **Advanced** **Django** **admin** **after** **operator** **content**; **keyboard** **shortcut** `r` → **`/siteconfig/reports/`**; **`audit_admin_gravity` `--strict`**. *Superseded by **1069–1075** row for operator strip + config mutation + strict regressions.* |
