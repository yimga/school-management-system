...
4. **Sign-in URLs (slug-first):** School owners and staff should sign in on **`https://{slug}.<base>/authentication/login/`** (their workspace). The public host (`runmycampus.com`) is a **discovery fallback**: school picker, `/find/`, `/discover/`, then handoff to the slug URL (session cookies on `.<base>` cross hosts). The manager host login form is **operator-only**: unauthenticated visits to `manager.<base>/authentication/login/` redirect to the public host unless the request carries operator intent (`?next=/super/…`, `?next=/admin/…`, or `?cp=1`). After login, tenant members are routed via `resolve_public_post_login_handoff()` / `build_public_handoff_to_tenant_workspace()` in `apps/schools/tenant_login_redirect.py` — not left on generic public auth. Pending tenants (`is_active=False`) may use `/authentication/*` on their subdomain (`PENDING_TENANT_AUTH_PREFIXES` in `apps/schools/middleware.py`).
...
| School owner / staff (fallback) | `https://runmycampus.com/authentication/login/` → pick school → slug handoff |
