...
- **Platform `/admin/` (manager):** Changelists for `siteconfig` and `global_registries` render a control-plane banner when `is_manager_host` is true (`templates/admin/siteconfig/change_list.html`, `app_index.html`, `templates/admin/global_registries/change_list.html`). See [PLATFORM_ADMIN_TO_SUPER_SYSTEM_CONFIG.md](PLATFORM_ADMIN_TO_SUPER_SYSTEM_CONFIG.md).
