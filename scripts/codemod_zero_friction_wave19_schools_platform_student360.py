#!/usr/bin/env python3
"""Wave 19 — schools + platform_runtime + student360 zone friction burndown."""
...
SKIP_PARTS = ("/email/", "/emails/", "/partials/", "/_v2/")


def _zone_files() -> list[Path]:
    paths: list[Path] = []
    for zone in ("schools", "platform_runtime", "student360"):
        ...
        for path in base.rglob("*.html"):
            ...
            if any(part in rel for part in SKIP_PARTS):
                continue
            if path.name.endswith("_fragment.html"):
                continue
            paths.append(path)
    return sorted(paths)
