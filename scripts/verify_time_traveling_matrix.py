#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for time_traveling_matrix."""
...
OUT_PATH = REPO / "docs" / "generated" / "time_traveling_matrix_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.time_traveling_matrix"
TEST_MODULE = "apps.governance.turbo.tests.test_time_traveling_matrix"
...
def main() -> int:
    ...
    verdict_slug = "TIME_TRAVELING_MATRIX"
    ...
    if failures:
        print(f"verify_time_traveling_matrix: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_time_traveling_matrix: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
