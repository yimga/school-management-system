...
OUT_PATH = REPO / "docs" / "generated" / "customer_comms_templates_audit.json"

BLOCK_KEY = '_template_dir:docs/customer_comms'
...
def main() -> int:
    ...
    verdict_slug = "CUSTOMER_COMMS_TEMPLATES"
    ...
    if failures:
        print(f"verify_customer_comms_templates: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_customer_comms_templates: {verdict} (scope={total})")
    return 0
