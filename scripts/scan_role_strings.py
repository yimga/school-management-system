...
def _compare(findings: list[dict]) -> int:
    ...
    # Compare on (path, role) MULTISETS, not (path, line, role): a role literal
    # that merely shifts line number when unrelated code is added/removed above it
    # is NOT a new hardcoded role. Keying on the line made every wave trip this gate
    # on cosmetic line-drift, turning CI-green into a manual re-baseline treadmill. A
    # genuine regression — a brand-new role literal in a file, or one MORE
    # occurrence of a role in a file — still increments the (path, role) count and
    # trips the gate.
    baseline_counts = Counter(
        (item["path"], item["role"]) for item in baseline.get("findings", [])
    )
    current_counts = Counter((item["path"], item["role"]) for item in findings)
    new = current_counts - baseline_counts  # positive counts: occurrences added
    removed = baseline_counts - current_counts
    # Map (path, role) -> current line numbers so the report still locates sites.
    lines_by_key: dict[tuple, list] = {}
    for item in findings:
        lines_by_key.setdefault((item["path"], item["role"]), []).append(item["line"])
    _print_summary(findings)
    if new:
        print("\nNEW hardcoded role-string literals introduced:")
        for (path, role), count in sorted(new.items()):
            locs = ", ".join(str(n) for n in sorted(lines_by_key.get((path, role), []))[:6])
            print(f"  {path}  \"{role}\"  +{count}  [lines: {locs}]")
    if removed:
        print("\nRemoved (consider updating baseline):")
        for (path, role), count in sorted(removed.items()):
            print(f"  {path}  \"{role}\"  -{count}")
