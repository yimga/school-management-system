#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for w3c_verifiable_credentials."""
...
OUT_PATH = REPO / "docs" / "generated" / "w3c_verifiable_credentials_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.w3c_verifiable_credentials"
TEST_MODULE = "apps.governance.turbo.tests.test_w3c_verifiable_credentials"
...
def main() -> int:
    ...
    verdict_slug = "W3C_VERIFIABLE_CREDENTIALS"
    ...
    if failures:
        print(f"verify_w3c_verifiable_credentials: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_w3c_verifiable_credentials: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
