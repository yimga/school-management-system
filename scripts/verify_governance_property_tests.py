...
OUT_PATH = REPO / "docs" / "generated" / "governance_property_tests_audit.json"

BLOCK_KEY = '_module_exists:apps/governance/tests/test_governance_property_invariants.py'
...
def main() -> int:
    ...
    verdict_slug = "GOVERNANCE_PROPERTY_TESTS"
    ...
    if failures:
        print(f"verify_governance_property_tests: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_governance_property_tests: {verdict} (scope={total})")
    return 0
