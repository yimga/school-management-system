#!/usr/bin/env python3
"""Wave 16 — portal + evals zone friction burndown."""
...
BLOCK_MARKERS = ('{% block content %}', '{% block backend_page %}')

PORTAL_EVALS_FILES = sorted(
    list((ROOT / "templates" / "evals").glob("*.html"))
    + list((ROOT / "templates" / "portal").rglob("*.html"))
)
...
def main() -> int:
    changed: list[str] = []
    for path in PORTAL_EVALS_FILES:
        if patch_file(path):
