#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for adversarial_redteam."""
...
OUT_PATH = REPO / "docs" / "generated" / "adversarial_redteam_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.adversarial_redteam"
TEST_MODULE = "apps.governance.turbo.tests.test_adversarial_redteam"
...
def main() -> int:
    ...
    verdict_slug = "ADVERSARIAL_REDTEAM"
    ...
    if failures:
        print(f"verify_adversarial_redteam: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_adversarial_redteam: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
