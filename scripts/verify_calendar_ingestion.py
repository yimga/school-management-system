...
OUT_PATH = REPO / "docs" / "generated" / "calendar_ingestion_audit.json"

BLOCK_KEY = 'calendar_sources'
...
def main() -> int:
    ...
    verdict_slug = "CALENDAR_INGESTION"
    ...
    if failures:
        print(f"verify_calendar_ingestion: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_calendar_ingestion: {verdict} (scope={total})")
    return 0
