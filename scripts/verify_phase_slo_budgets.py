...
OUT_PATH = REPO / "docs" / "generated" / "phase_slo_budgets_audit.json"

BLOCK_KEY = '_doc_exists:docs/PHASE_SLO_BUDGETS.md'
...
def main() -> int:
    ...
    verdict_slug = "PHASE_SLO_BUDGETS"
    ...
    if failures:
        print(f"verify_phase_slo_budgets: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_phase_slo_budgets: {verdict} (scope={total})")
    return 0
