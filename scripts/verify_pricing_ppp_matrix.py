...
OUT_PATH = REPO / "docs" / "generated" / "pricing_ppp_matrix_audit.json"

BLOCK_KEY = 'pricing_band'
SUBKEYS: tuple[str, ...] = ('ppp_band', 'fx_hedge_band_pct', 'local_currency_required')
...
def main() -> int:
    ...
    verdict_slug = "PRICING_PPP_MATRIX"
    ...
    if failures:
        print(f"verify_pricing_ppp_matrix: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_pricing_ppp_matrix: {verdict} (scope={total})")
    return 0
