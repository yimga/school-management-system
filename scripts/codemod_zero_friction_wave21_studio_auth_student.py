#!/usr/bin/env python3
"""Wave 21 — studio_os + auth + student zone friction burndown."""
...
BLOCK_MARKERS = (
    ...
    "{% block cockpit_canvas_body %}",
)

SKIP_PARTS = ("/email/", "/emails/", "/partials/", "/components/")


def _zone_files() -> list[Path]:
    paths: list[Path] = []
    for zone in ("studio_os", "auth", "student"):
