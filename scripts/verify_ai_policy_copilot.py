#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for ai_policy_copilot."""
...
OUT_PATH = REPO / "docs" / "generated" / "ai_policy_copilot_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.ai_policy_copilot"
TEST_MODULE = "apps.governance.turbo.tests.test_ai_policy_copilot"
...
def main() -> int:
    ...
    verdict_slug = "AI_POLICY_COPILOT"
    ...
    if failures:
        print(f"verify_ai_policy_copilot: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_ai_policy_copilot: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
