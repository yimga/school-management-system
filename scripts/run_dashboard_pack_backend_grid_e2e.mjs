#!/usr/bin/env node
/**
 * Boot demo-school on 8013 and run dashboard-pack-backend-grid Playwright spec.
 */
...
async function waitForServer(maxSeconds = 180) {
  ...
  console.error(`run_dashboard_pack_backend_grid_e2e: server not ready at ${loginUrl}`);
  process.exit(1);
}
...
console.log('=== dashboard pack e2e: migrate ===');
runSync(['manage.py', 'migrate', '--noinput'], baseEnv);

console.log('=== dashboard pack e2e: seed demo-school ===');
runSync(
  [
    'manage.py',
    'seed_demo_tenant_users',
    `--school-slug=${slug}`,
    `--password=${baseEnv.E2E_TENANT_PASSWORD}`,
  ],
  baseEnv,
);

console.log('=== dashboard pack e2e: seed TOTP ===');
runSync(
  [
    '-c',
    `
...
device.save()
print(f'Seeded TOTP for {username}')
`.trim(),
  ],
  baseEnv,
);

console.log(`=== dashboard pack e2e: runserver 127.0.0.1:${port} ===`);
const serverLog = path.join(repo, 'artifacts', 'dashboard-pack-e2e', 'runserver.log');
...
console.log('=== dashboard pack e2e: Playwright ===');
...
const pw = spawnSync(
  ...
  [
    pwCli,
    'test',
    'tests/e2e/dashboard-pack-backend-grid.spec.js',
  ...
  ],
  {
    ...
    env: {
      ...process.env,
      ...baseEnv,
      PLAYWRIGHT_TENANT_BASE_URL: `http://demo-school.runmycampus.com:${port}`,
      TENANT_E2E_BASE_URL: `http://demo-school.runmycampus.com:${port}`,
...
if (pw.status === 0) {
  console.log('DASHBOARD_PACK_BACKEND_GRID_E2E_PASS');
}
