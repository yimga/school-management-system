#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for federated_emis_aggregator."""
...
OUT_PATH = REPO / "docs" / "generated" / "federated_emis_aggregator_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.federated_emis_aggregator"
TEST_MODULE = "apps.governance.turbo.tests.test_federated_emis_aggregator"
...
def main() -> int:
    ...
    verdict_slug = "FEDERATED_EMIS_AGGREGATOR"
    ...
    if failures:
        print(f"verify_federated_emis_aggregator: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_federated_emis_aggregator: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
