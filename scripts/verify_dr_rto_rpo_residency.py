...
OUT_PATH = REPO / "docs" / "generated" / "dr_rto_rpo_residency_audit.json"

BLOCK_KEY = 'dr_rto_rpo'
SUBKEYS: tuple[str, ...] = ('rto_minutes', 'rpo_minutes', 'cross_region_failover_allowed', 'counsel_signed_continuity_table', 'residency_label')
...
def main() -> int:
    ...
    verdict_slug = "DR_RTO_RPO_RESIDENCY"
    ...
    if failures:
        print(f"verify_dr_rto_rpo_residency: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_dr_rto_rpo_residency: {verdict} (scope={total})")
    return 0
