...
OUT_PATH = REPO / "docs" / "generated" / "labor_law_matrix_audit.json"

BLOCK_KEY = 'labor_law'
SUBKEYS: tuple[str, ...] = ('employer_of_record_options', 'payroll_authority', 'working_time_rule', 'severance_formula', 'collective_bargaining_presence', 'cross_school_employment_allowed', 'non_compete_enforceable')
...
def main() -> int:
    ...
    if failures:
        print(f"verify_labor_law_matrix: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_labor_law_matrix: {verdict} (scope={total})")
    return 0
