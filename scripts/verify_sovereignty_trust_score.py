#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for sovereignty_trust_score."""
...
OUT_PATH = REPO / "docs" / "generated" / "sovereignty_trust_score_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.sovereignty_trust_score"
TEST_MODULE = "apps.governance.turbo.tests.test_sovereignty_trust_score"
...
def main() -> int:
    ...
    verdict_slug = "SOVEREIGNTY_TRUST_SCORE"
    ...
    if failures:
        print(f"verify_sovereignty_trust_score: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_sovereignty_trust_score: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
