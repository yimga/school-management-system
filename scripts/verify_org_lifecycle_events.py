...
OUT_PATH = REPO / "docs" / "generated" / "org_lifecycle_events_audit.json"

BLOCK_KEY = '_lifecycle_doc'
...
def main() -> int:
    ...
    verdict_slug = "ORG_LIFECYCLE_EVENTS"
    ...
    if failures:
        print(f"verify_org_lifecycle_events: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_org_lifecycle_events: {verdict} (scope={total})")
    return 0
