#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for multimodal_terminology."""
...
OUT_PATH = REPO / "docs" / "generated" / "multimodal_terminology_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.multimodal_terminology"
TEST_MODULE = "apps.governance.turbo.tests.test_multimodal_terminology"
...
def main() -> int:
    ...
    verdict_slug = "MULTIMODAL_TERMINOLOGY"
    ...
    if failures:
        print(f"verify_multimodal_terminology: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_multimodal_terminology: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
