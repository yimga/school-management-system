...
def validate_template_links():
    ...
    valid_prefixes = {
        '/admin/', '/authentication/', '/api/', '/portal/', '/evals/', '/siteconfig/',
        '/reports/', '/analytics/', '/finance/', '/payroll/', '/compliance/', '/kb/', '/',
    }
    if template_dir.exists():
        for html_file in template_dir.rglob('*.html'):
            ...
            for link in links:
                if '{{' in link or '{%' in link or '<' in link or link.startswith('http') or not link or link.startswith('#') or link.startswith('?') or link.lower().startswith('javascript:') or link.lower().startswith(('mailto:', 'tel:', 'data:')):
                    continue
...
def main():
    ...
    print("=" * 80)
    critical_routes = [
        ('/', 'home'), ('/authentication/login/', 'login'), ('/admin/', 'backend admin'),
        ('/portal/parent/', 'parent dashboard'), ('/evals/teacher/', 'teacher dashboard'),
        ('/backend/', 'frontend admin redirect'), ('/authentication/backend/', 'frontend admin dashboard'),
