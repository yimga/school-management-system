#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for cross_vertical_kernel."""
...
OUT_PATH = REPO / "docs" / "generated" / "cross_vertical_kernel_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.cross_vertical_kernel"
TEST_MODULE = "apps.governance.turbo.tests.test_cross_vertical_kernel"
...
def main() -> int:
    ...
    verdict_slug = "CROSS_VERTICAL_KERNEL"
    ...
    if failures:
        print(f"verify_cross_vertical_kernel: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_cross_vertical_kernel: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
