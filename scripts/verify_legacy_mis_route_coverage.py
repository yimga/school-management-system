...
OUT_PATH = REPO / "docs" / "generated" / "legacy_mis_route_coverage_audit.json"

BLOCK_KEY = 'legacy_mis_incumbents'
...
def main() -> int:
    ...
    verdict_slug = "LEGACY_MIS_ROUTE_COVERAGE"
    ...
    if failures:
        print(f"verify_legacy_mis_route_coverage: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_legacy_mis_route_coverage: {verdict} (scope={total})")
    return 0
