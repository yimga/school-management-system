#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for cross_org_marketplace."""
...
OUT_PATH = REPO / "docs" / "generated" / "cross_org_marketplace_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.cross_org_marketplace"
TEST_MODULE = "apps.governance.turbo.tests.test_cross_org_marketplace"
...
def main() -> int:
    ...
    verdict_slug = "CROSS_ORG_MARKETPLACE"
    ...
    if failures:
        print(f"verify_cross_org_marketplace: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_cross_org_marketplace: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
