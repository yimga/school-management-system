...
OUT_PATH = REPO / "docs" / "generated" / "rollback_runbooks_audit.json"

BLOCK_KEY = '_rollback_runbooks'
...
def main() -> int:
    ...
    verdict_slug = "ROLLBACK_RUNBOOKS"
    ...
    if failures:
        print(f"verify_rollback_runbooks: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_rollback_runbooks: {verdict} (scope={total})")
    return 0
