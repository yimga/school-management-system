...
OUT_PATH = REPO / "docs" / "generated" / "exam_board_coverage_audit.json"

BLOCK_KEY = 'exam_boards'
...
def main() -> int:
    ...
    verdict_slug = "EXAM_BOARD_COVERAGE"
    ...
    if failures:
        print(f"verify_exam_board_coverage: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_exam_board_coverage: {verdict} (scope={total})")
    return 0
