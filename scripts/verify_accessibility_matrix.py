...
OUT_PATH = REPO / "docs" / "generated" / "accessibility_matrix_audit.json"

BLOCK_KEY = 'regulatory_matrix.accessibility_statute'
SUBKEYS: tuple[str, ...] = ('platform_baseline', 'local_statutes', 'sign_languages')
...
def main() -> int:
    ...
    verdict_slug = "ACCESSIBILITY_MATRIX"
    ...
    if failures:
        print(f"verify_accessibility_matrix: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_accessibility_matrix: {verdict} (scope={total})")
    return 0
