...
OUT_PATH = REPO / "docs" / "generated" / "national_sso_brokers_audit.json"

BLOCK_KEY = 'national_identity_brokers'
...
def main() -> int:
    ...
    verdict_slug = "NATIONAL_SSO_BROKERS"
    ...
    if failures:
        print(f"verify_national_sso_brokers: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_national_sso_brokers: {verdict} (scope={total})")
    return 0
