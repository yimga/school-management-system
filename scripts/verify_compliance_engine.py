#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for realtime_compliance_engine."""
...
OUT_PATH = REPO / "docs" / "generated" / "compliance_engine_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.realtime_compliance_engine"
TEST_MODULE = "apps.governance.turbo.tests.test_realtime_compliance_engine"
...
def main() -> int:
    ...
    verdict_slug = "COMPLIANCE_ENGINE"
    ...
    if failures:
        print(f"verify_compliance_engine: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_compliance_engine: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
