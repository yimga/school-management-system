#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for living_competitor_tracker."""
...
OUT_PATH = REPO / "docs" / "generated" / "competitor_tracker_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.living_competitor_tracker"
TEST_MODULE = "apps.governance.turbo.tests.test_living_competitor_tracker"
...
def main() -> int:
    ...
    verdict_slug = "COMPETITOR_TRACKER"
    ...
    if failures:
        print(f"verify_competitor_tracker: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_competitor_tracker: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
