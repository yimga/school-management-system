...
OUT_PATH = REPO / "docs" / "generated" / "minority_language_rights_audit.json"

BLOCK_KEY = 'regional_languages'
...
def main() -> int:
    ...
    verdict_slug = "MINORITY_LANGUAGE_RIGHTS"
    ...
    if failures:
        print(f"verify_minority_language_rights: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:20]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_minority_language_rights: {verdict} (scope={total})")
    return 0
