#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for formal_verification_tla."""
...
OUT_PATH = REPO / "docs" / "generated" / "tla_specs_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.formal_verification_tla"
TEST_MODULE = "apps.governance.turbo.tests.test_formal_verification_tla"
...
def main() -> int:
    ...
    if failures:
        print(f"verify_tla_specs: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_tla_specs: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
