#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for regulator_api_federation."""
...
OUT_PATH = REPO / "docs" / "generated" / "regulator_api_federation_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.regulator_api_federation"
TEST_MODULE = "apps.governance.turbo.tests.test_regulator_api_federation"
...
def main() -> int:
    ...
    verdict_slug = "REGULATOR_API_FEDERATION"
    ...
    if failures:
        print(f"verify_regulator_api_federation: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_regulator_api_federation: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
