#!/usr/bin/env python3
"""Phase 6 turbo verifier: runtime + tests gate for agentic_self_healing_matrix."""
...
OUT_PATH = REPO / "docs" / "generated" / "matrix_freshness_audit.json"

CONTRACT_MODULE = "apps.governance.turbo.agentic_self_healing_matrix"
TEST_MODULE = "apps.governance.turbo.tests.test_agentic_self_healing_matrix"
...
def main() -> int:
    ...
    if failures:
        print(f"verify_matrix_freshness: {verdict} ({len(failures)})", file=sys.stderr)
        for line in failures[:10]:
            print(f"  - {line}", file=sys.stderr)
        return 1
    print(f"verify_matrix_freshness: {verdict} (runtime healthy, {tests_run} tests passed)")
    return 0
