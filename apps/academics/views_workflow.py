...
class WorkflowWizardView(View):
    ...
    def get_config(self, workflow_key):
        return get_object_or_404(WorkflowConfig, workflow_key=workflow_key, is_active=True)

    def get(self, request, workflow_key):
        config = self.get_config(workflow_key)
        steps = config.steps or []
        if not steps:
            return render(request, "academics/workflow_empty.html", {"workflow_key": workflow_key})
    ...
    def post(self, request, workflow_key):
        ...
        if next_index >= len(steps):
            return render(request, "academics/workflow_done.html", {"workflow_key": workflow_key, "workflow_config": config})
        from django.shortcuts import redirect
        from django.urls import reverse
        url = reverse("academics:workflow_wizard", kwargs={"workflow_key": workflow_key}) + f"?step={next_index}"
        return redirect(url)
