...
def compute_ca_marks_for_candidate(
    ...
    use_practical_split = ca_config.get(
        "use_practical_split", False
    )  # Technical schools
    ca_components = ca_config.get(
        "ca_components", ["seq1", "seq2"]
    )  # Default: seq1 + seq2
    ...
    for eval_obj in evaluations:
        ...
        if seq1_val > 0 or seq2_val > 0:
            ...
            if total_weight > 0:
                ca_score = (
                    seq1_val * weight_seq1 + seq2_val * weight_seq2
                ) / total_weight
            else:
                ca_score = (
                    (seq1_val + seq2_val) / 2 if (seq1_val + seq2_val) > 0 else None
                )
    ...
    # Calculate totals
    total_ca = (
        float(total_weighted_sum / total_coefficient_sum)
        if total_coefficient_sum > 0
        else None
    )
    general_subtotal = (
        float(general_weighted_sum / general_coefficient_sum)
        if general_coefficient_sum > 0
        else None
    )
    technical_subtotal = (
        float(technical_weighted_sum / technical_coefficient_sum)
        if technical_coefficient_sum > 0
        else None
    )
...
def get_document_checklist_summary(candidate: CertificationCandidate) -> Dict[str, any]:
    ...
    if not checklist:
        if session.preset:
            checklist = session.preset.document_checklists.filter(
    ...
    completed_items = sum(
        1 for s in statuses if s.status in ["RECEIVED", "VERIFIED", "WAIVED"]
    )
    missing_items = [
        {
            "code": item.code,
