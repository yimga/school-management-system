...
class GradeViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if role == User.Role.TEACHER:
            ...
            return Grade.objects.filter(scope).select_related(
        ...
        student_profile = StudentProfile.objects.filter(user=user).first()
        if student_profile:
            return Grade.objects.filter(student=student_profile).select_related(
                "student__user", "subject"
            )
        ...
        if role == User.Role.PARENT:
            ...
            return Grade.objects.filter(student_id__in=child_ids).select_related(
                "student__user", "subject"
            )
    ...
    def create(self, request, *args, **kwargs):
        ...
        if user_role == User.Role.TEACHER and not request.user.is_staff:
            ...
            if not can_edit_student_grades(
                request.user, student_obj.id, subject_obj.id
            ):
                return Response(
    ...
    def class_performance(self, request):
        ...
        top_performers = (
            grades.values("student__user__first_name", "student__user__last_name")
    ...
    def get(self, request):
        ...
        if user.is_staff or role in admin_roles:
            ...
            for classroom_id, subject_id_scope in assignments:
                scope |= Q(
                    student__classroom_id=classroom_id, subject_id=subject_id_scope
                )
        ...
        agg = queryset.aggregate(
            ...
            r2=Count(
                Case(
                    When(score__gte=50, score__lt=70, then=1),
                    output_field=IntegerField(),
                )
            ),
            r3=Count(
                Case(
                    When(score__gte=70, score__lt=85, then=1),
                    output_field=IntegerField(),
                )
            ),
        ...
        return Response(
            {
                ...
                "filters": {"subject_id": subject_id, "term": term},
            }
        )
