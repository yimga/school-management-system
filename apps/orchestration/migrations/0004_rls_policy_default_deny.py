# RLS default-deny for tenant-scoped orchestration tables.
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in TABLES:
            short = table.replace("orchestration_", "", 1)
            policy_name = f"orchestration_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("orchestration", "0003_enable_rls_postgresql"),
    ]
