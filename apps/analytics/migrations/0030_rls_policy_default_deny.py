# RLS default-deny for tenant-scoped analytics tables.
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in TABLES:
            short = table.replace("analytics_", "", 1)
            policy_name = f"analytics_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("analytics", "0029_enable_rls_postgresql"),
    ]
