...
class FeeDefaultPredictor:
    ...
    def extract_features(student) -> np.ndarray:
        ...
        total_outstanding = (
            invoices.filter(status="PENDING").aggregate(total=models.Sum("amount"))[
                "total"
            ]
            or 0
        )
        ...
        late_payments = payments.filter(
            status="COMPLETED", paid_at__gt=models.F("invoice__due_date")
        ).count()
        ...
        on_time_payments = payments.filter(
            status="COMPLETED", paid_at__lte=models.F("invoice__due_date")
        ).count()
        consistency_score = (
            (on_time_payments / total_invoices) if total_invoices > 0 else 1.0
        )
...
class PerformanceForecaster:
    ...
    def extract_features(student) -> np.ndarray:
        ...
        _trends = (
            AdvancedAnalyticsService.get_performance_trends(student_profile, days=270)
            if student_profile
            else []
        )  # ~3 terms
        ...
        attendance_rate = (
            getattr(student_profile, "attendance_rate", 90.0)
            if student_profile
            else 90.0
        )
        ...
        is_at_risk = (
            1.0 if (student_profile and student_profile in at_risk_students) else 0.0
        )
    ...
    def extract_features(student) -> np.ndarray:
        ...
        try:
            ...
            log_exception_with_context(
                ...
                school_id=None,
                extra={
                    "context": "extract_features.attendance",
...
class MLPredictionService:
    ...
    def get_high_risk_students(self, threshold: float = 0.7) -> List:
        ...
        for student in Student.objects.filter(is_active=True):
            ...
            if (
                report["fee_default"]["probability"] > threshold
                or report["churn"]["probability"] > threshold
                or report["performance"]["predicted_score"] < 40
            ):
                high_risk.append(
                    {
                        "student": student,
