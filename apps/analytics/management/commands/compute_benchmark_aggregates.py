...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--region", type=str, default="", help="Limit to region code (e.g. CMR).")
        parser.add_argument("--dry-run", action="store_true", help="Only print, do not save.")
    ...
    def handle(self, *args, **options):
        ...
        for school in schools:
            region_code = getattr(school.default_region, "code", None) if school.default_region else "GLOBAL"
            ...
            for row in evals:
                ...
                if avg_score is None or n == 0:
                    continue
                if dry_run:
                    self.stdout.write(f"Would create: {region_code}/{sub_system} subject={subject_id} term={term_id} average_score={avg_score} n={n}")
                    continue
                ...
                if created_flag:
                    created += 1
        if not dry_run:
            self.stdout.write(self.style.SUCCESS(f"Benchmark aggregates updated. New rows: {created}."))
        else:
