# RLS default-deny for tenant-scoped apicenter tables.
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in TABLES:
            short = table.replace("apicenter_", "", 1)
            policy_name = f"apicenter_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("apicenter", "0010_enable_rls_postgresql"),
    ]
