"""Global-local gap closure (batch 1488) — contract pins for apicenter.
...
class AiContextTenantSafetyTest(SimpleTestCase):
...
class AiInventoryRedactionTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
        artifact = _artifact("ai_safety_gap_closure.json")
        self.assertTrue(artifact.is_file(), "missing audit artifact: " + str(artifact))
...
class AiMissingContextFallbacksTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
        artifact = _artifact("ai_safety_gap_closure.json")
        self.assertTrue(artifact.is_file(), "missing audit artifact: " + str(artifact))
        key = ROOT / "services/ai_helpers.py"
...
class AiTenantContextBoundaryTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
