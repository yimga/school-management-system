...
def tenant_task(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if use_tenants:
            if not schema_name:
                raise ValueError("schema_name required when USE_DJANGO_TENANTS=True for tenant_task")
            try:
        ...
        if school_id is None:
            raise ValueError("school_id required in shared schema (RLS) mode for tenant_task")
        with transaction.atomic():
            if connection.vendor == "postgresql":
                with connection.cursor() as cur:
                    cur.execute("SET LOCAL app.current_school_id = %s", [str(school_id)])
            return fn(*args, **kwargs)
