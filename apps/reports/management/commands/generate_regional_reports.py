...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options.get("region"):
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(f"Region not found: {options['region']}")
                )
                return
        ...
        for student in students:
            try:
                report_data = self._build_report_data(
                    student, academic_year, term, language, region
                )
                ...
                self.stdout.write(
                    self.style.ERROR(f"[!] Error for {student}: {str(e)}")
                )
    ...
    def _build_report_data(self, student, academic_year, term, language, region):
        ...
        return {
            ...
            "class": student.current_classroom.name
    ...
    def _send_report_email(self, student, report_data, language):
        ...
        logo_file = (
            site.get_theme_background("logo")
            if hasattr(site, "get_theme_background")
            else None
        )
        if not logo_file or not getattr(logo_file, "url", None):
            logo_file = (
                getattr(site, "logo", None)
                if getattr(site, "logo", None) and getattr(site.logo, "name", None)
                else None
            )
        ...
        context = {
            ...
            "report_url": report_data.get(
                "report_url", "https://schoolmanagement.local/reports/view/"
            ),
            "school_name": getattr(site, "site_name", None)
