    ...
    for ev in qs.order_by("student__student_code", "subject_assignment__subject__name"):
        ...
        writer.writerow(
            [
                ...
                str(ev.total_score)
                if getattr(ev, "total_score", None) is not None
                else "",
                (ev.remarks or "")[:200],
            ]
        )
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--school", type=str, default=None, help="Optional: school UUID"
        )
        parser.add_argument(
            "--classroom", type=int, default=None, help="Optional: classroom ID"
        )
        parser.add_argument(
            "--output",
            type=str,
