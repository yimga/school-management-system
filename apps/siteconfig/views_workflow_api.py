...
def workflow_list_api(request):
    ...
    assignments = TenantWorkflow.objects.filter(
...
def workflow_run_api(request):
    ...
    if tw_id is None:
        return JsonResponse(
            {"ok": False, "error": "tenant_workflow_id required"}, status=400
        )
    try:
        tw = TenantWorkflow.objects.get(id=tw_id, school=school, is_active=True)
    except TenantWorkflow.DoesNotExist:
        return JsonResponse(
            {"ok": False, "error": "Workflow not found or inactive"}, status=404
        )
...
def workflow_preview_api(request):
    ...
    out = {
        "effective_dsl": None,
        "template": None,
        "staging": bool(request.GET.get("staging")),
    }
    if tw_id:
        try:
            tw = (
                TenantWorkflow.objects.filter(school=school)
                .select_related("template")
                .get(id=tw_id)
            )
        ...
        out["template"] = {
            "id": tw.template_id,
            "code": tw.template.code,
            "name": tw.template.name,
        }
    ...
    if template_id:
        ...
        out["template"] = {
            ...
            "code": t.code,
            "name": t.name,
            "certified": getattr(t, "certified", False),
        }
        return JsonResponse(out)
    return JsonResponse(
        {"error": "template_id or tenant_workflow_id required"}, status=400
    )
...
def dashboard_registry_api(request):
    ...
    school = getattr(request, "school", None)
    role = (
        request.GET.get("role") or getattr(request.user, "role", None) or ""
    ).strip() or None
    ...
    if runtime is not None and getattr(runtime, "_school", None):
        dash = runtime.dashboard_for(
            role=role, user=request.user, page=page or "backend", include_registry=True
        )
    else:
