...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✓ Validation passed for {len(regions_to_import)} region(s)\n"
                )
            )
    ...
    def _validate_import_data(self, regions):
        ...
        for i, region in enumerate(regions):
            ...
            if month and not (1 <= month <= 12):
                errors.append(
                    f"Region {i + 1}: Invalid academic_year_start_month ({month})"
                )
    ...
    def _import_regions(self, regions, options):
        ...
        for region_data in regions:
            ...
            region_obj = RegionConfig.objects.create(
                ...
                academic_year_start_month=region_data.get(
                    "academic_year_start_month", 9
                ),
                term_count_per_year=region_data.get("term_count_per_year", 3),
                enable_online_admissions=region_data.get(
