...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for region in regions:
            ...
            if issues:
                ...
                self.stdout.write(self.style.SUCCESS(f"✓ {region.code} - {region.name} (All checks passed)"))
        ...
        self.stdout.write('\n' + '=' * 60)
        if total_issues == 0:
            self.stdout.write(self.style.SUCCESS(f"✓ All {regions.count()} region(s) validated successfully!"))
        else:
            self.stdout.write(self.style.ERROR(f"✗ Found {total_issues} issue(s) across {regions.count()} region(s)"))
    ...
    def _validate_region(self, region):
        ...
        if region.academic_year_start_month < 1 or region.academic_year_start_month > 12:
            issues.append({
    ...
    def _generate_report(self, validation_report):
        ...
        for item in validation_report:
            ...
            self.stdout.write(f"Year Start:      Month {region.academic_year_start_month}")
            ...
            self.stdout.write(f"Portals:         {', '.join(portals) if portals else 'None'}")
