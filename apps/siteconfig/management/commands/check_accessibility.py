...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Accessibility Compliance Check (WCAG 2.2)"))
        self.stdout.write(self.style.SUCCESS(f"Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"))
        ...
        for page_name, result in results.items():
            status = self.style.SUCCESS("✓ PASS") if not result['issues'] else self.style.ERROR("✗ FAIL")
            ...
            if result['issues']:
                ...
                if len(result['issues']) > 3:
                    self.stdout.write(f"  ... and {len(result['issues']) - 3} more issues")
    ...
    def _check_page(self, client, path, page_name):
        ...
        try:
            ...
            if response.status_code != 200:
                self.stdout.write(self.style.WARNING(f"  ⚠ {page_name}: HTTP {response.status_code}"))
            ...
            status_text = self.style.SUCCESS("PASS") if not issues else self.style.ERROR(f"FAIL ({len(issues)} issues)")
...
"""
        ...
        for page_name, result in results.items():
            ...
            status_text = 'PASS' if not result['issues'] else f'FAIL ({len(result["issues"])} issues)'
