    ...
    def show_status(self, language=None):
        ...
        for lang_code in languages:
            ...
            if file_path.exists():
                ...
                self.stdout.write(f"[+] {lang_code:6} ({lang_name:15}): {count:4} strings, {size:8} bytes")
            else:
                lang_name = SUPPORTED_LANGUAGES.get(lang_code, lang_code)
                self.stdout.write(self.style.WARNING(f"[!] {lang_code:6} ({lang_name:15}): Not initialized"))
        ...
        try:
            ...
            for lang_code, translations in import_data.get('languages', {}).items():
                ...
                if lang_code not in SUPPORTED_LANGUAGES:
                    self.stdout.write(self.style.WARNING(f'Skipping unsupported language: {lang_code}'))
                    continue
                
                TranslationManager.bulk_import(lang_code, translations)
                count += 1
                self.stdout.write(f"  [+] Imported {len(translations)} strings for {lang_code}")
