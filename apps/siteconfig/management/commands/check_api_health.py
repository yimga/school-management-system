...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if sms_api and sms_token:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.stdout.write(self.style.ERROR(f"  ✗ SMS API: UNHEALTHY (HTTP {response.status_code})"))
        ...
        if whatsapp_api and whatsapp_token:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.stdout.write(self.style.ERROR(f"  ✗ WhatsApp API: UNHEALTHY (HTTP {response.status_code})"))
                    results.append(("WhatsApp API", "UNHEALTHY", response.status_code))
                    all_healthy = False
            except requests.exceptions.RequestException as e:
                self.stdout.write(self.style.ERROR(f"  ✗ WhatsApp API: ERROR ({str(e)})"))
        ...
        if slack_webhook:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.stdout.write(self.style.ERROR(f"  ✗ Slack Webhook: UNHEALTHY (HTTP {response.status_code})"))
                    results.append(("Slack Webhook", "UNHEALTHY", response.status_code))
                    all_healthy = False
            except requests.exceptions.RequestException as e:
                self.stdout.write(self.style.ERROR(f"  ✗ Slack Webhook: ERROR ({str(e)})"))
        ...
        if all_healthy:
            ...
            self.stdout.write(self.style.ERROR("✗ Some APIs are unhealthy or unreachable"))
