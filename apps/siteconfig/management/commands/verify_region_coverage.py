...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        missing = [
            c for c in countries if (c.get("code") or "").upper() not in existing_codes
        ]
        covered = len(countries) - len(missing)

        self.stdout.write(
            "RegionConfig coverage: %s / %s countries" % (covered, len(countries))
        )
        if missing:
            self.stdout.write(
                self.style.WARNING(
                    "Missing regions (%s): %s"
                    % (len(missing), [m.get("code") for m in missing[:20]])
                )
            )
            if len(missing) > 20:
            ...
            self.stdout.write(
                self.style.SUCCESS("All catalog countries have a RegionConfig.")
            )
        ...
        try:
            ...
            self.stdout.write(
                "Provinces/States in catalog: %s (optional; seed per deployment)."
                % provinces
            )
