...
from apps.siteconfig.admin import (
    RegionConfigAdmin,
        ...
        for i, scale_type in enumerate(["0-20", "0-100", "0-10", "a-f", "gpa"]):
            GradingScaleConfig.objects.create(
                ...
                if scale_type == "0-20"
                    ...
                    100
                    if scale_type == "0-100"
                    else (10 if scale_type == "0-10" else 5)
                ),
...
class ManagementCommandTestCase(TestCase):
    ...
    def test_export_config_command(self):
        ...
        try:
            call_command(
                "export_config", "--format", "json", "--output", test_file, stdout=out
            )
    ...
    def test_import_config_command(self):
        ...
        out = StringIO()
        call_command(
            "export_config", "--format", "json", "--output", export_file, stdout=out
        )
