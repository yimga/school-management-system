# RLS default-deny for tenant-scoped feedback tables.
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in TABLES:
            short = table.replace("feedback_", "", 1)
            policy_name = f"feedback_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("feedback", "0008_enable_rls_postgresql"),
    ]
