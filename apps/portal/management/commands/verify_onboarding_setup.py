    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if hasattr(SiteSettings, 'admission_number_mode'):
                self.stdout.write(self.style.SUCCESS('  ✓ admission_number_mode field exists'))
            else:
                issues.append('SiteSettings missing admission_number_mode field')
                self.stdout.write(self.style.ERROR('  ✗ admission_number_mode field missing'))
            
            if hasattr(SiteSettings, 'admission_number_pattern'):
                self.stdout.write(self.style.SUCCESS('  ✓ admission_number_pattern field exists'))
            else:
        ...
        try:
            ...
            if 'admission_number_mode' in save_source:
                self.stdout.write(self.style.SUCCESS('  ✓ StudentProfile.save() includes admission number logic'))
            else:
        ...
        try:
            ...
            if hasattr(views, 'link_child_wizard'):
                self.stdout.write(self.style.SUCCESS('  ✓ link_child_wizard view exists'))
            else:
        ...
        if wizard_template.exists():
            self.stdout.write(self.style.SUCCESS('  ✓ link_child_wizard.html template exists'))
        else:
            issues.append('link_child_wizard.html template missing')
            self.stdout.write(self.style.ERROR('  ✗ link_child_wizard.html template missing'))
        ...
        if dashboard_template.exists():
            ...
            if 'onboarding' in dashboard_content or 'Get Started' in dashboard_content:
                self.stdout.write(self.style.SUCCESS('  ✓ dashboard.html includes onboarding content'))
            else:
                warnings.append('dashboard.html may not have onboarding content')
                self.stdout.write(self.style.WARNING('  ⚠ dashboard.html may not have onboarding content'))
        else:
        ...
        try:
            ...
            try:
                ...
                self.stdout.write(self.style.SUCCESS(f'  ✓ link_child URL configured: {url}'))
        ...
        if hasattr(settings, 'SESSION_SAVE_EVERY_REQUEST') and settings.SESSION_SAVE_EVERY_REQUEST:
            self.stdout.write(self.style.SUCCESS('  ✓ SESSION_SAVE_EVERY_REQUEST is enabled'))
        else:
            warnings.append('SESSION_SAVE_EVERY_REQUEST not enabled (wizard may not persist data)')
            self.stdout.write(self.style.WARNING('  ⚠ SESSION_SAVE_EVERY_REQUEST not enabled'))
        ...
        if not issues and not warnings:
            ...
            if issues:
                ...
                self.stdout.write('\n' + self.style.SUCCESS('Warnings only - you can proceed with testing, but review warnings.'))
        
        self.stdout.write('\n' + '='*60)
        self.stdout.write('\nNext steps:')
        self.stdout.write('  1. Run migration if needed: python manage.py migrate siteconfig')
        self.stdout.write('  2. Start server: python manage.py runserver')
        self.stdout.write('  3. Test wizard: Navigate to /parent/link-child/ as a parent user')
        self.stdout.write('  4. Check dashboard: Verify onboarding progress indicator appears')
        self.stdout.write('')
