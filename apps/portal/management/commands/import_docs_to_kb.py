...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if created:
            self._safe_write(
                self.style.SUCCESS(f"Created root KB category: {operator_root.name}")
            )
        else:
        ...
        operator_categories = [
            ...
            (
                "onboarding",
                "Onboarding (Students, Teachers, Parents)",
                "fa-user-plus",
                3,
            ),
            (
                "academics",
                "Academics (Assignments, Marks, OCR)",
                "fa-graduation-cap",
                4,
            ),
            ...
            (
                "communication",
                "Communication (Messaging, Groups, Announcements)",
                "fa-comments",
                7,
            ),
        ...
        for md_file in iter_source_files():
            ...
            if (not explicit_mapping) and any(
                pattern in md_file.name.upper() for pattern in skip_patterns
            ):
            ...
            try:
                ...
                title, summary, html_content = self._process_markdown(
                ...
                if existing and not options["overwrite"]:
                    self._safe_write(
                        f"  [SKIP] Skipping existing: {title} (use --overwrite to update)"
                    )
                    skipped_count += 1
                    continue
                ...
                self._safe_write(
                    self.style.ERROR(f"  [ERROR] Error processing {md_file.name}: {e}")
                )
        ...
        if options.get("generate_odt"):
            self._safe_write("")
            self._safe_write(
                self.style.SUCCESS("Generating ODT exports for KB articles...")
            )
    ...
    def _process_markdown(self, content, filename):
        ...
        if title_match:
            ...
            title = (
                filename.replace(".md", "").replace("_", " ").replace("-", " ").title()
            )
        ...
        if MARKDOWN_AVAILABLE:
            try:
                ...
                self._safe_write(
                    self.style.WARNING(
                        f"Markdown conversion error: {e}, using simple conversion"
                    )
                )
    ...
    def _clean_html(self, html):
        ...
        html = re.sub(
            r"<table>", '<table class="table table-bordered table-striped">', html
        )
        ...
        html = re.sub(r"<pre><code>", '<div class="code-block"><pre><code>', html)
