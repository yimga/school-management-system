...
urlpatterns = [
    # FAQ URLs
    path("faq/", views_kb.faq_list, name="faq_list"),  # rbac-allow: public knowledge-base FAQ list
    path("faq/<int:faq_id>/", views_kb.faq_detail, name="faq_detail"),  # rbac-allow: public knowledge-base FAQ detail
    path("faq/<int:faq_id>/vote/", views_kb.faq_vote, name="faq_vote"),
    path("faq/submit/", views_kb.faq_submit, name="faq_submit"),
    # Knowledge Base URLs
    path("", views_kb.kb_home, name="kb_home"),  # rbac-allow: public KB home — published articles only
    path("category/<slug:category_slug>/", views_kb.kb_category, name="kb_category"),  # rbac-allow: public KB category index
    path("article/<slug:article_slug>/", views_kb.kb_article, name="kb_article"),  # rbac-allow: public KB article — published+is_public filter at QS level
    path(  # rbac-allow: public KB article download (published-only)
    ...
    # Search
    path("search/", views_kb.kb_search, name="kb_search"),  # rbac-allow: public KB search — only surfaces published articles
    # User contributions
