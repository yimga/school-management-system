...
def _stream_chunks(prompt: str, viewport: str) -> Iterator[bytes]:
    """Bridge stream_litellm -> SSE bytes. Always yields a [DONE] terminator."""
    try:
        from services.ai_gateway_stream import stream_litellm
    except ImportError:
    ...
    emitted = 0
    try:
        for chunk, _meta in stream_litellm(prompt, viewport=viewport):
            if chunk:
                emitted += 1
...
def ai_stream_view(request: HttpRequest) -> Any:
    ...
    response = StreamingHttpResponse(
        _stream_chunks(prompt, viewport),
