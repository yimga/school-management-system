...
class DocumentUploadForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "visible_to_roles": forms.SelectMultiple(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            "visible_to_roles"
        ].help_text = "Select roles that can view this document. Leave empty for all authenticated users."
        self.fields["document_pack"].queryset = DocumentPack.objects.filter(
            is_active=True
        ).order_by("name")
        ...
        if self.is_bound:
            pack_id = self.data.get(self.add_prefix("document_pack")) or self.data.get(
                "document_pack"
            )
            if pack_id:
                selected_pack = DocumentPack.objects.filter(
                    pk=pack_id, is_active=True
                ).first()
    ...
    def clean(self):
        ...
        if (
            cleaned_data.get("requires_signature")
            and cleaned_data.get("document_type") != PortalFeatureItem.DocumentType.FORM
        ):
        ...
        allowed_states = {
            code for code, _label in lifecycle_state_choices(document_pack)
        }
        if lifecycle_state and lifecycle_state not in allowed_states:
            raise ValidationError(
                "Selected lifecycle state is not allowed for the chosen document pack."
            )
