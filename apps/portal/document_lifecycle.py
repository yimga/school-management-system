...
DOCUMENT_LIFECYCLE_VALID_TRANSITIONS = {
    DOCUMENT_LIFECYCLE_DRAFT: (DOCUMENT_LIFECYCLE_REVIEW, DOCUMENT_LIFECYCLE_APPROVED, DOCUMENT_LIFECYCLE_RETRACTED),
    DOCUMENT_LIFECYCLE_REVIEW: (DOCUMENT_LIFECYCLE_APPROVED, DOCUMENT_LIFECYCLE_RETRACTED, DOCUMENT_LIFECYCLE_DRAFT),
    DOCUMENT_LIFECYCLE_APPROVED: (DOCUMENT_LIFECYCLE_ARCHIVED, DOCUMENT_LIFECYCLE_RETRACTED, DOCUMENT_LIFECYCLE_REVIEW),
...
def normalize_document_retention_rule(pack: Any = None) -> dict[str, int | None]:
    ...
    try:
        archive_after_days = int(archive_after_days) if archive_after_days not in (None, "") else None
    except (TypeError, ValueError):
        archive_after_days = None
    try:
        expire_after_days = int(expire_after_days) if expire_after_days not in (None, "") else None
    except (TypeError, ValueError):
...
def lifecycle_state_choices(pack: Any = None) -> list[tuple[str, str]]:
    ...
    return [(code, label) for code, label in DOCUMENT_LIFECYCLE_CHOICES if code in allowed]


def is_valid_document_transition(previous_state: str | None, next_state: str, pack: Any = None) -> bool:
...
def calculate_document_retention_review_at(document: Any) -> Any:
    ...
    if archive_after_days is None:
        return None
    base_dt = getattr(document, "published_at", None) or getattr(document, "created_at", None) or timezone.now()
...
def build_document_search_index(document: Any) -> str:
    ...
    return " ".join(str(value).strip() for value in values if str(value).strip()).lower()
