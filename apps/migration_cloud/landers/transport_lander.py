        ...
        for row in canonical_rows:
            ...
            try:
                ...
                obj, created, preserved = upsert_with_conflict_detection(
                    ctx=ctx, domain="transport", model=Route,
                    lookup=lookup_kwargs, defaults=defaults, legacy_id=route_name,
                )
                if preserved:
                    result.skipped += 1
                    record_id_mapping(ctx=ctx, legacy_id=route_name, canonical_obj=obj, domain="transport")
                    continue
                if created:
                    result.created += 1
