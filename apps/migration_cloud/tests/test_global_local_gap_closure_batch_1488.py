"""Global-local gap closure (batch 1488) — contract pins for migration_cloud.
...
class AiFieldMappingContractTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
        ...
        key = ROOT / "apps/migration_cloud"
        self.assertTrue(key.exists() or key.is_dir(), "missing contract path: " + str(key))

class LegacyFileIngestionTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
        artifact = _artifact("ai_auto_migration_pipeline_audit.json")
        self.assertTrue(artifact.is_file(), "missing audit artifact: " + str(artifact))
        key = ROOT / "apps/migration_cloud"
...
class MigrationDataCleanupDashboardTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
        artifact = _artifact("ai_auto_migration_pipeline_audit.json")
        self.assertTrue(artifact.is_file(), "missing audit artifact: " + str(artifact))
        key = ROOT / "apps/migration_cloud"
...
class VisualDataCleanupContractTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
...
class MigrationAiCredentialRedactionTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
