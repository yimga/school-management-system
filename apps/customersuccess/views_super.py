...
def api_maturity_scores(request):
    ...
    if school_id:
        ...
        seen = set()
        out = []
        for s in TenantMaturityScore.objects.filter(school=school).order_by(
            "-computed_at"
        ):
    ...
    latest = TenantMaturityScore.objects.filter(school_id=OuterRef("pk")).order_by(
        "-computed_at"
    )
    schools = (
        ...
        rows.append(
            {
                ...
                "name": s["name"],
                "slug": s["slug"],
                "latest_maturity_at": s["latest_maturity_at"].isoformat()
                if s.get("latest_maturity_at")
                else None,
                "latest_maturity_score": str(s["latest_maturity_score"])
                if s.get("latest_maturity_score") is not None
                else None,
            }
        )
...
def api_intervention_suggestions(request):
    ...
    qs = TenantInterventionSuggestion.objects.select_related("school").order_by(
        "priority", "-created_at"
    )
    if school_id:
        qs = qs.filter(school_id=school_id)
