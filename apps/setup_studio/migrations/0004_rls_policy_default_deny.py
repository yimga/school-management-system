# RLS default-deny for tenant-scoped setup_studio tables.

from django.db import connection, migrations

from apps.schools.rls import should_apply_rls

TABLES = ['setup_studio_setupprogress']
USING_CLAUSE = """(
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in TABLES:
            short = table.replace("setup_studio_", "", 1)
            policy_name = f"setup_studio_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("setup_studio", "0003_enable_rls_postgresql"),
    ]
