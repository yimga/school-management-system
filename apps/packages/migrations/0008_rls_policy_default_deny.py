# RLS default-deny for tenant-scoped packages tables.
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in TABLES:
            short = table.replace("packages_", "", 1)
            policy_name = f"packages_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("packages", "0007_enable_rls_postgresql"),
    ]
