...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if reset:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"Deleted {deleted} existing Ultra High-End experience pack(s)."
                )
            )
        ...
        for theme_slug, exp_code in ULTRA_PORTAL_SLUGS:
            theme_pack = ThemePack.objects.filter(
                slug=theme_slug, is_active=True
            ).first()
            if not theme_pack:
                self.stdout.write(
                    self.style.WARNING(
                        f"  Skip {exp_code}: ThemePack slug={theme_slug} not found. Run seed_admin_dashboard_palettes first."
                    )
                )
                continue
            ...
            pack, was_created = ExperiencePack.objects.update_or_create(
                ...
                defaults={
                    ...
                    "description": defaults.get(
                        "description", "Ultra high-end experience pack."
                    ),
            ...
            if was_created:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"  + Created: {pack.name} (code={pack.code}, theme_pack_id={pack.theme_pack_id})"
                    )
                )
            else:
                updated += 1
