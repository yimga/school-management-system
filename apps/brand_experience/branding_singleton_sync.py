"""
Sync the siteconfig tenant settings compatibility write surface <-> PlatformGlobalBranding (bounded context).
"""
...
# Field names shared between the slim tenant settings row and PlatformGlobalBranding.
...
def sync_platform_branding_row_from_sitesettings(site: Any) -> None:
    """
    After the slim tenant settings row saves, copy concrete branding/report-default columns into
...
def merge_platform_global_branding_into_base(base: Any) -> None:
    """
    Overlay PlatformGlobalBranding onto the shallow tenant-settings copy used by
...
def mirror_platform_global_branding_fk_ids_to_runtime_payload() -> None:
    ...
    owned_payload on the slim row and domain snapshots stay consistent after Batch 3.
    """
    row = get_platform_global_branding()
    if row is None:
        return
    ...
    try:
        from apps.platform_runtime.helpers import persist_platform_runtime_payload_updates
