...
WORKFLOWS: dict[str, WorkflowDefinition] = {
    ...
    "workflow_flight_deck_simulation": WorkflowDefinition(
        key="workflow_flight_deck_simulation",
        title="Flight Deck scenario simulation",
        audience=AUDIENCE_OPERATOR,
        module="platform_runtime",
        route="platform_runtime:workflow_progress_simulate",
        purpose="Operator preset that proves live progress, failure isolation, remediator, and success archive.",
        steps=(
            WorkflowStep("lint_verify", "Lint & Verify"),
            WorkflowStep("build_package", "Build Package"),
            WorkflowStep("integration_test", "Integration Test"),
            WorkflowStep("cloud_deploy", "Cloud Deploy"),
        ),
        success_state="Run is archived to Success Logs after pass or resume-from-failure.",
        default_tags=(TAG_PLATFORM_ONLY, TAG_AUDIT_LOGGED, TAG_REVERSIBLE),
        slo_seconds=30,
        owner_team="platform-provisioning",
    ),
