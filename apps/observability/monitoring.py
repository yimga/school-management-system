...
def _read_process_rss_mb() -> float:
    ...
    try:
        import resource  # Unix only; not on Windows
...
class SystemHealthMonitor:
    ...
    def record_metric(metric_type: str, value: float, threshold: float = 100.0):
        ...
        metric = SystemHealthMetric.objects.create(
            metric_type=metric_type, value=value, threshold=threshold, status=status
        )
...
def _sync_health_metric_incident(metric, *, status: str, alert=None):
    ...
    metric_key = (
        str(getattr(metric, "metric_type", "") or "").strip().lower() or "unknown"
    )
...
class AnomalyDetector:
    ...
    def calculate_baseline(metric_type: str, days: int = 7) -> float:
        ...
        metrics = SystemHealthMetric.objects.filter(
            metric_type=metric_type, recorded_at__gte=cutoff, status="healthy"
        )
...
class DetailedHealthCheckEndpoint(View):
    ...
    def get(self, request):
        ...
        recent_metrics = (
            SystemHealthMetric.objects.filter(recorded_at__gte=cutoff)
            .values("metric_type")
            .annotate(
        ...
        # Get active alerts
        active_alerts = HealthCheckAlert.objects.filter(is_resolved=False).count()
...
class MetricsHistoryEndpoint(View):
    ...
    def get(self, request):
        ...
        data = list(
            query.values("metric_type", "value", "status", "recorded_at").order_by(
                "recorded_at"
            )
        )
