# RLS default-deny for tenant-scoped school_events tables.
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in SCHOOL_EVENTS_TABLES:
            short = table.replace("school_events_", "")
            policy_name = f"school_events_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("school_events", "0002_enable_rls_postgresql"),
    ]
