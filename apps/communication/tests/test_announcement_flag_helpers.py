...
class AnnouncementFlagHelperTests(SimpleTestCase):
    ...
    def test_get_announcement_feature_flags_falls_back_when_policy_lookup_fails(self):
        ...
        with patch("apps.communication.views_announcements.get_effective_flags", return_value={}):
            with patch(
    ...
    def test_get_announcement_feature_flags_uses_default_when_flag_resolution_fails(self):
        request = self.factory.get("/")

        with patch("apps.communication.views_announcements.get_effective_flags", side_effect=TypeError):
            with patch(
                "apps.communication.views_announcements.default_announcement_submit_for_approval_roles",
