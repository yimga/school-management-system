...
class CommunicationApiTenantIsolationTests(TestCase):
    ...
    def test_message_create_rejects_cross_tenant_recipient(self):
        ...
        with rls_school(self.school.id):
    ...
    def test_broadcast_only_targets_current_school(self):
        ...
        with rls_bypass():
            self.assertEqual(
                Message.objects.filter(school=self.school, recipient=self.parent).count(),
                1,
            )
            self.assertEqual(
                Message.objects.filter(recipient=self.other_parent).count(), 0
            )
