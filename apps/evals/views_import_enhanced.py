...
def grade_import_job_detail(request, job_id):
    ...
    if request.GET.get("export") == "csv":
        ...
        response["Content-Disposition"] = f'attachment; filename="import_log_{job.id}.csv"'
    ...
    return render(request, "evals/grade_import_job_detail.html", {
        "job": job,
...
def grade_import_job_list(request):
    ...
    return render(request, "evals/grade_import_job_list.html", {
        "jobs": paginated_jobs,
        "year": year,
        "term": term,
...
def grading_analytics_dashboard(request):
    ...
    assignments = TeacherAssignment.objects.filter(
        academic_year=year,
        is_active=True,
    ).values("teacher__user__full_name", "subject_assignment__subject__name").annotate(
        total=Count("id")
    )
    ...
    return render(request, "evals/grading_analytics_dashboard.html", {
        "year": year,
        "term": term,
