...
def _rankings_cache_prefix(
    subject_id=None, classroom_id=None, year_id=None, term_id=None
):
    school_id = None
    if classroom_id:
        school_id = (
...
def get_cached_rankings(
    year_id=None, term_id=None, subject_id=None, classroom_id=None, force_refresh=False
):
    """
    ...
    rankings = (
        query.values(
            "student_id", "student__user__first_name", "student__user__last_name"
        )
    ...
    for idx, ranking in enumerate(rankings, start=1):
        percentile = (
            (100 - ((idx - 1) / max(rankings.count() - 1, 1) * 100))
            if rankings.count() > 1
            else 100
        )
        result.append(
...
# tenant-isolation-allow: cache-key-includes-tenant-prefix-from-caller
    ...
    cache_ttl_minutes = (
        getattr(site_settings, "cache_rankings_interval_minutes", None) or 60
    )
...
def invalidate_rankings_cache(
    year_id=None, term_id=None, subject_id=None, classroom_id=None
):
