...
def completion_by_class(academic_year=None):
    ...
    for eval_obj in qs.iterator():
        ...
        if eval_obj.is_complete_for_ranking if hasattr(eval_obj, "is_complete_for_ranking") else True:
    ...
    return {
        class_id: {
            ...
            "percent": int(round((completed[class_id] / totals[class_id]) * 100)) if totals[class_id] else 0,
        }
