...
class GradeImportJob(models.Model):
    ...
    error_summary = models.TextField(
        blank=True, help_text="JSON summary of validation/processing errors"
    )
    processing_notes = models.TextField(blank=True)

    # Optional: delta detection
    is_delta_update = models.BooleanField(
        default=False, help_text="True if this is incremental update"
    )
    ...
    def __str__(self):
        return (
            f"{self.filename} ({self.status}) - {self.academic_year} {self.term.label}"
        )
...
class GradeImportRowLog(models.Model):
    ...
    import_job = models.ForeignKey(
        GradeImportJob, on_delete=models.CASCADE, related_name="row_logs"
    )
    ...
    student = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name="import_row_logs",
    )
    subject_assignment = models.ForeignKey(
        SubjectAssignment, on_delete=models.SET_NULL, null=True
    )
    ...
    previous_values = models.JSONField(
        default=dict, help_text="Old Evaluation scores before update"
    )
    new_values = models.JSONField(
        default=dict, help_text="New Evaluation scores after update"
    )
...
class EvaluationDelta(models.Model):
    ...
    # Parent imports
    from_import = models.ForeignKey(
        GradeImportJob, on_delete=models.CASCADE, related_name="deltas_detected"
    )
    ...
    evaluation = models.ForeignKey(
        "evals.Evaluation", on_delete=models.CASCADE, related_name="deltas"
    )
    student = models.ForeignKey(
        StudentProfile, on_delete=models.CASCADE, related_name="evaluation_deltas"
    )
...
class EvaluationEvidence(models.Model):
    ...
    evaluation = models.ForeignKey(
        "evals.Evaluation", on_delete=models.CASCADE, related_name="evidence"
    )
...
class CompetencyRubric(models.Model):
    ...
    # Rubric metadata
    academic_year = models.ForeignKey(
        AcademicYear, on_delete=models.CASCADE, related_name="competency_rubrics"
    )
    term = models.ForeignKey(Term, on_delete=models.CASCADE)
    subject_assignment = models.ForeignKey(SubjectAssignment, on_delete=models.CASCADE)
...
class CompetencyItem(models.Model):
    ...
    rubric = models.ForeignKey(
        CompetencyRubric, on_delete=models.CASCADE, related_name="items"
    )
    name = models.CharField(
        max_length=255, help_text="e.g., 'Circuit Wiring', 'Welding Technique'"
    )
    description = models.TextField(blank=True)
...
class StudentCompetencyAssessment(models.Model):
    ...
    student = models.ForeignKey(
        StudentProfile, on_delete=models.CASCADE, related_name="competency_assessments"
    )
    competency_item = models.ForeignKey(
        CompetencyItem, on_delete=models.CASCADE, related_name="student_assessments"
    )
    teacher = models.ForeignKey(TeacherProfile, on_delete=models.CASCADE)

    # Assessment result
    level = models.CharField(
        max_length=20, choices=CompetencyItem.CompetencyLevel.choices
    )
    observations = models.TextField(
        blank=True, help_text="Teacher notes on this competency"
    )
...
class ClockHourTracking(models.Model):
    ...
    student = models.ForeignKey(
        StudentProfile, on_delete=models.CASCADE, related_name="clock_hour_records"
    )
    subject_assignment = models.ForeignKey(SubjectAssignment, on_delete=models.CASCADE)

    date = models.DateField()
    hours = models.DecimalField(
        max_digits=4, decimal_places=2, help_text="Hours spent in workshop"
    )
    activity_description = models.CharField(
        max_length=255, help_text="e.g., 'Welding - Arc Techniques'"
    )

    recorded_by = models.ForeignKey(
        TeacherProfile, on_delete=models.SET_NULL, null=True
    )
    ...
    @classmethod
    def total_hours_for_student(
        cls, student, subject_assignment, start_date=None, end_date=None
    ):
...
class ReportCardTemplate(models.Model):
    ...
    school_logo = models.ImageField(
        upload_to="report_templates/logos/", null=True, blank=True
    )
    school_letterhead_text = models.TextField(
        blank=True, help_text="School name, address, contact info"
    )
...
class MockExamEvaluation(models.Model):
    ...
    mock_session = models.ForeignKey(
        MockExamSession, on_delete=models.CASCADE, related_name="evaluations"
    )
