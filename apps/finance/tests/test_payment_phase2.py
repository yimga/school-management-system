...
class PaymentMethodTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            "finance_pm_%s" % uid, "pm_%s@test.com" % uid, "pass"
        )
...
class PaymentTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            "finance_pay_%s" % uid, "pay_%s@test.com" % uid, "pass"
        )
...
class PaymentAuditLoggingTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            "finance_auditor_%s" % uid, "audit_%s@test.com" % uid, "pass"
        )
        self.method = PaymentMethod.objects.create(
            name="Audit Card",
            method_type="card",
    ...
    def test_mark_processing_logs_audit(self):
        ...
        log = PaymentAuditLog.objects.filter(
            payment=self.payment, action_type="payment_initiated"
        ).last()
    ...
    def test_mark_completed_logs_audit(self):
        ...
        log = PaymentAuditLog.objects.filter(
            payment=self.payment, action_type="payment_completed"
        ).last()
        self.assertIsNotNone(log)
    ...
    def test_mark_failed_logs_audit(self):
        ...
        log = PaymentAuditLog.objects.filter(
            payment=self.payment, action_type="payment_failed"
        ).last()
        self.assertIsNotNone(log)
...
class TransactionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            "finance_tx_%s" % uid, "tx_%s@test.com" % uid, "pass"
        )
        self.method = PaymentMethod.objects.create(
            name="Card",
            method_type="card",
...
class AmountValidatorTestCase(TestCase):
    ...
    def test_amount_below_minimum(self):
        ...
        validator = AmountValidator()
        self.assertFalse(
            validator.validate_amount(Decimal("0.001"), min_amount=Decimal("0.01"))
        )
    ...
    def test_amount_above_maximum(self):
        ...
        validator = AmountValidator()
        self.assertFalse(
            validator.validate_amount(Decimal("1000.00"), max_amount=Decimal("500.00"))
        )
...
class PaymentMethodValidatorTestCase(TestCase):
    ...
    def test_validate_api_configuration_valid(self):
        ...
        self.assertTrue(
            validator.validate_api_configuration(
...
class RefundValidatorTestCase(TestCase):
    ...
    def test_valid_refund_amount(self):
        ...
        self.assertTrue(
            validator.validate_refund_amount(Decimal("500.00"), Decimal("250.00"))
        )
    ...
    def test_refund_exceeds_payment(self):
        ...
        validator = RefundValidator()
        self.assertFalse(
            validator.validate_refund_amount(Decimal("500.00"), Decimal("600.00"))
        )
...
class RefundRequestTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="finance_refund_%s" % uid,
            email="refund_%s@test.com" % uid,
            password="pass",
        )
        self.method = PaymentMethod.objects.create(
            name="Card",
...
class PaymentAuditLogTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            "finance_auditlog_%s" % uid, "auditlog_%s@test.com" % uid, "pass"
        )
