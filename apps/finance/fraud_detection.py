...
class ReceiptFraudDetector:
    ...
    def _analyze_upload_pattern(
        ...
        invoice_uploads = PaymentProofUpload.objects.filter(
            invoice_id=invoice_id
        ).exclude(status=PaymentProofUpload.Status.REJECTED).count()
    ...
    def _analyze_ip_address(
        ...
        recent_uploads_same_ip = PaymentProofUpload.objects.filter(
            ip_address=ip_address,
            created_at__gte=window_start
        ).exclude(uploaded_by_id=uploaded_by_id).count()
