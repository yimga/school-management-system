...
class BankDepositVerifier:
    ...
    def verify_deposit(
        self, receipt_upload, bank_statements: List[any], tolerance_days: int = 7
    ) -> Dict:
        """
        ...
        if (
            not receipt_upload.transaction_reference
            and not receipt_upload.uploaded_amount
        ):
            return {
                "verified": False,
                "matched_entry": None,
        ...
        if not relevant_statements:
            return {
                ...
                "discrepancies": [
                    f"No bank statements found between {search_start} and {search_end}"
                ],
            }
        ...
        if receipt_upload.transaction_reference:
            matched = self._match_by_reference(
                receipt_upload.transaction_reference, relevant_statements
            )
            if matched:
                return {
                    "verified": True,
    ...
    def _match_by_reference(
        self, transaction_reference: str, bank_statements: List[any]
    ) -> Optional[any]:
        ...
        for stmt in bank_statements:
            ...
            if (
                ...
                and stmt.transaction_reference.upper().strip() == ref_upper
            ):
                return stmt
    ...
    def verify_mtn_momo_deposit(
        self, receipt_upload, mtn_momo_statements: List[any]
    ) -> Dict:
        ...
        return {
            ...
            "match_method": "no_match",
            "discrepancies": [
                f"MTN MoMo transaction {receipt_upload.transaction_reference} not found"
            ],
        }

    def verify_orange_money_deposit(
        self, receipt_upload, orange_money_statements: List[any]
    ) -> Dict:
        ...
        return {
            ...
            "match_method": "no_match",
            "discrepancies": [
                f"Orange Money transaction {receipt_upload.transaction_reference} not found"
            ],
        }
