...
class PayPalProcessor:
    ...
    def create_billing_plan(
        self, name, amount, frequency="MONTH", tenure_type="REGULAR"
    ):
...
class FlutterwaveProcessor:
    ...
    def charge_card(
        self, card_number, cvv, expiry_month, expiry_year, amount, currency="NGN"
    ):
...
class MTNMobileMoneyProcessor:
    ...
    def request_payment(
        self, amount, currency, phone_number, reference, description=""
    ):
...
class OrangeMoneyProcessor:
    ...
    SUPPORTED_COUNTRIES = [
        ...
        "ML",
        "BF",
    ]  # Cameroon, Senegal, Ivory Coast, Mali, Burkina Faso
...
class MultiGatewayProcessor:
    ...
    def __init__(self, configs):
        ...
        for provider, config in configs.items():
            try:
                self.processors[provider] = ProcessorFactory.get_processor(
                    provider, **config
                )
            except (ImportError, AttributeError, TypeError, ValueError, KeyError) as e:
                logger.warning(
                    "Failed to initialize payment provider %s: %s", provider, e
                )
