...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--account-id", type=int, help="Verify only for specific bank account"
        )
        parser.add_argument(
            "--days",
            type=int,
                    ...
                    if auto_approve and verification_result["match_confidence"] >= 0.9:
                        with transaction.atomic():
                            ...
                            if not receipt_upload.verification_data:
                                ...
                                receipt_data = (
                                    verification_service.extract_receipt_data(
                                        receipt_upload.receipt_file
                                    )
                                )
                                receipt_upload.verification_data = receipt_data
                                if receipt_data.get("amount"):
                                    receipt_upload.uploaded_amount = receipt_data[
                                        "amount"
                                    ]
