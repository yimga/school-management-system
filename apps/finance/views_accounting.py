...
def trial_balance(request: HttpRequest):
    ...
    accounts = LedgerAccount.objects.filter(profile=profile, is_active=True).annotate(
        debit_total=Coalesce(Sum("lines__debit", filter=line_filter), Decimal("0.00")),
        credit_total=Coalesce(Sum("lines__credit", filter=line_filter), Decimal("0.00")),
    ).order_by("code")
...
def suspense_queue(request: HttpRequest):
    ...
    return render(request, "finance/suspense_queue.html", {
        "queue": queue,
