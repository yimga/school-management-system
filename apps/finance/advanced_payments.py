...
class SplitPaymentPart(models.Model):
    ...
    payer = models.ForeignKey(User, on_delete=models.CASCADE, related_name='payment_splits')
    amount = models.DecimalField(max_digits=10, decimal_places=2)
...
class PaymentAdvancedService:
    ...
    @staticmethod
    def create_recurring_subscription(user, plan: PaymentPlan, payment_method_id: str, start_date=None):
    ...
    def get_payment_analytics(user) -> Dict:
        ...
        return {
            ...
            'payment_completion_rate': (float(total_paid) / float(total_invoiced) * 100) if total_invoiced > 0 else 0,
        }
