"""Global-local gap closure (batch 1488) — contract pins for interop.
...
class SchemaMappingValidationTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
        artifact = _artifact("universal_schema_mapping_audit.json")
        self.assertTrue(artifact.is_file(), "missing audit artifact: " + str(artifact))
        key = ROOT / "apps/interop"
...
class SelfHealingIntegrationSandboxTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
