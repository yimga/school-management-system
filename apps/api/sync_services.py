    ...
    with transaction.atomic():
        for idx, item in enumerate(items):
            ...
            if not isinstance(changes, dict):
                results.append({"index": idx, "status": 400, "data": {"error": "changes must be an object"}})
                continue
            updates = {k: v for k, v in changes.items() if k in allowed}
            if not updates:
                results.append({"index": idx, "status": 400, "data": {"error": "No allowed fields to update"}})
                continue

            try:
                instance = model.objects.get(pk=pk)
            except model.DoesNotExist:
                results.append({"index": idx, "status": 404, "data": {"error": "Not found"}})
                continue
            ...
            if client_updated_at and server_dt:
                ...
                if client_updated_at < server_dt:
                    ...
                    server_data = _serialize_instance_for_conflict(instance, entity_type, allowed)
                    conflict_id = None
                    ...
                    results.append({
                        "index": idx,
                        "status": 409,
                        "data": {"error": "conflict", "server_updated_at": server_dt.isoformat(), "conflict_id": conflict_id},
                        "conflict_id": conflict_id,
                    })
                    continue
            ...
            results.append({
                "index": idx,
                "status": 200,
                "data": {"id": instance.pk, "updated_at": new_updated_at.isoformat() if new_updated_at else None},
            })
