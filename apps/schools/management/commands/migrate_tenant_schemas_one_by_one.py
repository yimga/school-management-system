...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--dry-run", action="store_true", help="Only list tenants, do not run migrations.")
    ...
    def handle(self, *args, **options):
        ...
        for client in Client.objects.all().order_by("id"):
            ...
            if not schema_name or schema_name == "public":
                continue
            if dry_run:
                self.stdout.write("Would migrate tenant: %s (schema %s)" % (client.name, schema_name))
                ok += 1
                continue
            try:
        ...
        if failed:
            self.stdout.write(self.style.ERROR("Failed %s schema(s): %s" % (len(failed), [f[0] for f in failed])))
        self.stdout.write(self.style.SUCCESS("Done. OK=%s Failed=%s" % (ok, len(failed))))
