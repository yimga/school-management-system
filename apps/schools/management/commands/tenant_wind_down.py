...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--school-id", type=int, help="School (tenant) ID to wind down")
        parser.add_argument("--slug", type=str, help="School slug to wind down")
        parser.add_argument("--export-only", action="store_true", help="Only export data; do not deactivate")
        parser.add_argument("--no-deactivate", action="store_true", help="Same as --export-only")
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING("Compliance/people not available; skipping per-student export."))
            else:
                students = StudentProfile.objects.filter(school=school).values_list("id", flat=True)[:1000]
                count = 0
                for sid in students:
                    try:
            ...
            if not export_only:
                ...
                self.stdout.write(self.style.SUCCESS(f"Tenant {school.slug} deactivated (wind-down complete)."))
            else:
