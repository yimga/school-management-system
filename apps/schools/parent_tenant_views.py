...
def parent_tenant_dashboard(request):
    ...
    if not school and request.session.get("school_id"):
        school = School.objects.filter(id=request.session["school_id"], is_active=True).first()
    if not school:
        return HttpResponseForbidden("School context required.")
    child_schools = School.objects.filter(parent_school=school, is_active=True).order_by("name")
    if not child_schools.exists():
        return HttpResponseForbidden("This school has no child schools. Parent-tenant dashboard is for parent organizations only.")
    ...
    return render(request, "schools/parent_tenant_dashboard.html", {
