...
FEATURE_REGISTER: tuple[FeatureRow, ...] = (
    ...
    # Forward-looking rows from the "Cloud-Dependency-Collapse" lifecycle manifest.
    # Honest status only: primitives exist (see notes) but the productized,
    # lifecycle-complete flow is not yet shipped. Flip to "shipped" + add a
    # resolving proof_* per Wave A of the plan, never before.
    FeatureRow(
        feature_slug="offline-intake-wizard",
        label="Offline-first intake wizard (IndexedDB-cached registration)",
        capability_domain="identity",
        status="in_progress",
        notes="VERIFIED 2026-06-07: wizard framework SHIPPED (setup_studio:tenant_wizard) + generic offline-db.js/OfflineSyncQueue. GAP: intake-wizard-specific offline cache + enable_offline_intake_wizard flag.",
    ),
    FeatureRow(
    ...
    FeatureRow(
        ...
        capability_domain="studio_os",
        status="in_progress",
        notes="VERIFIED 2026-06-07: greedy conflict-checker kernel ships (apps/academics/timetable_solver.py + views_timetable_solver.py, TimeSlot/Resource/LessonRequest). Full CSP solver (backtracking, soft constraints) deferred to Wave N+1.",
    ),
    FeatureRow(
        ...
        capability_domain="identity",
        status="planned",
        notes="Home: apps/student360 + people. Single identity, split scheduling across two programs.",
    ),
    FeatureRow(
        ...
        status="in_progress",
        notes="VERIFIED 2026-06-07: redacted time-boxed handover packet ships (apps/schoolops/substitute_handover.py + views/forms, TeacherAbsenceTrigger/HandoverPacket). GAP: auto-find candidate + WhatsApp/SMS broadcast not yet wired.",
    ),
    FeatureRow(
    ...
    FeatureRow(
        ...
        capability_domain="billing",
        status="planned",
        notes="VERIFIED 2026-06-07: only tax-rate foundation exists (finance/tax_engine.py VAT/GST, 40+ jurisdictions). ZERO stamped-invoice code (no NF-e/CFDI XML, no SAT/SEFAZ adapter, no fiscal signing). Genuinely planned, not in-progress.",
    ),
    FeatureRow(
    ...
    FeatureRow(
        ...
        capability_domain="classroom",
        status="planned",
        notes="Carved out of omr-scan-grading 2026-06-07: distinct from text OCR — needs computer-vision detection of 4-orientation printed cards in one camera sweep. Separate build from the shipped OCR marksheet path.",
    ),
    FeatureRow(
