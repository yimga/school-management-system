...
def get_tenant_rls_status(table_names: list[str]) -> dict[str, bool]:
    ...
    table_names must be an iterable of strings (not a bare str/bytes, which would iterate
    by character). Only tables that exist are included. Duplicate relnames are collapsed to
    ...
    if connection.vendor != "postgresql" or getattr(
        ...
        return {}
    if isinstance(table_names, (str, bytes)) or not table_names:
        return {}
