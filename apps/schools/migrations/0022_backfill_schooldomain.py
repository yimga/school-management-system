# Generated for RunYourCampus powerhouse: backfill SchoolDomain from School.subdomain and School.custom_domain
...
def backfill_schooldomain(apps, schema_editor):
    ...
    base_domain = (os.getenv("MULTI_TENANT_BASE_DOMAIN") or "runyourcampus.com").strip().lower()
    for school in School.objects.filter(is_active=True):
        # Subdomain: subdomain.runyourcampus.com
        if school.subdomain:
