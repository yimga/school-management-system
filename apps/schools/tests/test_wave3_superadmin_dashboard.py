...
class Wave3SuperadminIndexNoDirectTenantCrudTests(TestCase):
    ...
    def test_super_dashboard_url_resolves(self):
        ...
        self.assertIn(response.status_code, (200, 302), "Super dashboard must load for platform user.")
    ...
    def test_superadmin_index_template_is_control_plane(self):
        ...
        try:
            ...
            self.assertIsNotNone(t, "index_superadmin.html must exist for control plane.")
...
class Wave3TenantDataAccessRequiresImpersonationOrSupportTests(TestCase):
    ...
    def test_switch_to_tenant_requires_control_plane(self):
        ...
        names = [getattr(p, "name", None) for p in super_urls.urlpatterns if getattr(p, "name", None)]
        self.assertIn("switch_to_tenant", names, "Impersonation entry must be in super_urls.")