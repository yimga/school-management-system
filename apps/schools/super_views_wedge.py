...
def super_curriculum_packs(request):
    ...
    uk_statutory_reports_url = _safe_reverse("siteconfig:report_library") or _safe_reverse("reports:report_list")
    return render(
        request,
...
def super_one_sis_any_lms(request):
    ...
    integration_url = _safe_reverse("siteconfig:integration_catalog") or _safe_reverse("apicenter:dashboard")
...
def super_advancement_hub(request):
    ...
    campaigns_url = _safe_reverse("communication:group_list")  # or broadcast list if exists
...
def super_advancement_phase2_placeholder(request):
    """Phase 2 advancement: donors, campaigns, gifts, receipts — in-product placeholder until full implementation."""
    return render(
        request,
...
def super_geography(request):
    ...
    continent_packs = [
        {"wedge": 7, "continent": "Africa", "codes": ["LCA", "WAEC", "AFR_FR"], "description": "LCA (low-connectivity), WAEC (Anglophone West Africa), Francophone Africa."},
        {"wedge": 8, "continent": "Asia", "codes": ["ASIA"], "description": "Asia Education Pack; national curricula and ministry reporting."},
        {"wedge": 9, "continent": "Europe (beyond UK)", "codes": ["EU", "GBR"], "description": "EU pack (FRA, DEU, etc.); UK/GBR (wedge 3)."},
        {"wedge": 10, "continent": "North America", "codes": ["US", "CAN"], "description": "United States and Canada; FERPA, PIPEDA."},
        {"wedge": 11, "continent": "South America", "codes": ["BRA", "LATAM_ES"], "description": "Brazil (BRA); Spanish South America (ARG, COL, CHL, etc.)."},
        {"wedge": 12, "continent": "Oceania", "codes": ["AUS", "NZL"], "description": "Australia and New Zealand; national curricula and statutory."},
        {"wedge": 13, "continent": "MENA", "codes": ["MENA"], "description": "Middle East & North Africa; AR/EN/FR; ministry and calendar."},
    ]
    create_school_url = _safe_reverse("super:create_school_wizard")
    geography_list = []
    for item in continent_packs:
        ...
        for c in item["codes"]:
            ...
            create_school_with_pack_url = (create_school_url + "?pack=" + quote(c)) if create_school_url else None
            packs.append({
                "code": c,
                "name": raw.get("name", c),
...
def super_education_systems(request):
    ...
    from apps.registries.services import WEDGE_14_22_SECTOR_CODES, build_education_system_support_accordion
...
def super_group_campuses(request):
    ...
    create_school_url = _safe_reverse("super:create_school_wizard")
    tenant_360_url_name = "super:tenant_360"
    ...
    standalone_ids = set(
        School.objects.filter(parent_school_id__isnull=True, is_active=True)
        .values_list("id", flat=True)
    ) - group_ids
    standalone = list(School.objects.filter(pk__in=standalone_ids).order_by("name")[:50].only("id", "name", "slug"))
    schools_list_url = _safe_reverse("super:schools_list")
