# RLS default-deny for tenant-scoped metadata tables.
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in TABLES:
            short = table.replace("metadata_", "", 1)
            policy_name = f"metadata_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("metadata", "0013_enable_rls_postgresql"),
    ]
