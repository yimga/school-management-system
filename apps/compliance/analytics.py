...
class ComplianceAnalytics:
    ...
    def get_compliance_overview(self):
        ...
        completed = RegionalComplianceRequirement.objects.filter(status__in=['implemented', 'active']).count()
        pending = RegionalComplianceRequirement.objects.filter(status='pending').count()
        overdue = sum(1 for r in RegionalComplianceRequirement.objects.all() if r.is_overdue())
        ...
        return {
            ...
            'pending': pending,
            'overdue': overdue,
            'completion_percentage': round((completed / total_requirements * 100) if total_requirements > 0 else 0, 1),
            'on_time_percentage': round(((total_requirements - overdue) / total_requirements * 100) if total_requirements > 0 else 0, 1),
        }
    ...
    def get_regional_metrics(self):
        ...
        for region in RegionConfig.objects.all():
            ...
            metrics[region.code] = {
                ...
                'overdue': overdue,
                'compliance_score': round((active / total * 100) if total > 0 else 0, 1),
                'status': 'Critical' if overdue > 0 else ('On Track' if active == total else 'At Risk'),
            }
    ...
    def get_document_status(self):
        ...
        for doc_type, _ in docs.model._meta.get_field('document_type').choices:
            ...
            languages = docs.filter(document_type=doc_type).values('language').distinct().count()
    ...
    def get_timeline_data(self, days=90):
        ...
        for i in range(days, 0, -1):
            ...
            timeline.append({
                ...
                'pass_rate': round((checks_that_day.filter(status='pass').count() / checks_that_day.count() * 100) if checks_that_day.count() > 0 else 0, 1),
    ...
    def get_regional_comparison(self, regions=None):
        ...
        for region in regions:
            ...
            comparison.append({
                ...
                'region_name': region.name,
                'requirement_completion': round((completed / total_reqs * 100) if total_reqs > 0 else 0, 1),
                'check_pass_rate': round((passed_checks / total_checks * 100) if total_checks > 0 else 0, 1),
    ...
    def get_critical_items(self):
        ...
        for check in ComplianceCheck.objects.filter(status='fail').order_by('-check_date')[:10]:
        ...
        return sorted(critical, key=lambda x: {'critical': 0, 'high': 1, 'medium': 2}.get(x['severity'], 3))
