...
class ThreatDetectionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        SchoolMembership.objects.create(
            user=self.user1, school=self.school, role="ADMIN", is_primary=True
        )
        SchoolMembership.objects.create(
            user=self.user2, school=self.other_school, role="ADMIN", is_primary=True
        )
    ...
    def test_after_hours_access_detection(self):
        ...
        after_hours_time = timezone.now().replace(
        ...
        after_hours_findings = [
            f for f in findings if f["type"] == "AFTER_HOURS_ACCESS"
        ]
