...
class ComplianceRuleTestCase(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="compliance_rule_%s" % uid,
            email="rule_%s@test.com" % uid,
            password="pass",
        )
...
class RegionalComplianceRequirementTestCase(TestCase):
    def setUp(self):
        uid = id(self)
        self.user = User.objects.create_user(
            username="compliance_req_%s" % uid,
            email="req_%s@test.com" % uid,
            password="pass",
        )
...
class ComplianceCheckTestCase(TestCase):
    def setUp(self):
        uid = id(self)
        self.user = User.objects.create_user(
            username="compliance_checker_%s" % uid,
            email="checker_%s@test.com" % uid,
            password="pass",
        )
        self.admin = User.objects.create_user(
            username="compliance_admin_%s" % uid,
            email="admin_%s@test.com" % uid,
            password="pass",
        )
        self.region, _ = RegionConfig.objects.get_or_create(
            code="TKN",
        ...
        self.requirement = RegionalComplianceRequirement.objects.create(
            region=self.region, rule=self.rule, status="active", created_by=self.admin
        )
...
class LegalDocumentTestCase(TestCase):
    def setUp(self):
        uid = id(self)
        self.user = User.objects.create_user(
            username="compliance_legal_%s" % uid,
            email="legal_%s@test.com" % uid,
            password="pass",
        )
        self.region, _ = RegionConfig.objects.get_or_create(
            code="TNG",
    ...
    def test_multi_language_documents(self):
        ...
        docs = LegalDocument.objects.filter(
            region=self.region, document_type="privacy_policy"
        )
...
class StudentIDValidatorTestCase(TestCase):
    ...
    def test_valid_student_id(self):
        id_format = StudentIDFormat.objects.create(
            region=self.region,
            format_pattern="CMR-YY-nnnn",
            prefix="CMR",
            min_length=10,
            max_length=20,
            example_id="CMR-24-0001",
        )
...
class ComplianceAuditLogTestCase(TestCase):
    def setUp(self):
        uid = id(self)
        self.user = User.objects.create_user(
            username="compliance_auditor_%s" % uid,
            email="auditor_%s@test.com" % uid,
            password="pass",
        )
        self.region, _ = RegionConfig.objects.get_or_create(
            code="TUS",
...
class RegionalComplianceValidatorTestCase(TestCase):
    def setUp(self):
        uid = id(self)
        self.user = User.objects.create_user(
            username="compliance_validator_%s" % uid,
            email="validator_%s@test.com" % uid,
            password="pass",
        )
        self.region, _ = RegionConfig.objects.get_or_create(
            code="TN2",
    ...
    def test_compliance_score(self):
        for i in range(5):
            ...
            RegionalComplianceRequirement.objects.create(
                region=self.region, rule=r, status=status, created_by=self.user
            )
