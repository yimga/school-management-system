...
class ComplianceAnalyticsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="compliance_analytics_%s" % uid,
            email="analytics_%s@test.com" % uid,
            password="pass",
        )
    ...
    def test_compliance_overview_with_data(self):
        ...
        RegionalComplianceRequirement.objects.create(
            region=self.region, rule=rule, status="active", created_by=self.user
        )
        ...
        RegionalComplianceRequirement.objects.create(
            region=self.region, rule=rule2, status="pending", created_by=self.user
        )
    ...
    def test_check_statistics_with_data(self):
        ...
        req = RegionalComplianceRequirement.objects.create(
            region=self.region, rule=rule, created_by=self.user
        )
    ...
    def test_regional_comparison(self):
        ...
        region2, _ = RegionConfig.objects.get_or_create(
            code="TS2",
            defaults={
                "name": "TestRegion2",
                "default_language": "en",
        ...
        RegionalComplianceRequirement.objects.create(
            region=region2, rule=rule2, status="pending", created_by=self.user
        )
    ...
    def test_critical_items_overdue(self):
        ...
        RegionalComplianceRequirement.objects.create(
            region=self.region, rule=rule, deadline=past_deadline, created_by=self.user
        )
