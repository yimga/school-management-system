...
class Command(BaseCommand):
    ...
    def _check_foreign_keys(self):
        ...
        if self.verbose or orphaned == 0:
            self.stdout.write(
                f"  ✓ Foreign key integrity OK ({orphaned} orphaned found)"
            )
    ...
    def _check_term_positions(self):
        ...
        invalid_positions = Term.objects.filter(position__isnull=False).exclude(
            ...
            self.issues.append(
                {
                    ...
                    "terms": list(
                        invalid_positions.values_list("id", "name", "position")
                    ),
                }
            )
        ...
        dup_positions = (
            Term.objects.exclude(position__isnull=True)
            .values("academic_year", "position")
            .annotate(count=Count("id"))
            .filter(count__gt=1)
        )
    ...
    def _check_user_profiles(self):
        ...
        teachers_no_profile = User.objects.filter(role="TEACHER").exclude(
            id__in=TeacherProfile.objects.values_list("user_id")
        )
    ...
    def _check_audit_logs(self):
        ...
        recent_evals = Evaluation.objects.filter(updated_at__gte=last_week).count()

        recent_audit = AuditLog.objects.filter(
            timestamp__gte=last_week, model_name="Evaluation"
        ).count()
        ...
        AccessLog.objects.filter(timestamp__gte=last_week).count()

        if self.verbose or (recent_evals == 0 or recent_audit > 0):
            self.stdout.write(
                f"  ✓ Audit logs present ({recent_audit} audit logs this week)"
            )
    ...
    def _check_payment_consistency(self):
        ...
        unmatched_payments = Payment.objects.filter(invoice__isnull=True).count()
    ...
    def _check_role_based_access(self):
        ...
        non_super_admins = User.objects.filter(role="ADMIN", is_superuser=False).count()
