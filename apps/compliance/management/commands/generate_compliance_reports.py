...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--daily", action="store_true", help="Generate daily audit trail summary"
        )
        parser.add_argument(
            "--weekly", action="store_true", help="Generate weekly data access analysis"
        )
        parser.add_argument(
            "--monthly", action="store_true", help="Generate monthly integrity check"
        )
        parser.add_argument(
            "--all", action="store_true", help="Generate all report types"
        )
        parser.add_argument(
            "--school-id",
    ...
    def handle(self, *args, **options):
        ...
        if school_id:
            self.scope_school = School.objects.filter(
                pk=school_id, is_active=True
            ).first()
    ...
    def _generate_daily_audit(self):
        ...
        stats = {
            ...
            "critical_actions": audit_logs.filter(sensitivity="CRITICAL").count(),
        }
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✓ Daily audit report created: {stats['total_actions']} actions"
                )
            )
            return report
            ...
            log_exception_with_context(
                ...
                extra={
                    "command": "generate_compliance_reports",
                    "report_type": "AUDIT_TRAIL",
                    "error": str(e),
                },
            )
            self.stdout.write(
                self.style.ERROR(f"✗ Failed to create daily audit report: {e}")
            )
            return None
    ...
    def _generate_weekly_access(self):
        ...
        access_logs = scope_access_logs(
            AccessLog.objects.filter(timestamp__gte=week_ago), self.scope_school
        )
        ...
        stats = {
            ...
            "by_type": dict(
                access_logs.values("access_type")
                .annotate(count=Count("id"))
                .values_list("access_type", "count")
            ),
            "by_user": dict(
                access_logs.values("user__username")
                .annotate(count=Count("id"))
                .values_list("user__username", "count")
            ),
            "top_resources": list(
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✓ Weekly access report created: {total_access} accesses"
                )
            )
            return report
        except _GENERATE_COMPLIANCE_REPORTS_ERRORS as e:
            ...
            self.stdout.write(
                self.style.ERROR(f"✗ Failed to create weekly access report: {e}")
            )
            return None
    ...
    def _generate_monthly_integrity(self):
        ...
        users_without_audit = list(
            school_user_queryset(self.scope_school)
            .exclude(
                id__in=scope_audit_logs(
                    AuditLog.objects.all(), self.scope_school
        ...
        if orphaned_sessions > 0:
            issues.append({"type": "ORPHANED_SESSIONS", "count": orphaned_sessions})
            # Attempt to fix: delete orphaned sessions
            try:
                deleted = UserActivitySession.objects.filter(
                    user__isnull=True
                ).delete()[0]
        ...
        suspicious = scope_sessions(
            UserActivitySession.objects.filter(
                is_suspicious=True, login_timestamp__gte=month_ago
            ),
            self.scope_school,
        ).count()

        if suspicious > 0:
            issues.append({"type": "SUSPICIOUS_SESSIONS", "count": suspicious})
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✓ Monthly integrity report created: {len(issues)} issues"
                )
            )
            for fix in fixes:
            ...
            self.stdout.write(
                self.style.ERROR(f"✗ Failed to create monthly integrity report: {e}")
            )
            return None
