...
MIDDLEWARE_PROTECTED_PREFIXES = ("super/",)
...
class Command(BaseCommand):
    ...
    def _check_patterns(self, patterns, prefix=""):
        ...
        for pattern in patterns:
            ...
            if any(
                skip in str(path)
                for skip in ["admin/", "static/", "media/", ".well-known"]
            ):
                continue
    ...
    def _check_view_access(self, path, view):
        ...
        has_login_required = any(
            self._has_login_protection(func) for func in view_funcs
        )
        has_permission_required = any(
            self._has_decorator(func, "permission_required") for func in view_funcs
        )
        ...
        has_staff_member_required = any(
            self._has_staff_member_protection(func) for func in view_funcs
        )
        has_user_passes_test = any(
            self._has_decorator(func, "user_passes_test") for func in view_funcs
        )
        has_drf_permission_control = self._has_drf_permission_control(view)
        has_manual_auth_guard = any(
            self._has_manual_auth_guard(func) for func in view_funcs
        )
        has_manual_permission_guard = any(
            self._has_manual_permission_guard(func) for func in view_funcs
        )
        ...
        needs_protection = not is_public_route and not any(
            skip in str(path)
            for skip in [
                "login",
                "logout",
        ...
        if self.verbose:
            ...
            self.stdout.write(
                f"  {status}: {', '.join(decorators) if decorators else 'none'}"
            )
    ...
    def _is_middleware_protected(self, path: str) -> bool:
        normalized = str(path or "").lstrip("/")
        return any(
            normalized.startswith(prefix) for prefix in MIDDLEWARE_PROTECTED_PREFIXES
        )
