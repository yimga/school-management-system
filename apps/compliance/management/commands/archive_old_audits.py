...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        old_sessions = UserActivitySession.objects.filter(login_timestamp__lt=session_cutoff)
        ...
        if dry_run:
            ...
            self.stdout.write(f"  - {audit_count} AuditLog records (older than {audit_log_days} days)")
            self.stdout.write(f"  - {access_count} AccessLog records (older than {access_log_days} days)")
            self.stdout.write(f"  - {session_count} UserActivitySession records (older than {session_days} days)")
            self.stdout.write(f"\nOldest records:")
            ...
            if session_count > 0:
                ...
                self.stdout.write(f"  UserActivitySession: {oldest_session.login_timestamp}")
        else:
            # Delete old records
