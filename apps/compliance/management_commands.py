...
class CheckComplianceCommand(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if failed_logins > 5:
            self.stdout.write(
                self.style.ERROR(f"✗ Failed Logins (1h): {failed_logins}")
            )
        else:
...
class DetectThreatsCommand(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✓ Threat detection complete. Open incidents: {open_incidents}"
            )
        )
...
class VerifyDataIntegrityCommand(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if errors:
            self.stdout.write(
                self.style.ERROR(f"✗ {len(errors)} integrity issues found")
            )
            for error in errors:
...
class GenerateComplianceReportCommand(BaseCommand):
    ...
    def generate_audit_trail(self, start, end):
        ...
        logs = (
            AuditLog.objects.filter(timestamp__range=[start, end])
            .values("user", "action")
            .annotate(count=Count("id"))
        )

        return {
            "total_changes": AuditLog.objects.filter(
    ...
    def generate_access_control(self, start, end):
        ...
        access_logs = AccessLog.objects.filter(timestamp__range=[start, end])
