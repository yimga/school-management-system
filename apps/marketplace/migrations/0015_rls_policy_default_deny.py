# RLS default-deny for tenant-scoped marketplace tables.
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in TABLES:
            short = table.replace("marketplace_", "", 1)
            policy_name = f"marketplace_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("marketplace", "0014_enable_rls_postgresql"),
    ]
