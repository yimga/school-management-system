...
class PortalRoleHelperTests(SimpleTestCase):
    ...
    def test_get_effective_portal_role_falls_back_to_user_role_when_preference_lookup_fails(self):
        ...
        with patch("apps.accounts.portal_roles.get_effective_site_settings", side_effect=AttributeError):
...
class ImpersonationTokenHelperTests(SimpleTestCase):
    def test_parse_impersonation_token_returns_none_for_invalid_payload(self):
        ...
        with patch("apps.accounts.views_impersonation.TimestampSigner.unsign", return_value=payload):
    ...
    def test_parse_impersonation_token_returns_payload_for_valid_token(self):
        payload = base64.b64encode(json.dumps({"school_id": "abc", "user_id": 42}).encode()).decode()
