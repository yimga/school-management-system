"""A–Z owner onboarding: account → brand → MFA waive/setup → provision kick.

Proves the new-school path that was stuck in production:
- wizard completes without a confirmed MFA device when soft-launch allows waive
...
class OwnerOnboardingAZFlowTests(TestCase):
    ...
    def test_az_password_brand_waive_mfa_marks_complete(self):
        ...
        with mock.patch(
            "apps.accounts.views_owner_onboarding._owner_mfa_waive_allowed",
            return_value=True,
        ):
            r4 = self.client.post(
        ...
        self.assertEqual(r4.url, reverse("accounts:owner_onboarding_done"))

        with mock.patch(
            "apps.schools.tasks.complete_provisioning_for_school",
            return_value={"queued": True, "is_active": False},
        ):
            # Avoid public→tenant hop so the same client finishes the wizard.
            with mock.patch(
                "apps.schools.provision_email_urls.tenant_subdomain_host_exists",
                return_value=False,
            ):
                r5 = self.client.get(reverse("accounts:owner_onboarding_done"))
        self.assertEqual(r5.status_code, 200)
        self.school.refresh_from_db()
        self.assertTrue(self.school.settings["owner_onboarding"]["completed"])
        self.assertTrue(self.school.settings["owner_onboarding"].get("mfa_waived"))
