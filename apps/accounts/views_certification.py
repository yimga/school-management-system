...
def _year_gce_enabled_or_forbidden(request, year: AcademicYear | None):
    if not year:
        return HttpResponseForbidden("No active academic year. Create/activate an academic year first.")
...
def _check_session_lock(session: CertificationExamSession, action: str, request, allow_override=True):
    """
    ...
    if action == "add_candidates":
        if not session.can_add_candidates(user=request.user if request.user.is_authenticated else None, check_override=allow_override):
            ...
            can_override = allow_override and _is_admin_user(request.user) if request.user.is_authenticated else False
            return False, lock_reason, can_override

    elif action == "edit_candidates":
        if not session.can_edit_candidates(user=request.user if request.user.is_authenticated else None, check_override=allow_override):
            if session.registration_closes_at and now > session.registration_closes_at:
...
def certification_home(request):
    ...
    sessions = CertificationExamSession.objects.filter(academic_year=year).order_by("-created_at")
    return render(
        request,
        "accounts/certification_home.html",
        {
            ...
            "admin_session_url": reverse("admin:academics_certificationexamsession_changelist"),
        },
    )
...
def certification_session_detail(request, session_id: int):
    ...
    # Check lock status
    can_add, add_lock_reason, can_override_add = _check_session_lock(session, "add_candidates", request)
    can_edit, edit_lock_reason, can_override_edit = _check_session_lock(session, "edit_candidates", request)
    ...
    return render(
        ...
        "accounts/certification_session_detail.html",
        {
            ...
            "has_registration_override": session.has_admin_override("registration_locked"),
...
def certification_export_zip(request, session_id: int):
    ...
    use_document_checklist = session.document_checklist or (preset and preset.document_checklists.filter(is_default_for_preset=True).exists())
    ...
    if use_ca_export:
        ...
        for subj_code in subject_codes:
            if use_practical_split:
                headers.extend([f"{subj_code}_theory", f"{subj_code}_practical", f"{subj_code}_total"])
            else:
    ...
    if use_document_checklist:
        ...
        if candidates.exists():
            ...
            checklist = session.document_checklist or (preset and preset.document_checklists.filter(is_default_for_preset=True).first())
            if checklist:
    ...
    centre_no = session.exam_centre_number.strip() if session.exam_centre_number else "CENTRE"
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        zf.writestr("photos/.keep", "Photos folder (<= 50KB per candidate as per board guidance).")
...
class CertificationBulkCandidateForm(forms.Form):
    classrooms = forms.ModelMultipleChoiceField(queryset=Classroom.objects.none(), required=True)
    specialties = forms.ModelMultipleChoiceField(queryset=Specialty.objects.none(), required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["classrooms"].widget.attrs.update({"class": "form-select", "size": "8"})
        self.fields["specialties"].widget.attrs.update({"class": "form-select", "size": "8"})
        self.fields["include_inactive_students"].widget.attrs.update({"class": "form-check-input"})
...
def certification_bulk_add_candidates(request, session_id: int):
    ...
    is_allowed, lock_reason, can_override = _check_session_lock(session, "add_candidates", request)
    if not is_allowed:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            students_qs = StudentProfile.objects.filter(academic_year=year, classroom__in=classrooms)
            if specialties:
                students_qs = students_qs.filter(specialty__in=specialties)
            if not include_inactive:
                students_qs = students_qs.filter(is_active=True)

            students_qs = students_qs.select_related("classroom", "specialty").order_by("last_name", "first_name")
            ...
            with transaction.atomic():
                for student in students_qs.iterator(chunk_size=200):
                    if skip_existing and CertificationCandidate.objects.filter(session=session, student=student).exists():
                        skipped += 1
                        continue
                    obj, was_created = CertificationCandidate.objects.get_or_create(session=session, student=student)
                    if was_created:
                        created += 1
                    else:
                        skipped += 1
    ...
    return render(
        ...
        "accounts/certification_bulk_add_candidates.html",
        {
            ...
            "has_override": request.GET.get("override") == "1" or session.has_admin_override("registration_locked"),
        },
    )
...
def certification_session_override(request, session_id: int):
    ...
    if action == "set":
        ...
        messages.success(request, f"Override enabled for {override_type.replace('_', ' ')}.")
    elif action == "clear":
        ...
        messages.success(request, f"Override cleared for {override_type.replace('_', ' ')}.")
    else:
