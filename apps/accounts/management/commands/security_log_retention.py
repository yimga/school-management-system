...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--dry-run", action="store_true", help="Only report counts, do not delete."
        )
        parser.add_argument(
            "--anonymize-days",
            type=int,
            default=90,
            help="Anonymize IP for logs older than N days (0=disable).",
        )
        parser.add_argument(
            "--delete-days",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        cutoff_anonymize = (
            now - timedelta(days=anonymize_days) if anonymize_days else None
        )
        ...
        if count_delete:
            self.stdout.write(
                f"Would delete {count_delete} logs older than {delete_days} days (before {cutoff_delete.date()})."
            )
            if not dry_run:
                to_delete.delete()
        ...
        if cutoff_anonymize and anonymize_days > 0:
            to_anon = (
                SecurityAuditLog.objects.filter(created_at__lt=cutoff_anonymize)
                .exclude(ip_address="")
            ...
            if count_anon:
                self.stdout.write(
                    f"Would anonymize IP for {count_anon} logs older than {anonymize_days} days."
                )
                if not dry_run:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"Anonymized {count_anon} logs.")
                    )
        return
