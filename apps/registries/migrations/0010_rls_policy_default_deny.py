# RLS default-deny for tenant-scoped registries tables.
...
def apply_default_deny(apps, schema_editor):
    ...
    with connection.cursor() as cursor:
        for table in TABLES:
            short = table.replace("registries_", "", 1)
            policy_name = f"registries_tenant_{short}"
...
class Migration(migrations.Migration):
    dependencies = [
        ("registries", "0009_enable_rls_postgresql"),
    ]
