"""Global-local gap closure (batch 1488) — contract pins for student360.
...
class DualIdentityProfileContractTest(SimpleTestCase):
...
class StakeholderRelationshipGraphTest(SimpleTestCase):
    def test_artifact_and_key_path(self):
        artifact = _artifact("crm_lifecycle_gap_closure.json")
        self.assertTrue(artifact.is_file(), "missing audit artifact: " + str(artifact))
        key = ROOT / "apps/student360"
