...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--file",
            required=True,
            help="Path to a JSON file containing a list of snapshots.",
        )
    ...
    def handle(self, *args, **options):
        ...
        snapshots = (
            payload if isinstance(payload, list) else payload.get("snapshots") or []
        )
        if not isinstance(snapshots, list):
            raise CommandError(
                "Snapshot file must contain a list or a top-level 'snapshots' list."
            )
        ...
        for item in snapshots:
            ...
            apply_processor_snapshot(
                school=school,
                processor_code=str(item.get("processor_code") or "").strip()
                or "manual",
                ...
                external_customer_ref=str(
                    item.get("external_customer_ref") or ""
                ).strip(),
                external_subscription_ref=str(
                    item.get("external_subscription_ref") or ""
                ).strip(),
                currency_code=item.get("currency_code"),
                billed_amount=item.get("billed_amount"),
                current_period_start=parse_datetime(item["current_period_start"])
                if item.get("current_period_start")
                else None,
                current_period_end=parse_datetime(item["current_period_end"])
                if item.get("current_period_end")
                else None,
                trial_end_date=parse_date(item["trial_end_date"])
                if item.get("trial_end_date")
                else None,
                happened_at=parse_datetime(item["happened_at"])
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Imported {applied} platform billing snapshot(s).")
        )
