...
def invoice_lines_from_app_ledger(school, *, period_start=None, period_end=None):
    """
    Generate invoice line items from platform ledger entries for app installs (6.3 / 29.10).
    Used for proration and invoice line generation from ledger. Returns list of dicts
    with keys: description, amount, currency_code, reference, source, source_ref, happened_at.
    period_start/period_end: optional date filter for happened_at (inclusive).
    """
    from .models import PlatformLedgerEntry

    account, _ = ensure_billing_account_for_school(school)
    qs = PlatformLedgerEntry.objects.filter(
        school=school,
        billing_account=account,
        source="marketplace_app_install",
        status=PlatformLedgerEntry.Status.POSTED,
    ...
    if period_start is not None:
        qs = qs.filter(happened_at__date__gte=period_start)
    if period_end is not None:
        qs = qs.filter(happened_at__date__lte=period_end)
    lines = []
    for entry in qs:
        lines.append({
            "description": entry.description or "App install",
            "amount": entry.amount,
